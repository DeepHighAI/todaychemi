...
describe('signUpWithEmail', () => {
  it('throws WeakPasswordError with tooShort when password is under 8 chars', async () => {
    ...
    await expect(signUpWithEmail('user@example.com', 'test')).rejects.toBeInstanceOf(
      WeakPasswordError,
    );
  ...
  it('throws WeakPasswordError with missingClasses for letters-only password', async () => {
    const { signUpWithEmail, WeakPasswordError } = await import('@/lib/auth/email');
    const error = await signUpWithEmail('user@example.com', 'abcdefgh').catch((e) => e);
  ...
  it('calls supabase.auth.signUp with valid password (test1234)', async () => {
  ...
  it('throws when supabase signUp returns an error', async () => {
    ...
    await expect(signUpWithEmail('user@example.com', 'test1234')).rejects.toEqual(fakeError);
  });
  ...
  it('resolves without a value on successful signup', async () => {
    ...
    const result = await signUpWithEmail('new@example.com', 'newuser1234');
