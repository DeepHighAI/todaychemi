...
F5(케미카드)·F6(오늘 홈)·F8(오늘의 나는?)는 LLM API 호출이 발생한다.
...
| F8 오늘의 나는? | ~$0.05/모드 (GPT-5) | `cache_key`(chart_hash + type + target_date) 기준 캐시. 같은 날 같은 모드 재호출 = 무료 |
...
| WhatifTrigger | `오늘의 나는?` spotlight CTA |
