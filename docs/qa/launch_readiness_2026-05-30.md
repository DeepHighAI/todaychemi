...
## 2026-06-25 Pre-Launch Field-Bug Fixes Update

Field QA from a real Apps-in-Toss miniapp build surfaced three user-facing bugs. All three were fixed as pre-launch polish (branch `codex/today-self-refresh`). None change the launch verdict, pricing, scope, or any §1.1 decision, and none touch external dashboard settings — the integrated launch gate still fails only on the same external setup/evidence items (launch env, Auth, OpenAI/ZDR, Toss live, Vercel, Operations/E2E, External settings checklist). **Verdict unchanged: 오픈 보류 (external settings/evidence only).**

- **Issue 1 — onboarding bounce (miniapp).** After completing onboarding and tapping 시작하기, the user was intermittently returned to the start of onboarding (succeeded on retry). Root cause: `miniapp/.../Step4Review.tsx` awaited `invalidateQueries(['me-chart'])`, but TanStack Query v5 defaults to `refetchType: 'active'`; `['me-chart']` is inactive while on `/onboarding` (outside `ProfileGate`), so the await resolved without refetching and `ProfileGate` read a stale `null` chart and redirected back. Not an auth failure — `/api/onboarding` already accepts the miniapp Bearer token. Fix: `invalidateQueries({ queryKey: ['me-chart'], refetchType: 'all' })` (await the inactive refetch before navigate) plus a `ProfileGate` defense that holds `LoadingState` while a chart refetch is in flight on `null` data instead of bouncing.
- **Issue 2 — birthday wheel scroll jank (miniapp).** The birthday wheel-picker scroll felt unnatural. Root cause in `miniapp/.../wheel-column.tsx`: `onChange` fired on every scroll frame, re-rendering the parent mid-scroll and fighting native scroll-snap; programmatic re-centering was instant; items had no `scroll-snap-stop`. Fix: debounce `onChange` to scroll-rest only (settle timer), add `scroll-snap-stop: always`, add iOS momentum (`-webkit-overflow-scrolling: touch`) and `overscroll-behavior: contain`. The web picker shares the pattern and is deferred to a follow-up (out of scope by decision).
- **Issue 3 — "오늘 메시지를 준비하지 못했어요" today fallback (web + miniapp).** The today card showed the server TEMPLATE fallback (HTTP 200, `is_fallback: true`). Root cause: GPT-5 generation typically takes 14–26s but the server `TODAY_LLM_TIMEOUT_MS` was 25s, so slow cold generations hit `LLM_TIMEOUT` → TEMPLATE. Fix: raise `TODAY_LLM_TIMEOUT_MS` to 45s within the route's `maxDuration=60` (Hobby cap) so slow generations complete and cache; pairs with the already-in-progress 40s reassurance spinner (`LoadingState` info tone). No LLM model change and no client retry (by decision). Shared Next.js backend, so this fixes web and miniapp together.

Verification (this change): root `pnpm tsc --noEmit`, `pnpm lint`, and targeted `pnpm vitest run` for the touched files pass; miniapp `tsc --noEmit` and `vitest run` pass. Live on-device re-verification still requires a `.ait` rebuild + console re-upload (operator). LLM-cost live smoke for Issue 3 is deferred to staging/production.
