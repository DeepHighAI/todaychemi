...
> 본 문서는 LLM에 절대 전달하지 않는 PII 필드의 단일 truth source다. AGENTS.md §5, tech_stack.md §3.5, FRONTEND-PREP.md §11.2, fluttering-gathering-island.md §6.4는 본 문서를 참조한다.
...
> **gender 포함 근거**: AGENTS.md §5는 6필드(birth_date, name, nickname, email, birth_place, gender) 모두 미전달을 명시. 이전 스펙(fortune_architecture.md §7.6)에서 gender·birth_place를 LLM 위탁 가능으로 분류한 것은 ADR-004·AGENTS.md §5와 충돌하므로 **무효**다.
...
OpenAI production traffic은 ZDR(Zero Data Retention) 적용이 확인된 `OPENAI_PROJECT_ID`와 같은 프로젝트의 API key로만 호출한다. Anthropic fallback은 외부 LLM subprocessor로 취급하며, production key 설정 전 retention/training/DPA 조건을 확인하고 동일한 non-PII allowlist만 전송한다.

## 위반 시 절차

코드 리뷰에서 5필드 + gender 원본이 LLM 페이로드 직렬화 경로에 포함된 것을 발견하면:
1. 즉시 PR 차단
2. AGENTS.md §1.1에 따라 사용자 보고
3. ADR-004 위반으로 기록
