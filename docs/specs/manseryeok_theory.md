...
**고전 근거**: 淵海子平 「地支藏遁歌」(`classic_yhzp_006/007`)와 12지 전수 일치 — RAG 검수 리포트(`docs/qa/myeongri_rag_review_2026_06_12.md` 항목 1) 참조. 가중 비율의 사령 일수 대조는 三命通會 「論人元司事」(`classic_smth_006/007`).
...
**가중치 (정수 ×10 스케일, derived_version 2)**: 천간 1글자 = 10 (동일 스케일).
부동소수(0.3+0.5류) 비결정성 회피를 위해 정수만 사용(ADR-035).

| 지지 구분 | 정기 | 중기 | 여기 | 근거 |
|---|---|---|---|---|
| 기본 8지 | 10 | 5 | 3 | 정기 우위 — 고전 사령 일수(18~23일/30일) 방향 정합 |
| 사계월(辰戌丑未) | 10 | 3 | 5 | **R1 (2026-06-12 사용자 확정)**: 三命通會 사령 일수 여기7 > 중기(묘고)5 서열 반영 |

- 구현: `jijangganWeightsFor(branch)` (`src/lib/saju/jijanggan.ts`). 사계월은 3슬롯이 항상
  채워져 있어 교환이 오행 총합을 보존한다(테스트 잠금).
- **derived_version 1→2 마이그레이션**: theory v3·chart_hash 불변(기둥 입력 무변). 저장된
  v1 derived 는 `resolveDerivedForLlm` 경계에서 조용히 재계산(self-heal) — DB 재기록 불필요.
  캐시 회전은 동반 프롬프트 범프(6모드 v0.17 등)가 담당 (ADR-040 cache-coupling).
- R2(정기 가중 일수 비례 상향)는 **불채택** — 사령일수 코퍼스와 장간가의 구성 학파 차이
  (卯 여기 甲 유무·辰 중기 壬/癸·午 己 유무·亥 戊 유무)로 "일수 비례" 매핑이 정의 불가
  (검수 리포트 R2 절 참조).
...
산식 전문 (**학파 단순화 — RAG 검수(2026-06-12): 구조(월령 최우선·일간 중심·억부 대응)는 고전
직접 근거(`classic_yhzp_008`·`classic_zpjj_001`·`classic_jcs_006`), 계수·임계 수치는 고전 무수치
영역의 자체 단순화로 잠정 유지** — `src/lib/saju/sinkang.ts`):
...
오행 레벨 판정(천간 픽 아님). **RAG 검수(2026-06-12): 억부 원칙은 고전 직접 근거
(`classic_jcs_006/009`), 신약→인성 고정·중화 보완 룰은 현대 통설 범위 내 선택으로 명기 유지.
조후·종격 미반영 한계는 "용신 후보" 라벨로 수용** — `src/lib/saju/yongsin.ts`:
...
> **RAG 고전 근거 검수 완료 (2026-06-12)** — 공개 고전 원문 14건을 RAG 자산화하고 verbatim
> 근거로 7항목 전부 판정: `docs/qa/myeongri_rag_review_2026_06_12.md` (PASS 6 · 검증 불가 명기 1 ·
> **R1 적용 완료(derived v2)** · R2 불채택). 크라우드 휴먼 검수는 §7.2 백로그 유지.
> 외부 전문가용 자기완결 의뢰서: `docs/qa/myeongri_specialist_review_request.md` (코드 지식 불필요)

검수 항목 — RAG 검수 결과는 위 리포트, 향후 휴먼 검수 시 동일 항목 재확인:
