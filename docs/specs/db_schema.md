...
create table public.user_charts (
  ...
  chart_core            jsonb   not null,  -- pillars, day_master, five_elements_balance, sinsal_tags, derived(v3+, ADR-040)
...
> 교차분석(`cross_analysis`, ADR-040)은 **비영속** — 요청 시 매번 계산되며 어떤 테이블에도 저장되지 않는다 (derived는 chart_core jsonb 내장).
