...
> **Status: DRAFT — §1.1 결정 대기 4건**
>
> 결정 매트릭스 4개 항목이 미결정이므로 구현 보류 상태.
> 사용자가 §1.1 절차로 4건을 확정하면 이 문서를 LOCK으로 변경 후 TDD 진입.
...
| # | 결정 항목 | 옵션 A | 옵션 B | 추천 | 영향 파일 |
|---|---|---|---|---|---|
| **D1** | **라우트 형태** | Route Handler `POST /api/hapcards/[id]/replay` | Server Action `POST /actions/replay` (`api_routes.md:15`) | A — 테스트 가능성↑, 캐시 헤더 제어 가능, 기존 hapcards 패턴 일관성 | `api_routes.md` 갱신(§12 의무), Next.js routing 디렉토리 신규 |
| **D2** | **요금** | 4p 차감(50% of 8p original) (`api_routes.md:15` 기재) | 8p 차감(동일 요금) | A — `api_routes.md` 명시값 채택 | `token_ledger` INSERT delta 값, UI 설명 문구 |
| **D3** | **레이트리미트 (TBD-5)** | 인연당 일 1회 | 인연당 주 3회 | **미결 — 사용자 결정 필요** | DB `UNIQUE(hapcard_id, date_trunc('day', created_at))` 또는 app-side counter, `REPLAY_RATE_LIMITED` 에러 코드 |
| **D4** | **`token_ledger.reason` 네이밍** | `'replay_use'` (`db_schema.md:319` canonical) | `'replay'` (`payments.md` 표기) | A — `db_schema.md`가 DDL 권위 출처 | `payments.md` 정정(§12 의무) |

> **결정 후 작업**: D1~D4 확정 → `api_routes.md` + `db_schema.md` + `payments.md` 동시 갱신(§12) → 이 문서 LOCK → TDD 진입.
...
동일 `(hapcard_id, 일진 date)` 2회 호출 시 첫 호출 결과 반환(no double charge).

구현 방안 (D3 결정 후 선택):
- DB unique constraint: `UNIQUE(hapcard_id, date_trunc('day', created_at))` + `ON CONFLICT DO NOTHING RETURNING *`
- 또는 cache key 조회 후 존재 시 기존 행 반환
