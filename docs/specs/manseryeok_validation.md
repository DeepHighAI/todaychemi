...
| 소스 | 등급 | 용도 |
|---|---|---|
| **한국천문연구원(KASI) 천체력·절기 데이터** | **진본 (Reference Truth)** | 최종 중재자. 사주 계산의 물리학적 기반. |
| `urstory/manseryeok-js` | 1차 동의 (Agreement) | KASI 기반 오픈소스, fallback 1순위 |
| 포스텔러 만세력 Pro | 참고 동의 | 상용 서비스 일치율 참고 |
| 네이버 만세력 | 참고 동의 | 2차 참고 |

**지표 정의**:
- `accuracy`: ssaju vs KASI 기반 자체 계산 일치율 (진본 일치) — **Gate 통과 기준**
