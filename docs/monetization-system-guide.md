...
| `NEXT_PUBLIC_TOSS_CLIENT_KEY` | client | TossPayments 위젯 클라이언트 키 |
| `TOSS_SECRET_KEY` | server only | TossPayments confirm/조회 API |
| `CSRF_SECRET` | server only | HMAC 서명 키 (32자 이상 랜덤) |
| `DAILY_LOGIN_TOKENS` | server | 일일 로그인 보너스 (기본값 `2`) |
| `SIGNUP_BONUS_TOKENS` | server | 가입 보너스 (기본값 `20`) |
...
CREATE TABLE IF NOT EXISTS public.token_transactions (
    ...
    type TEXT NOT NULL CHECK (type IN (
        ...
        'spend_feature',       -- 기능 사용 차감
        'spend_coaching'       -- 코칭 차감 (현재 미사용)
    )),
    description TEXT,
    reference_id TEXT,
...
POST /api/webhooks/toss
  1. HMAC 없음 → getPayment(paymentKey)로 Toss 재조회 (진실원천 확인)
...
> ⚠️ Toss webhook HMAC 검증 미구현 (`src/lib/payments/cancellation.ts`의 `calculateTokensToRevoke` 참조). 새 프로젝트에서는 [Toss Webhook Secret](https://docs.tosspayments.com/reference/webhook)으로 HMAC 서명 검증 추가 권장.
...
**원본**: `src/app/api/tokens/daily-login/route.ts`

```typescript
const TOKENS_PER_DAILY_LOGIN = Number(process.env.DAILY_LOGIN_TOKENS) || 2;

// POST /api/tokens/daily-login
// 1. validateCsrfToken() → getAuthenticatedUser()
// 2. KST 자정 이후 'daily_login' type count 확인
//    → count > 0: { tokensGranted: 0, alreadyClaimed: true }
// 3. adjust_tokens(+TOKENS_PER_DAILY_LOGIN, 'daily_login', null)
// 4. { tokensGranted: 2, alreadyClaimed: false }
```
...
**원본**: `src/app/api/auth/register/route.ts:151-171`, `src/app/api/auth/callback/route.ts:65-92`

```typescript
const SIGNUP_BONUS_TOKENS = Number(process.env.SIGNUP_BONUS_TOKENS) || 20;

// 이메일 가입 (register route):
//   가입 성공 → 'signup_bonus' count > 0 이면 스킵 (멱등)
//   → adjust_tokens(+20, 'signup_bonus', `signup_${userId}`)

// OAuth 가입 (callback route):
//   created_at이 30초 이내 신규 계정만 보너스 지급 (기존 로그인 제외)
//   → 동일 count 기반 멱등성

// Achievement 보상도 'signup_bonus' type 재사용 (src/lib/gamification/checker.ts)
```
...
⑤ Rail 3 — 토큰 적립
   - src/app/api/tokens/daily-login/route.ts
   - src/app/api/ads/reward-verify/route.ts
   - 가입 보너스: auth/register + auth/callback에 adjust_tokens 삽입
...
| `NEXT_PUBLIC_TOSS_CLIENT_KEY` | `test_ck_...` | ✅ (웹 결제) |
| `TOSS_SECRET_KEY` | `test_sk_...` | ✅ (웹 결제) |
| `CSRF_SECRET` | `<32자 이상 랜덤>` | ✅ |
| `DAILY_LOGIN_TOKENS` | `2` | 선택 (기본 2) |
| `SIGNUP_BONUS_TOKENS` | `20` | 선택 (기본 20) |
...
| Webhook 보안 | HMAC 없음, getPayment 재조회로 대체 | Toss Webhook Secret으로 HMAC 서명 검증 추가 |
