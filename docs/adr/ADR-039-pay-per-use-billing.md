...
**Deciders:** batisututu
**Amended:** 2026-06-07 — 초기 3상품 정가와 부적 차감량 1차 인상 (§1.1 D6 확정. 사유: 앱인토스 인앱결제 수수료 약 20% 반영 + 웹·미니앱 가격 통일. 1부적=100원 등가 유지)
**Amended:** 2026-06-10 — **인연 등록 슬롯 과금 추가(모델 B)**: 인연 2명까지 무료 등록, 3번째부터 `relation_slot` 1,000원/10부적. 기존 기능별 과금 유지·병행 (§1.1 승인, 본 문서 §9)
**Amended:** 2026-06-14 — **오픈초기 50% 할인 이벤트 초안**: 현금 결제 금액만 할인하고 정가와 무료 부적 차감량은 유지하는 정책을 확정. 실제 금액은 2026-06-19/06-22 확정가로 대체
**Amended:** 2026-06-19 — **가격 인상 (§1.1 확정)**: 정가 케미카드·인연 등록 1,000 → **1,100원**, 오늘의 나는? 800 → **880원**, 케미 다시 맞추기 600 → **880원**. 오픈초기 50% 할인 현금가 **550 / 440 / 440 / 550원**, 무료 부적 10/8/6/10 → **11/9/9/11p** (부적도 비례, 1부적≈100원 유지; 880→9 반올림). 50% 할인 메커니즘·웹·미니앱 통일 유지
**Amended:** 2026-06-22 — **앱인토스 IAP 콘솔 확정가 반영 (§1.1 사용자 확정)**: `hapcard` 케미카드 판매가 550원(공급가 500원), `whatif` 오늘의 나는? 판매가 440원(공급가 400원), `replay` 케미 다시 맞추기 판매가 440원(공급가 400원), `relation_slot` 인연 슬롯 판매가 550원(공급가 500원). `feature-prices.ts`, 미니앱 표시가, 앱인토스 SKU 판매가, Vercel 서버 SKU 매핑을 동일하게 유지
...
유료 기능은 **케미카드(hapcard)·오늘의 나는?(whatif)·케미 다시 맞추기(replay)·인연 슬롯(relation_slot)** 4종이다. 이들을 사용
...
무료 부적 잔액이 충분하면 `token_ledger`에서 차감(케미카드 11p·오늘의 나는? 9p·케미 다시 맞추기 9p·인연 슬롯 11p).
잔액 부족 시 **1회성 현금 결제**(웹: Toss Payment Widget V2 / 앱인토스 미니앱: 인앱결제 IAP — 2026-06-07 D3)로 전환한다. 구독·번들 없음.

### 2. 가격 단일 출처
정가: 케미카드 1,100원 / 오늘의 나는? 880원 / 케미 다시 맞추기 880원 / 인연 슬롯(3번째+) 1,100원.
오픈초기 이벤트 현금가: 케미카드 550원 / 오늘의 나는? 440원 / 케미 다시 맞추기 440원 / 인연 슬롯(3번째+) 550원.
앱인토스 콘솔 공급가는 각각 500원 / 400원 / 400원 / 500원이며 VAT 포함 판매가가 위 현금가와 일치한다.
(2026-06-07·06-10·06-14·06-19·06-22 개정, 웹·미니앱 통일). 유일 출처는
...
`feature_ref` 포맷: 케미카드/오늘의 나는? = `cache_key`, 케미 다시 맞추기 = `replay:{hapcard_id}:{jinjin_date}`,
...
- **하이브리드**: 무료 부적 11p 우선 차감 → 부족 시 오픈초기 현금 550원(정가 1,100원, §1과 동일, 1부적≈100원 등가).
...
**해소·수용 리스크(문서화)**: ① **count 게이트 TOCTOU — 차단 완료(§1.1 2026-06-10)**. 초기엔
2-스텝(count SELECT 후 INSERT)이라 동시 제출 시 무료 슬롯 초과가 가능했고 /review·/codex 가
CRITICAL/BLOCK 지적, 일시 수용했으나 후속으로 **원자적 조건부 INSERT RPC**
(`insert_relation_if_under_free_cap`, `20260610140000`)로 차단. 무료 등록을
`INSERT ... SELECT ... WHERE (count < FREE_RELATION_SLOTS)` 단일문으로 묶어 동시 요청도
row-level 직렬화로 정확히 한도까지만 통과(0행 = 슬롯 초과 → 유료 경로). ② 무료경로 더블탭 = 2 pending·2
...
pending 마다 open row 가 쌓일 수 있다(머니 리스크 아님, row 비대). **draft PII purge 도입
(§1.1 2026-06-10, `20260610120000_pending_draft_purge.sql`)**: 미결제·미머티리얼라이즈 행은
30일 후 삭제(pending/confirmed 결제 보유 행 = paid 고아는 절대 보호), 전달 완료·삭제 소비
행은 7일 후 draft 만 `'{}'` 스크럽(멱등 마커 유지, 크래시-복구 창은 draft 보존). 일 1회
pg_cron. 유저당 open-pending 캡(미전달 pending 10개, 초과 시 429 RATE_LIMITED)도 적용. lazy recovery 는 confirmed 결제
...
- `src/lib/payments/feature-prices.ts` — `FEATURE_PRICES_KRW` 단일 출처(오픈초기 현금가 550/440/440/550, 정가 1,100/880/880/1,100, 무료 부적 11/9/9/11p).
...
- `supabase/migrations/20260610130000_pending_delivered_at.sql` — delivered_at 상태머신(materialize FK 수정, /qa) + purge delivered_at 기준 재작성.
- `supabase/migrations/20260610140000_free_relation_cap_rpc.sql` — `insert_relation_if_under_free_cap` 원자 조건부 INSERT RPC(무료 슬롯 TOCTOU 차단).
- `src/lib/relations/insert.ts` — `insertFreeRelationIfUnderCap`(무료 원자 RPC, null=슬롯초과) + `insertRelationAndComputeChart`(머티리얼라이즈, pk 고정) + `computeAndUpsertChart`(공유 best-effort).
- `src/lib/relations/materialize.ts` — `materializeRelationSlot`(delivered_at 상태머신 멱등, relation_id=pending_id deterministic, 삭제 소비 판별).
- `src/app/api/relations/route.ts` — 무료 원자 RPC 게이트 + lazy recovery + open-pending 캡(10) + 스테이징 + 402/환불.
