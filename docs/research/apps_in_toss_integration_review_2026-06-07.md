...
| 2 | **디지털 상품 결제는 인앱결제(IAP)만 허용. 토스페이먼츠 명시적 금지** | 웹은 ADR-039 pay-per-use, 미니앱은 IAP 4상품(550/440/440/550원) | 결제 클라이언트 전면 교체 완료 방향 (서버 잠금 구조 `isFeatureUnlocked`는 재사용 가능) |
...
나쁜 소식: **수수료**. 인앱결제는 앱마켓 수수료 15%(향후 30% 가능) + 토스 수수료 5%(현재 CBT 프로모션 0%)가 붙는다. 2026-06-22 확정 SKU 판매가(550/440/440/550원) 기준으로 콘솔 정산 화면에서 LLM 원가 대비 마진 재계산이 필요하다.
...
- 위반 시 과태료 최대 3,000만 원. → 합카드·오늘카드·오늘의 나는? 화면에 "AI 생성" 라벨 + 온보딩/웰컴에 고지 문구 추가 개발 필요.
...
**확정 정책**: 디지털 상품(케미카드·오늘의 나는?·케미 다시 맞추기·인연 슬롯)은 **인앱결제(IAP)만**. 토스페이는 실물 전용. 토스페이먼츠 금지.

**IAP 플로우** (문서 + `with-in-app-purchase` 샘플로 확인):
1. 콘솔에 상품(SKU) 등록: 케미카드 550원(공급가 500원) / 오늘의 나는? 440원(공급가 400원) / 케미 다시 맞추기 440원(공급가 400원) / 인연 슬롯 550원(공급가 500원) — 4종 (판매가 = 공급가 + VAT)
...
- 실제 정산액은 앱마켓·토스 수수료와 CBT 수수료 정책에 따라 달라지므로, 콘솔 정산 화면의 상품별 금액을 최종 기준으로 확인한다.
...
| 4 | IAP 상품 4종 등록 | 케미카드 550원(공급가 500원) / 오늘의 나는? 440원(공급가 400원) / 케미 다시 맞추기 440원(공급가 400원) / 인연 슬롯 550원(공급가 500원), 상품명·아이콘·설명 | 콘솔에서 등록 |
...
| 8 | 단가·마진 재검토 | 2026-06-22 확정 SKU 판매가(550/440/440/550) 기준 콘솔 정산액 확인. LLM 원가(GPT-5) 대비 마진 + 가격 인상 여부 | 가격 변경은 §1.1 |
...
4. **1차 화면 범위 이식**: 온보딩 → 인연 등록 → 합카드 → 오늘카드 (4 핵심 플로우 우선, 합피드/본명식/오늘의 나는?는 2차).

### P1 — 출시 검수 통과 요건
5. **IAP 결제 통합**: SKU 4종 + `createOneTimePurchaseOrder`/`processProductGrant` + 서버 unlock(주문 상태 mTLS 검증, 멱등) + `getPendingOrders` 복원 + REFUNDED 회수 정책. (샌드박스 필수 시나리오 테스트 포함)
6. **생성형 AI 고지/라벨**: 최초 사용 고지 + 합카드·오늘카드·오늘의 나는? 결과에 AI 라벨.
...
6. **D6. 가격**: 2026-06-22 확정가 기준 케미카드/인연 슬롯 550원, 오늘의 나는?/케미 다시 맞추기 440원. 가격 변경은 ADR-039 + `feature-prices.ts` + 미니앱 표시가 + 앱인토스 SKU 문서 동시 갱신 의무.
...
| D4 | 1차 출시 화면 범위 | **전체 8 플로우** (온보딩/인연등록/합피드/합카드/오늘홈/본명식/오늘의 나는?/다시합·공유 포함) |
| D5 | TDS 적용 깊이 | **채널톡으로 검수 기준 확인 후 범위 결정** ([2026-06-08 정정] TDS는 공식상 **선택** — §2.6 참조. "셸 TDS 확정" 전제는 재확인 대상) |
| D6 | 가격 | 정가 **1,100 / 880 / 880 / 1,100원**, 현금가·IAP 판매가 **550 / 440 / 440 / 550원**(케미카드/오늘의 나는?/케미 다시 맞추기/인연 슬롯, 웹+미니앱 통일) |

D6 후속 의무(§12 변경 매트릭스): `feature-prices.ts`(단일출처)·ADR-039·`payments.md`·`fluttering-gathering-island.md`·`PRD.md`·미니앱 표시가·앱인토스 SKU 가격을 동시에 갱신한다. 2026-06-22 현행 확정가는 케미카드/인연 슬롯 550원, 오늘의 나는?/케미 다시 맞추기 440원이며, 부분 갱신 후 PR 금지.
...
| Phase | 내용 | 주체 | 비고 |
|---|---|---|---|
| **P0. 가격 통일 (웹 선행)** | feature-prices.ts + ADR-039 + 문서 3종 + 미니앱 표시가 + 앱인토스 SKU 가격 동시 갱신 | 개발 세션 | 2026-06-22 확정가 기준 완료 상태 유지. §12 의무 |
| **P1. 콘솔·계약 준비** | 콘솔 가입→워크스페이스→앱 등록(appName 확정), 사업자 서류, 팝빌, mTLS 인증서 발급, IAP 상품 4종 등록(550/440/440/550), 토스 로그인 약관 + scope=user_key, 연결끊기 콜백 URL, **채널톡 문의 2건(운세 카테고리 사전 확인 + TDS 범위)** | **대표** | 개발 착수의 선행 조건 (특히 appName·mTLS·SKU) |
...
| **P5. IAP 결제** | SKU 4종 + createOneTimePurchaseOrder/processProductGrant + 서버 unlock(mTLS 주문검증, 멱등, 30초 내) + getPendingOrders 복원 + REFUNDED 회수 | 개발 세션 | ADR-039 모델 C 유지 |
| **P6. 정책 요건** | 생성형 AI 사전 고지 + 결과물 AI 라벨(웹 동시 적용 권장), 공유 intoss:// 딥링크 + 공개 OG(공유 토큰), 뒤로가기/가시성 처리 | 개발 세션 | AI 라벨은 법적 의무 |
| **P7. QA·검수·출시** | 샌드박스 IAP 4상품 필수 시나리오 + QR 토스앱 테스트 + 검토 요청(영업일 3일) → 출시 | 대표+개발 | 검수 요청은 테스트 1회 이상 완료 필요 |
...
1. **가격 stale**: 초기 검토안 가격은 폐기. 현행은 2026-06-22 확정가 **550/440/440/550원 + 부적 11/9/9/11** 기준으로 코드/문서/콘솔 SKU를 맞춘다.
...
| 오늘의 나는? | GPT-5, in 2,500+out 600 | $10–15 / 1,000 | **₩13–20** |
| 다시 맞추기 | 카드 재생성 ≈ 케미카드급 | (별도 추정) | **~₩15–21** |

→ LLM 원가 = 현행 SKU 판매가의 **대략 2~5%**. (종전 검토의 "건당 수천원" 추정은 ~1000× 오류로 폐기.)

**순수령** (수수료 차감. 공식 800원 예시의 net 약 80% 비율을 현행 SKU에 적용):

| 기능 | 판매가 | 미니앱 A 표준(15+5%≈80%) | 미니앱 B CBT(15+0%≈85%) | 미니앱 C worst(30+5%≈66.5%) | 웹 PG(~3%≈97%) |
|---|---|---|---|---|---|
| 케미카드 | 550 | ~440 | ~468 | ~366 | ~534 |
| 오늘의 나는? | 440 | ~352 | ~374 | ~293 | ~427 |
| 다시 맞추기 | 440 | ~352 | ~374 | ~293 | ~427 |
| 인연 슬롯 | 550 | ~440 | ~468 | ~366 | ~534 |

**LLM 차감 후 마진(미니앱 A):** 케미카드 ~₩419–427 / 오늘의 나는? ~₩332–339 / 다시 ~₩331–337, 인연 슬롯은 LLM 생성비 없음. 판매가 대비 **대략 75–80%** → 마진 건전.

**채널 비대칭(D6 동일가):** 미니앱은 웹 대비 ~17–18% 낮음(IAP 20% 수수료). 예: 케미카드 LLM후 웹 ~₩513–521 vs 미니앱-A ~₩419–427(건당 약 ₩94차). 적자 아님 — TAM 확대 비용으로 수용 가능. (종전 "~35% 낮다·순수령 610" 표기는 오류 정정.)

**Model C 선생성 sunk:** 미전환 선생성분 LLM(~₩17)만 손실. 전환율 20% 가정 시 결제 1건당 실효 LLM ≈ ₩85 ≪ 순수령 → 마진 위협 아님.

**CBT 일몰:** 토스 5% 복귀 시 net 85%→80%(케미카드·인연 슬롯 건당 약 ₩28↓, 440원 상품 건당 약 ₩22↓). 경미.

**결론:** **D6 가격 패리티는 마진상 안전**(미니앱도 대략 75–80% 마진). 채널별 차등가 불요. worst-case(30%) net 66%만 모니터링 + 인상 트리거 준비. (LLM 원가가 미미하므로 §4 #8 "LLM 원가 대비 마진" 우려는 해소.)
