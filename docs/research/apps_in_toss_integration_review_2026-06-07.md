...
### B2. 마진 워터폴 (미산출 — §8 30% 시나리오 보강)
- 신가 1,000/800/600 + IAP(앱마켓 15% + 토스 5%) 순수령 ≈ **610 / 490 / 365원**. CBT(토스 0%)에서는 ≈ 650 / 520 / 390원. 30% 악화 시 ≈ 460 / 370 / 280원.
- **채널 마진 비대칭**: D6 동일가지만 웹(Toss Payments PG ~3%)은 ~970/776/582원 실수령 → 미니앱이 구조적으로 ~35% 낮다. CBT 0% 일몰 시 추가 절벽.
- **Model C 선생성 sunk cost**: 결제 미전환 선생성분의 LLM 원가는 회수 불가 → 전환율 가정 필요.
- ⚠️ 기능별 GPT-5 토큰원가×FX를 실측해 3-regime(15%/30%/CBT-0%)로 계산하는 **마진 워터폴 표를 P1 전에 작성**(검토 추정치는 거칠어 신뢰 불가, 방법론만 채택). 마진 하한 미달 시 채널별 차등가 또는 LLM 원가 절감 검토.
