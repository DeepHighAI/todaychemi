...
- **정식 출시 전 필드 버그 3건 수정 완료 ✅ (2026-06-25, Claude Code)** — 실 `.ait` 미니앱 QA 제보 3건 polish → **main ff-push `4881a95c..0abf178d`(4커밋, Vercel 배포)**. §1.1 결정·가격·범위·런치 판정("오픈 보류") 불변. **Issue 1(온보딩 튕김, 미니앱)**: `Step4Review.tsx` `invalidateQueries(['me-chart'])` 가 TanStack v5 기본 `refetchType:'active'` 라 inactive(`/onboarding`=ProfileGate 밖) 쿼리를 리페치 안 함 → navigate 후 `ProfileGate` 가 stale `null` 보고 재튕김 → `refetchType:'all'` await + `ProfileGate` `data==null && isFetching` 동안 LoadingState 보류. 인증 문제 아님(`server.ts:24` Bearer 수락). **Issue 2(생일 휠 jank, 미니앱)**: `wheel-column.tsx` 매 프레임 `onChange` → snap 충돌 → settle(~90ms) 디바운스 + `scroll-snap-stop:always` + iOS momentum/`overscroll-behavior:contain`. 웹 picker follow-up(§1.1 미니앱만). **Issue 3(오늘 메시지 폴백, 웹+미니앱)**: `TODAY_LLM_TIMEOUT_MS` 25s 가 GPT-5 14~26s 초과 → TEMPLATE 폴백 → **45s 상향**(route `maxDuration=60` 안) + codex WIP(40s 안심 spinner·i18n) 유지. 모델 변경·클라 재시도 없음(§1.1). 공통 백엔드=웹·미니앱 동시. 문서: launch_readiness 2026-06-25 블록 + local_e2e_smoke 45s 정정. 검증: 루트 tsc0·lint0·**2928/2928**(325 files) + 미니앱 tsc0·**288/288**(55 files). 배포: ③ today 백엔드는 Vercel 라이브(웹·미니앱 공통); ①②+미니앱 spinner 는 `.ait` 재빌드(deploymentId `019efd29`, eyJ0·prod host·3 fix 검증)+토스 콘솔 업로드 완료. 잔여: 실기기 4점 검증. 상세 CLAUDE.md §2.
- **오늘 카드 속도 최적화(reasoning minimal + 빌더 병렬화) 완료 ✅ (2026-06-25, Claude Code)** — 모델 유지(gpt-5)하 속도개선 §1.1 확정. **A** `callOpenAi` reasoningEffort 파라미터(기본 'low' 유지=hapcard/replay 불변)+`callDailyHapLlm` 'minimal'. **B** `builder.ts` userChart‖relation 병렬(timedSettle, trace 계약 보존). **라이브 측정**: 단독축 low **50.2s**(45s 타임아웃 초과→폴백 재발) vs minimal **4.8s** ~10.4× · 인연 low **25.6s** vs minimal **3.2s** ~8×, 품질 동등. 사용자 결정=**minimal 유지**(low 는 타임아웃 초과·품질 이득 없음). 루트 **2930/2930**(+2)·tsc0·lint0. `perf/today-llm-speed`→main ff push(Vercel). 상세 CLAUDE.md §2. [[feedback-gpt5-reasoning-effort-today]]
- **오늘의 부적 리텐션 피처 코드리뷰 + 보완 완료 ✅ (2026-06-30, Claude Code)** — Codex 미니앱 홈 "액땜 부적" 리추얼 리뷰+하드닝. §1.1: D1 명리근거=파생 용신(`derived.yongsin.primary`, 케미카드 동일근거)·D2 ADR-010 위계=0건 사용자 핵심 CTA 아래·D3 앱인토스 `eventLog` 계측·D4 **ADR-041 신설**·ADR-038 carve-out(봉인 한자=리추얼 본질, 한글 뜻 병기) 명문화. 보완: C1 크래시 가드(빌더 nullable, yongsin→최소카운트→null 폴백)·A1 키보드 완료·P1 ref+rAF 리렌더 절감·V1 z-index·S4 변형 2→4. `daily-talisman.ts`/`daily-talisman-ritual.tsx`/`ait-analytics.ts`(신규)/`chart.ts`(`SajuDerived`). **미니앱 324/324**(59 files)·tsc0. §1.2 적대 리뷰(code-reviewer): 6항목 클린·2건 수정(canvas `role=img→slider` a11y·키보드 획 완화). **streak 추가**(§1.1 streak만, `advanceStreak` 결정형 연속일+"🔥 N일 연속" 배지). 스모크: V1 캔버스 포인터=`.liquid::before/::after` 둘 다 `pointer-events:none`→트랩 구조적 불가(코드 검증)·D2=DOM 순서 테스트·D3 eventLog→ait logs=실기기 전용. B1=branch `feature/daily-talisman-retention` 3커밋(광고 `67e3625c`/부적 `30c6a246`/미니앱 PostCSS 격리)+`.ait` 재빌드 완료(deploymentId `019f192b`, eyJ0·운영 광고ID·prod host). 빌드 차단 수정(환경): 미니앱이 루트 Tailwind `postcss.config.mjs` 상위 탐색해 실패 → `miniapp/postcss.config.mjs` 빈 plugins 격리. 상세 CLAUDE.md §2 + `docs/adr/ADR-041-daily-talisman-retention.md`.
...
| 오늘의 부적 (리텐션 리추얼) | `dailyTalisman` (코드) / "오늘의 부적"·"액땜 부적" (UI) | 단독 `부적`(결제 통화와 혼동), `talisman`(통화 의미) |
| 부적 (결제 무료 크레딧/통화) | `부적` (UI 통화 단위, ADR-039) | — (리텐션 "오늘의 부적" 리추얼과 혼용 금지) |
