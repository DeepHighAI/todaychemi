...
- **오늘의 부적 리텐션 피처 코드리뷰 + 보완 완료 ✅ (2026-06-30, Claude Code)** — Codex 미니앱 홈 "액땜 부적" 리추얼 리뷰+하드닝. §1.1: D1 명리근거=파생 용신(`derived.yongsin.primary`, 케미카드 동일근거)·D2 ADR-010 위계=0건 사용자 핵심 CTA 아래·D3 앱인토스 `eventLog` 계측·D4 **ADR-041 신설**·ADR-038 carve-out(봉인 한자=리추얼 본질, 한글 뜻 병기) 명문화. 보완: C1 크래시 가드(빌더 nullable, yongsin→최소카운트→null 폴백)·A1 키보드 완료·P1 ref+rAF 리렌더 절감·V1 z-index·S4 변형 2→4. `daily-talisman.ts`/`daily-talisman-ritual.tsx`/`ait-analytics.ts`(신규)/`chart.ts`(`SajuDerived`). **미니앱 324/324**(59 files)·tsc0. §1.2 적대 리뷰(code-reviewer): 6항목 클린·2건 수정(canvas `role=img→slider` a11y·키보드 획 완화). **streak 추가**(§1.1 streak만, `advanceStreak` 결정형 연속일+"🔥 N일 연속" 배지). 스모크: V1 캔버스 포인터=`.liquid::before/::after` 둘 다 `pointer-events:none`→트랩 구조적 불가(코드 검증)·D2=DOM 순서 테스트·D3 eventLog→ait logs=실기기 전용. B1=branch `feature/daily-talisman-retention` 2커밋(광고/부적)+`.ait` 재빌드. 상세 CLAUDE.md §2 + `docs/adr/ADR-041-daily-talisman-retention.md`.
