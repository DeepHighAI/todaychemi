...
> 본 문서는 합플(Hap-Plae) / Saju Lens 프로젝트 전용 규칙이다. 보편 규칙은 `C:\DEV\AGENTS.md`(상위 디렉토리)를 참조하며 중복 작성하지 않는다.
...
스캐폴드 후 디렉토리는 `FRONTEND-PREP.md` §2 참조. `src/types/` 이미 생성됨 — Contracts-first 시작점 (`C:\DEV\AGENTS.md`).

---

## 8. 도메인 용어 사전 (프로젝트 전용)

`C:\DEV\AGENTS.md` "Terminology Consistency" 표를 확장한다. 한 개념 = 한 용어 원칙은 동일.
...
| 도메인 용어 추가 | 본 파일 §8 + `C:\DEV\AGENTS.md` 해당 표 |
...
- `C:\DEV\AGENTS.md` — 보편 규칙 (언어·터미놀로지·Contracts-first·Agent routing)
...
- **커밋 규칙**: `C:\DEV\AGENTS.md` "Git Conventions" 준수 — English, imperative mood, `type: description` (feat|fix|refactor|test|docs|chore), 72자 이내
