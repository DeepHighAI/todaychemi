...
describe('RewardPopup', () => {
  it('가입 보상이면 환영 타이틀 + 획득 부적 수 + 사용 용도를 노출한다', () => {
    renderWithProviders(<RewardPopup open amount={50} isSignup onClose={vi.fn()} />);
  ...
  it('출석 보상이면 출석 타이틀을 노출한다', () => {
    renderWithProviders(<RewardPopup open amount={5} isSignup={false} onClose={vi.fn()} />);
  ...
  it('확인 버튼을 누르면 onClose 를 호출한다', async () => {
    const onClose = vi.fn();
    renderWithProviders(<RewardPopup open amount={50} isSignup onClose={onClose} />);
    await userEvent.click(screen.getByRole('button', { name: '확인' }));
    expect(onClose).toHaveBeenCalledTimes(1);
  });

  it('open=false 면 렌더하지 않는다', () => {
    renderWithProviders(<RewardPopup open={false} amount={50} isSignup onClose={vi.fn()} />);
