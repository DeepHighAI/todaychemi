...
- **명리 RAG 검수 완료 ✅ (2026-06-12, §1.1 2결정: 즉시 시드·경미 즉시 수정)** — 고전 원문 14건 RAG 자산화(維基文庫·ctext public domain: 淵海子平 地支藏遁歌/繼善篇·三命通會 사령일수/궁위/12운성·滴天髓 衰旺/中和/體用·**자평진전 신규**(zpjj) 용신/음양생사) + curator 감사 14/14 PASS + `pnpm seed:classics` 라이브 적용(**20→34 rows**) + retrieval 스모크(신규 주제 최상위 hit). **검수 리포트** `docs/qa/myeongri_rag_review_2026_06_12.md`: 7항목 = PASS 6 · 검증 불가 명기 1(연령 밴드) — 지장간 테이블 12지 전수 일치, 궁위 4라벨 4/4 고전 직접 근거, 신강약·용신 구조 고전 근거(수치는 자체 단순화 명기). **산식 권고 처분 완료(2026-06-12 사용자 확정)**: **R1 적용** — `jijangganWeightsFor` 사계월(辰戌丑未) {정기10·여기5·중기3} 교환 + **derived_version 1→2**(저장 v1은 resolveDerivedForLlm self-heal 재계산, DB 재기록·백필 불필요) + 프롬프트 범프(6모드 **v0.17**/v0.18 canary · today_with_relation v0.5/v0.6 · daily_hap v0.5 · whatif v0.3 — seed 15행 버전 갱신됨). **R2 불채택**(사령일수↔장간가 구성 학파 차로 일수 비례 매핑 정의 불가 — 리포트 명기). §6.1~6.3/6.7 고전 cross-ref 동기. daeun.current.year 측채널 **수용 확정**(ADR-040 §4 Accepted). 모드 태그: 썸합 3→6·첫합 4→7 보강.
