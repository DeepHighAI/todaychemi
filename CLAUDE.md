...
- **명리 RAG 검수 완료 ✅ (2026-06-12, §1.1 2결정: 즉시 시드·경미 즉시 수정)** — 고전 원문 14건 RAG 자산화(維基文庫·ctext public domain: 淵海子平 地支藏遁歌/繼善篇·三命通會 사령일수/궁위/12운성·滴天髓 衰旺/中和/體用·**자평진전 신규**(zpjj) 용신/음양생사) + curator 감사 14/14 PASS + `pnpm seed:classics` 라이브 적용(**20→34 rows**) + retrieval 스모크(신규 주제 최상위 hit). **검수 리포트** `docs/qa/myeongri_rag_review_2026_06_12.md`: 7항목 = PASS 6 · 검증 불가 명기 1(연령 밴드) — 지장간 테이블 12지 전수 일치, 궁위 4라벨 4/4 고전 직접 근거, 신강약·용신 구조 고전 근거(수치는 자체 단순화 명기). - **배포+시드 완료 ✅ (2026-06-12)** — main ff-push `5898dcf..1a4215b`(48커밋, Vercel auto-deploy) + `pnpm seed:prompts` 15행 라이브 검증(6모드 v0.17 active+v0.18 canary·today_with_relation v0.5/v0.6·daily_hap v0.5, 구버전 자동 rolled_back) + classics 34 rows 라이브. v3 백필 미실행=의도(전 경로 lazy + derived v1 self-heal). **/qa 라이브 스모크 완료 ✅ (2026-06-12)** — 케미카드 신규 2건(일합 v0.17·썸합) + 오늘 케미(3축, 합온도 78) + 또 다른 나(work) 검증. **파생층 PASS**: 십신 교차(방향)·운세 교차(대운 합충) 근거 탭 노출 ✓ · 환각 부재(신살·미제공 십신 0건) ✓ · 한자 미노출 ✓ · `[DERIVED_INVALID]`/`[CROSS_INVALID]` 0건 ✓. **ISSUE-001 [HIGH, pre-existing] 발견**: classic_citation 전면 0건 — 실 query-text 단문이라 유사도 max 0.349 < 임계 0.60, retrieval 항상 0 hit(v0.13부터 동일, 회귀 아님). ADR-015/018 핵심 가치 비작동 → **§1.1 수정 방향 결정 대기**(① 임계 하향 ② 쿼리 장문화 ③ topic_tags lexical 하이브리드). INFO: today 25s 생성 중 일시 에러 상태(기존 latency §1.3 계열) · whatif `[CLASSICAL_HANJA] 戊` warn 1건(안전망 정상) · Radix Dialog aria 경고. 리포트: `.gstack/qa-reports/qa-report-saju-derivation-smoke-2026-06-12.md`.
