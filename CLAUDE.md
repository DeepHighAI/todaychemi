# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

> 본 문서는 오늘케미(TWODAY) 프로젝트 전용 규칙이다. 보편 규칙은 `C:\DEV\CLAUDE.md`(상위 디렉토리)를 참조하며 중복 작성하지 않는다.

---

## 1. 필수 규칙 (사용자 확정, 절대 우선)

이 다섯 규칙은 다른 모든 가이드보다 우선한다. 위반 시 작업을 중단하고 사용자에게 보고할 것.

### 1.1 중요한 의사결정은 반드시 사용자에게 묻고 진행
"중요한 의사결정"의 범위:
- ADR 신규 작성·수정·폐기
- 기획서/PRD 섹션 의미 변경 (오탈자 수정 제외)
- 가격·과금·Phase 범위·6모드 taxonomy 변경
- 라이브러리 채택·교체 (잠금된 스택 외 추가)
- DB 스키마 변경, RLS 정책 변경
- LLM 모델·프롬프트 버전 변경
- 보안·개인정보 처리 범위 변경

→ `AskUserQuestion` 또는 직접 질문으로 사용자 승인 후 진행. 임의 가정 금지.

### 1.2 모든 코드는 개발 후 스킬로 테스트·검증
구현이 끝나면 다음 스킬 중 적합한 것을 호출하여 검증한다 (§10 매핑 표 참조):
- `/qa` — 기능 테스트 + 버그 수정
- `/qa-only` — 보고만, 수정 금지
- `/review` — PR 머지 전 최종 검토
- `/design-review` — 시각 QA + 수정
- `/browse` — 헤드리스 브라우저 동작 확인
- `/codex` — 2차 의견·도전·상담

검증 없이 "완료" 보고 금지. 어떤 스킬을 어떤 결과로 돌렸는지 명시.

### 1.3 테스트 중 관련 없는 코드는 절대 수정 금지
QA·디버깅·E2E 실행 중 발견한 *별개의* 이슈는:
1. 즉시 메모(§13 위치)에 기록
2. 현재 작업 완료 후 사용자에게 별도 보고
3. 사용자 승인 전에는 수정 금지

같은 PR에 무관한 리팩터링/정리 commit 추가 금지. 한 번에 한 가지 변경.

### 1.4 컨텍스트 60% 도달 시 오토메모리로 압축·인계

**적용 대상: Opus·Sonnet 모델 모두, 예외 없음.**

컨텍스트 사용량이 60%를 넘으면 즉시 아래 절차를 실행한다:

1. **상태 저장** — 진행 중 결정·미결 질문·다음 단계를 `session_<task>_handoff_<date>.md`로 작성
   - 완료된 파일 목록 + 미완료 파일 목록
   - 다음 세션에서 재개할 정확한 위치 (파일명·섹션명)
   - 작업 중인 타입 인터페이스·활성 ADR 번호
2. **인덱스 갱신** — `MEMORY.md`에 핸드오프 파일 1줄 추가
3. **CLAUDE.md 갱신** — §2 프로젝트 상태 업데이트 (완료된 것 반영)
4. **사용자 알림** — "컨텍스트 XX%, 압축 완료. 새 세션을 시작해 주세요." 보고
5. **새 세션 복원** — 세션 시작 시 `MEMORY.md` → 핸드오프 파일 순으로 읽어 컨텍스트 복원 후 작업 재개

**자율 이어가기 금지**: 압축 후 같은 세션에서 작업을 계속하면 안 된다. 반드시 새 세션을 시작해야 한다.

압축 시 보존 의무 항목은 §11 참조.

### 1.5 반복 실수·중요한 개발 메모는 별도 기록
다음은 발견 즉시 기록한다 (§13 위치):
- 같은 실수를 두 번 이상 한 경우
- 잠금된 ADR을 위반할 뻔한 시도
- 잘 알려지지 않은 명리 도메인 함정 (시나리오 추정 모드, 음력 변환 엣지케이스 등)
- 외부 API 동작 차이 (KASI vs ssaju vs manseryeok-js 불일치)
- 비결정형 결과를 만든 코드 패턴 (LLM이 점수에 개입 등)

기록 → 다음 작업 진입 전 해당 메모 확인이 의무.

### 1.6 모든 UI 작업은 UIDesign/ 와이어·디자인 시스템 준수 (비협상, 2026-05-06 확정)

- **시각적 단일 진실**: `UIDesign/system.css`(토큰), `UIDesign/primitives.jsx`(공용 컴포넌트), `UIDesign/screens-*.jsx`(화면)
- `UIDesign/` 자체는 **수정 금지**(§2와 동일). 프로덕션 구현은 반드시 `src/`에 작성.
- 화면 신규·수정 시 대응 와이어가 있으면 그것을 따름. 없으면 §1.1 사용자 승인 후 진행.
- **디자인 토큰**은 `src/app/globals.css` CSS 변수로만 정의. 컴포넌트 인라인 스타일·매직 넘버 금지.
- **"Toss × iOS 26 × M3 Expressive"** 무드 변경은 §1.1 승인 대상.
- 완료 검증: `/design-review` 또는 `/qa` 스킬로 와이어 일치 확인 후 보고.

### 1.7 사용자 대상 보고는 항상 한국어 (사용자 확정, 2026-06-21)

- 사용자에게 보이는 모든 응답·진행 보고·요약·질문(`AskUserQuestion` 포함)은 **한국어로 작성**한다.
- 예외(영문 유지): 코드 식별자·커밋 메시지·ADR/문서 본문 등 `C:\DEV\CLAUDE.md` 언어 규칙이 영어로 못박은 항목, 그리고 명령어·파일경로·기술 용어. 즉 *유저 사이드 산문*만 한글, 코드/식별자는 영문 병기 가능([[feedback-user-side-terminology]]).
- 의사결정 질문의 한글 작성은 [[feedback-korean-decision-questions]]와 동일하게 적용.

---

## 2. 프로젝트 상태 (2026-06-19 기준)

- **Phase 0 G0 게이트 ✅ 100% PASS** — KASI vs ssaju 100/100 (normal 50/50, boundary 30/30, edge 20/20). normalize.ts ssaju 프로덕션 승격 완료(年/月/時柱). 야자시 = 조자시 통합 학파.
- **PR-1 완료** — Next.js 16.2.4 스캐폴드 생성됨. `pnpm dev` 정상 동작.
- **PR-3 완료** — KASI 진본 검증 라이브러리 + 100건 픽스처 + G0 verify 100% PASS.
- **PR-2 완료 ✅** — C-4(SQL 20개) + C-7(RLS 통합 39개) + C-5(Auth 전체 구현) + C-6(prompt_versions 시드 6건) 전체 완료. **154/154 tests PASS** (17 files). tsc 0 errors. Supabase Free `jamhkucluhiibqpjsiov` 적용 완료.
  - C-5: `src/lib/auth/kakao.ts` + `src/app/auth/callback/route.ts` + `src/app/login/page.tsx` 완료.
  - C-6: `scripts/seed-prompts.ts` + `pnpm seed:prompts` → 6모드 v0.2 active 시드 완료.
- **F3 완료** — `prompts/system/` 6모드 system prompt v0.2 작성 완료 (commit `5f9625e`). 명리 specialist 검수 대기.
- **`.env.local` 완성** — KASI_SERVICE_KEY, OPENAI_API_KEY, Supabase 키 6개, KAKAO 3개 모두 입력 완료. `KAKAO_REDIRECT_URI` = `jamhkucluhiibqpjsiov` (2026-05-04 수정).
- `UIDesign/` 는 Babel CDN 기반 참조 프로토타입 — **수정 금지**, 프로덕션 코드는 `src/app/`에 작성

**F4 완료 ✅ + G4 RAG 시드 완료 (2026-05-05)** — **508/508 PASS**, 0 TS errors.
- Step 1~14 전체 완료. 주요 커밋: Step 11(`060e74f`) · Step 12(`335bb68`) · Step 13(`d922182`) · 이슈 수정(`ac0885e`) · docs 메모리 갱신(`daf7e1e`).
- `supabase db push 0021_classics` ✅ — RLS 통합 41/41 PASS.
- `rag_content/classics/` YAML 20건 시드 완료 (`approved_ai_pending_human`) — §7.2 명리 specialist 크라우드 검수 미수행, 향후 필요.

**F5 Sprint 진행 중 (2026-05-06)** — **767/767 PASS**, 0 TS errors.
- B1+B3 완료(jsdom+testing-library, route groups). B5(타입 리프트) + S-00(Kakao→Google OAuth) + S-01-A(로그인 polish) 완료. S-01-B+S-02(온보딩 단일 페이지) 완료. S-03(인연 등록 단일 페이지) 완료. S-04(합피드 페이지) 완료. S-05(합카드 호출 + 에러 UX) 완료. S-06(합카드 9섹션) 완료. S-06c-a(glossary tooltip) + S-06c-b(glossary bottom sheet) 완료. **S-06b(mini_radar 오각형 오버레이) 완료**. **chart 컴퓨트 A-eager 완료 — chartPending 영구 차단 해소** ✅.
- 완료 커밋: `6abc2a4`(B5 types) · `ab1c87e`(S-00 Google OAuth) · `199b89f`(S-01-A 로그인) · `7e21788`(onboarding types) · `0731125`(onboarding page+route) · `c44059b`(relation types) · `9935f32`(relations page+route) · `b6bb288`(GET /api/relations) · `e707d11`(feed page+grid) · `c3e07bf`(theory version const) · `ef30b5a`(feed mode query) · `dd10783`(hapcard page S-05) · `0c65f52`(S-06 9 sections compose) · `138b778`(S-06c-a glossary tooltip) · `a520141`(S-06c-b glossary bottom sheet) · `6f8bd68`(S-06b mini_radar) · `fcab0ee`(chart-hash) · `02ac388`(compute) · `72e149b`(onboarding eager) · `f4ad43f`(relations eager).
- §1.3 별도 이슈 잔여: ESLint 9 다운그레이드 별도 PR → **✅ 완료(12ae939)**. Kakao redirect_uri 동기화(사용자 보류). Supabase Google provider 활성화(Dashboard 수동). PR-2 시점 untracked 파일 정리(§1.1 결정 필요).
- **S-07a share 완료 ✅** — build-share-payload + share-handler(Web Share+clipboard) + ShareSheet(Drawer) + HapcardShare 통합 + page 배선. `docs/specs/replay.md` 작성 완료(§1.1 D1~D4 결정 매트릭스).
- S-07a 커밋: `99628c0`(build-share-payload) · `f4667eb`(share-handler) · `153d277`(ShareSheet) · `17bd916`(type fix) · `389b901`(HapcardShare+page wiring) · `0e2a8fa`(replay spec).
- §1.3 별도 이슈 잔여: ESLint 9 다운그레이드 별도 PR → **✅ 완료(12ae939)**. Kakao redirect_uri 동기화(사용자 보류). Supabase Google provider 활성화(Dashboard 수동). PR-2 시점 untracked 파일 정리(§1.1 결정 필요). `relation_nickname`/`relation_gender_normalized` builder.ts JOIN → **✅ 완료(7a0cb8c)**.
- **S-07b Replay 완료 ✅ (2026-05-06)** — buildReplay async 함수 + POST /api/hapcards/[id]/replay route handler GREEN. 커밋 `c69acff`. **805/805 PASS**, 0 TS errors.
- **`supabase db push 0022+0023` 적용 완료 ✅ (2026-05-06)** — `deduct_tokens` / `refund_tokens` RPC + `hapcard_replays_idempotency` UNIQUE 제약 라이브 반영. 검증 스크립트 `scripts/verify-replay-migrations.ts`.
- **후속 작업 완료 ✅ (2026-05-06)**:
  - Task 1 (`b9e4e72`): `database.types.ts` 재생성 → route.ts tokenRpc 캐스트 제거.
  - Task 2 (`7a0cb8c`): `relation_nickname`/`relation_gender_normalized` builder.ts JOIN 연결 + vitest testTimeout 15s 상향 (jsdom cold-load 회귀 해소). **810/810 PASS**, 0 TS errors.
  - Task 4 (`aa729ff`): `docs/specs/payments.md:271` `'hapcard'` → `'hapcard_use'` 정정.
- **§1.3 별도 이슈 잔여**: ESLint 9 다운그레이드 별도 PR → **✅ 완료(12ae939)**. `payments.md:271` `'refund'` ↔ `'bonus'` 불일치 → **✅ 완료(e3a6a03)**. 라우트 `'replay_refund'` reason → **✅ 완료(route.ts:115 canonical)**.
- **다음**: F5 sprint 이후 작업 §1.1 결정 대기.
- **§1.1 결정 2건 완료 ✅ (2026-05-06)**:
  - Decision 2 (`e3a6a03`): `token_ledger.reason` enum 6값 doc-only 동기화 — `db_schema.md:319` + `payments.md:271` + `0009_token_ledger.sql:6`. **810/810 PASS**, 0 TS errors.
  - Decision 1 (`12ae939`): ESLint flat config 마이그레이션 — `eslint.config.mjs` 신규 + `package.json` lint 스크립트 `eslint .` + eslint `10→9` 다운그레이드(typescript-eslint v8 호환). `pnpm lint` PASS (0 errors, 7 warnings). **810/810 PASS**, 0 TS errors.
- **§1.3 별도 이슈 추가**: 7개 파일 unused `eslint-disable @typescript-eslint/no-explicit-any` 경고 → **✅ 완료(9ac5bfa, 4 tracked + 3 untracked auto-fix). pnpm lint 0/0.**
- **PR-B(S-99) 완료 ✅ (2026-05-06)** — ErrorCard/LoadingState/EmptyState + global error/loading/not-found + error-codes 카탈로그. 커밋 `cec5038`. **833/833 PASS**, 0 TS errors.
- **PR-A1(S-03 today 백엔드) 완료 ✅ (2026-05-06)** — GET /api/today route + cache-key(sha256) + builder(3-tier fallback) + openai(GPT-5 mini) + kst-date + types + daily_hap.md system prompt. 커밋 `77a90ef`. **850/850 PASS**, 0 TS errors.
- **PR-C(S-96 OG 이미지) 완료 ✅ (2026-05-06)** — buildOgPayload(PII 0건) + OgTemplate(1200x630 Edge) + GET /api/og/hapcard/[id] + buildSharePayload URL에 range 추가 + page.tsx server wrapper(generateMetadata) + HapcardView.tsx 분리. 커밋 `acc94c6`. **866/866 PASS**, 0 TS errors. Auth 401 유지(메신저 크롤러 차단) — 공유 토큰 별도 PR.
- **PR-A2(Today UI + TabBar) 완료 ✅ (2026-05-06)** — §1.6 UIDesign 준수 규칙 + globals.css M3 토큰 + TabBar + layout.tsx 통합 + Today 6 컴포넌트 + /api/me/chart route + (app)/page.tsx Today 조립 (3 useQuery + Top-N=5 최근순 + chart=null guard) + /me placeholder. **911/918 PASS** (full suite; 7 실패는 jsdom cold-load 사전 flake — 단독 실행 시 18/18 GREEN). PR-A2 영역 단독: layout 3/3 + Today 컴포넌트 28/28 + /api/me/chart 4/4 + (app)/page 8/8 + /me 2/2 = **45/45 PASS**, 0 TS errors, 0 lint errors.
- §1.1 PR-A2 Phase 4 결정 사용자 확정: (1) /api/relations Top-N=5 (서버 created_at desc 그대로) (2) /api/me/chart 신규 호출 (user_charts 최신 1건).
- §1.3 별도 이슈 추가: jsdom cold-load 시 hookTimeout 부족 → **✅ 완료(72ba226, vitest.config.ts:34 hookTimeout:15000). 923/923 full suite GREEN.**
- **Phase 6 manual smoke 완료 ✅ (2026-05-06)** — Bug 1(globals.css @import 순서, caba530) + Bug 2(라우트/미들웨어 exclusion-list 재작성 + /app 리다이렉트 삭제, bda3033) 수정. **923/923 PASS**, 0 TS, 0 lint. /login 200 ✓, / Today 화면 도달 ✓, TabBar 3탭 ✓, /me placeholder ✓. §1.3 잔여: API 401(만료 쿠키 Playwright 세션 한정). hookTimeout flake → ✅ 완료(72ba226).
- **F5 /me 본명식 본 화면 완료 ✅ (2026-05-07)** — PillarGrid + DayMasterCard + MeHero + YunsePlaceholder + page composition(5섹션 + useQuery). **942/942 PASS**, 0 TS, 0 lint. 커밋: `eb3b05b`(ko.json) · `b146ce5`(컴포넌트 4종) · `0ad0b9e`(page 조립).
- **QA /me 본명식 완료 ✅ (2026-05-07)** — Email/Password 로그인 추가(영구 기능) + 테스트 계정 시드(Test1@test.com) + /me 5섹션 브라우저 시각 검증. OhaengBars 바 비가시 버그 수정. **948/948 PASS**, 0 TS, 0 lint. 커밋: `390d8f8`(email auth) · `23ae3c7`(ohaeng fix). §1.1 후속: Google SSO 진단(별도) · email auth 프로덕션 정책 결정 필요.
- **Email/Password 프로덕션 강화 완료 ✅ (2026-05-07)** — 비밀번호 정책(8자+letters_digits) + rate limit(30→10/5min) + signUp 흐름(/signup 페이지) + TDD 20건 신규. **969/969 PASS**, 0 TS, 0 lint. 커밋: `b9aaea9`(supabase config) · `7f35599`(password-policy) · `8747ff7`(signUpWithEmail) · `7510008`(i18n) · `9b45b4f`(signup page) · `b8267c9`(login link) · `22187a1`(auth.md). §사용자 수동 절차: Supabase Dashboard → Auth 정책 동기화 필요(jamhkucluhiibqpjsiov). Google SSO 별도 세션.
- **E1+E2 완료 ✅ (2026-05-07)** — UI 4토큰 소프트 변환(합→끌림/형→긴장/충→부딪힘/해→소모) + LLM v0.3 prompts + migration 0024. **1033/1033 PASS**, 0 TS, 0 lint. branch: `feature/e1-ui-term-conversion` (Cycles 1-12) + `feature/e2-llm-prompts-v0.3` (Cycles 13-15). §사용자 수동 절차: `pnpm db:push`(0024) → `pnpm seed:prompts` → v0.3 active 확인.
- **Y4 완료 ✅ (2026-05-09)** — ADR-033/036 합피드 자동 정렬 + 흐름 변화 큼 배지 전체 완료 (Cycles 1-14). hapcard_score_snapshots 테이블 + computeChangeScore + /api/feed(정렬) + FeedPage 전환 + ChangeBadge + i18n(feed.badge.change_significant). **1068/1068 PASS**, 0 TS, 0 lint. 커밋: `dbb7939`(Phase 1+2) · `c98f764`(feed route) · `b62ea17`(feed page+badge) · `bd50a6d`(i18n). §사용자 수동 절차: `pnpm db:push`(0025 migration 이미 dbb7939 시점 적용 완료). branch: `feature/y4-change-score-feed-sort`.
- **§1.3 cleanup 완료 ✅ (2026-05-09)** — `yunse_adjustment` fixture 3건 + replay route Zod parse. 0 TS errors 달성. 커밋: `287ec28`. ScoreBreakdownSchema + HapcardDbRowSchema 추가 (`src/types/hapcard.ts`). §1.3 잔여: vitest default-reporter 환경 이슈(별도 세션). 기타 §1.3 baseline 4건 → ✅ 완료.
- **S-08 만약합(Whatif) 기능 전체 구현 완료 ✅ (2026-05-09)** — 6모드 백엔드+UI 전체 완성. 주요 산출물: `src/types/diagnostic.ts`(6모드 타입) · `src/lib/whatif/`(builder/cache-key/output-schema/prompt-loader/query-text) · `/api/whatif/[type]` route · WhatifSheet + WhatifTrigger + 5개 섹션 컴포넌트 · WhatifView 4-state. **1174/1174 PASS**. §사용자 수동 절차: `pnpm db:push`(0026_whatif_results) 필요. 커밋 키: `518aa7e`(D1 supporting files) · `43945c6`(route) · `fffc024`(WhatifView) · `60fa445`(PII guard).
- **S-08 followups #1-#5 완료 ✅ (2026-05-09)** — `chore/s08-followups` → `master` fast-forward 병합. #1(`b6f7765`) DEFAULT_LLM_MODEL 통합 · #2.1(`4cd4057`) 에러 코드 카탈로그 6건 · #2.2(`6b14193`) WhatifView ErrorCard 매핑 · #3(`07a7060`) ClassicCitation 스키마 일원화(`src/lib/rag/citation-schema.ts`) · #4(`281a425`) refund_tokens 실패 로깅 · #5(`901291a`) ErrorCard CTA + INSUFFICIENT_TOKENS "충전하러 가기"→`/me`. **1192/1192 PASS**, 0 TS, 0 lint.
- **§1.3 cleanup 2차 완료 ✅ (2026-05-09)** — (1) Replay route 환불 catch 로깅(`313a2bb`, TDD 2건) (2) `messages/ko.json` `whatif.error.*` 블록 4키 전체 제거 (3) `0009_token_ledger.sql:6` 주석 whatif_use/whatif_refund 동기화(`05fedcf`). **1194/1194 PASS**, 0 TS, 0 lint.
- **§1.3 잔여 1건**: `INSUFFICIENT_TOKENS` CTA href `/me` → 충전 페이지 구현 시 `/payments/charge` 업데이트(`src/lib/errors/error-codes.ts:40`). 별도 PR.
- **B1 DB push 확인 완료 ✅ (2026-05-09)** — `0024_prompt_v0_3_rollback` + `0026_whatif_results` 이미 라이브 반영 확인(`supabase db push --dry-run` → "up to date"). `pnpm seed:prompts` v0.3 active 6/6 확인. `whatif_results` 테이블 쿼리 가능 확인. `scripts/verify-b1-migrations.ts` 신규 추가. **1194/1194 PASS**, 0 TS, 0 lint.
- **로컬 E2E manual smoke 가이드 완료 ✅ (2026-05-09)** — `docs/qa/local_e2e_smoke.md` 신규(454줄). 8 핵심 flow(회원가입/온보딩/인연등록/합피드/합카드/오늘홈/본명식/만약합) + LLM 비용 경고 + 알려진 제약. 커밋 `4b2a62f`.
- **P0 working tree 정리 완료 ✅ (2026-05-09)** — 19개 base migration(0001~0020) + supabase/.gitignore + G0 KASI toolchain(scripts 7종·tests 11건·fixtures) + PR-A2 layout/styles tests + DB contract/RLS tests + seed-prompts 커밋. UIDesign/ → .gitignore(로컬 전용). master clone 재현 가능 상태 회복. **1194/1194 PASS**, 0 TS, 0 lint. 커밋: `2f4948d`·`dc7bede`·`99f64b5`·`a74330b`·`f186817`·`89438c0`·`10f7539`. stale 브랜치 3개 삭제 완료(feature/e1·e2·s08).
- **Yunse Y0/Y1 완료 확인 ✅ (2026-05-09, 재검증)** — `src/types/chart.ts` YunseCore(4레이어: 대운·세운·월운·일운) + `src/lib/kasi/normalize.ts:mapSsajuToYunse()` + `src/components/me/yunse-card.tsx` YunseCard(실구현, placeholder 아님) + 테스트 passing. 메모리 stale 항목 해소.
- **P1/P2 정리 완료 ✅ (2026-05-09)** — C4 stale 브랜치 3개 삭제(feature/e1·e2·s08). C5 vitest reporter = non-issue(pnpm test 1194/1194 정상). C1 Yunse 완료 확인. 잔여: C2(D1 결제 페이지 의존) · B1(pnpm db:push 0024+0026 사용자 수동) · B2(Supabase Dashboard 수동).
- **§1.6 UI design-review 완료 ✅ (2026-05-10)** — /login·/signup·/feed·/onboarding·/relations/new 5개 화면 감사 + 5개 finding 수정(FINDING-001~005). 커밋 `ffcad13`·`365ebf6`·`0526aac`·`43af94b`. **1203/1203 PASS**, 0 TS, 0 lint. §1.3 잔여: time-accuracy Seg pill 변환·rounded-2xl 토큰화·Seg pill DRY·필터 매직픽셀(별도 PR).
- **Hapcard design-review Phase 0-9 완료 ✅ (2026-05-10)** — 13섹션 composition lock + AppBar/CtaBar/Timeline/ReplayButton/SharePreviewTile/MiniRadar 신규 + replayHint footer 제거 → ReplayButton 컴포넌트로 대체. commits `982ea52`~`6bd7ea6`(13개). **1252/1252 PASS**, 0 TS, 0 lint. /qa-only DONE_WITH_CONCERNS(6 PASS · 1 BLOCKED hapcard 500 · 1 INFO). 로컬 `main` 브랜치 fast-forward 완료. GitHub PR 생성 보류(git remote 미설정). §1.3 잔여: 테스트 계정 onboarding 시드 추가 → hapcard 브라우저 E2E #1-#13 별도 PR.
- **Phase B 완료 ✅ (2026-05-11)** — Hanja 노출 제거 + LLM 프롬프트 한글 친화 (ADR-038). 주요 산출물: `src/lib/glossary/hanja-readings.ts` + `post-process.ts` + `banned-phrases.containsClassicalHanja()` + builder UI 매핑 + 6모드 prompts v0.8 + 4 컴포넌트 safety-net. **1324/1324 PASS**, 0 TS, 0 lint. §1.3 잔여: evidence.tsx compound regex 확장 + SHINSAL_READINGS sort 강화 + `cause_factors` 렌더 컴포넌트 구현 시 `convertHanja()` 필수 + `containsClassicalHanja()` 런타임 연결 결정(§1.1 대기) (모두 별도 PR). §사용자 수동 절차: `pnpm seed:prompts` → v0.7 6건 rolled_back, v0.8 6건 active.
- **Phase B §1.3 잔여 정리 완료 ✅ (2026-05-13)** — 4건 중 3건 머지(`HapcardCauseFactors`는 backlog). PR1 `eac2fbb`(SHINSAL sort) · PR2 `49dad3c`(evidence compound regex + GLOSSARY_TERMS 6→18) · PR3 `bd4ed56`(containsClassicalHanja Option C warn-and-pass). **1335/1335 PASS**, 0 TS, 0 lint. §1.1 결정(2026-05-13): containsClassicalHanja = Option C(경고 로그 + 통과), 진행 방식 = 독립 3 PR 순차, `cause_factors` 렌더 컴포넌트 = 백로그(ADR-038:45 의무 명시).
- **Hapcard E2E #1-#13 브라우저 검증 완료 ✅ (2026-05-13)** — 11/13 PASS (84.6%). 4건 §1.3 이슈 수정: PR-X1 `043e166`(IljuChip ADR-038 convertHanja) · PR-X2 `8b2b665`(gauge breakdown Math.round) · PR-X3 `3f1831c`(OG try-catch 500 반환). **1338/1338 PASS**, 0 TS, 0 lint. §1.3 잔여: ISSUE-3(Body eyebrow "전체 해석"으로 변경 — ✅ 완료 `4233bda`) · ISSUE-4(OG Noto Sans KR 폰트 등록 + Edge 런타임 복원 — ✅ 완료 `dcf5b88`). 모든 §1.3 잔여 해소.
- **main ff push 완료 ✅ (2026-05-28)** — `origin/main` `0d78f97` → `cf288f6` (28 ff). 26 코드 커밋 + RAG 사전검토 docs(`43265f9`) + vaul jsdom mock(`cf288f6`). typecheck/lint/test 전체 PASS. Vercel 자동 배포 트리거됨. **1690/1690 PASS**.
- **오늘사이 방향 정렬 라운드 진행 중 (2026-05-28)** — `feature/today-direction-alignment` branch. Phase 1(G3+G4 카피 `39684d1`) + Phase 2(G1 환영 팝업 `7e61b49`) 완료. **1695/1695 PASS**(+5 신규). Phase 3(G2 오늘카드 인연 종합 — 별도 "오늘 합온도" 점수식 + GPT-5 격상 + 캐시 키 relation_chart_hash 추가, 6 sub-cycle TDD) 새 세션 재개 예정. 유저사이드 용어 통일 규칙 신규 영구 확정([[feedback-user-side-terminology]]).
- **오늘사이 Phase 3 (G2 오늘카드 인연 종합) 완료 ✅ (2026-05-28)** — 10-cycle TDD 모두 그린. `feature/today-direction-alignment`에 C1~C9 커밋(`bd4561b` C1 → `97c15c2` C9). **1760/1760 PASS**(+65 신규), 0 TS, 0 lint. 주요 산출물: `src/types/dailyHap.ts` 3필드(relation_id/relation_nickname/today_compat_score) · `src/lib/scoring/today.ts` 결정형 점수식(가중치 0.25/0.15/0.20/0.40, **W1 사용자 확정**, TODAY_SCORING_VERSION='1.0.0', SCORING_VERSION 독립) · `src/lib/today/cache-key.ts` 객체 인자(self+relation+date+prompt_ver+model_id) · `src/lib/today/builder.ts` fetchRelation/fetchRelationChart + 3축 빌더 · `src/lib/today/openai.ts` 3축 페이로드 + GPT-5 격상 · `src/lib/today/relation-picker.ts` 자동 선택(preferred → 최근 등록 → null) · `prompts/system/today_with_relation.md` v0.1(3 시나리오 예시) · `src/app/api/today/route.ts` relation_id query + flag 분기 + applyRelationMetaToResponse(캐시 hit 시에도 최신 relation 재주입) · `src/components/today/today-hero.tsx` 인연 chip + 오늘 합온도 라벨 + 인연 0건 CTA · `tech_stack.md` LLM 매핑 동시 갱신(§12). feature flag `NEXT_PUBLIC_TODAY_WITH_RELATION` 기본 켜기, `false` 시 단독축 즉시 복귀. DB 마이그레이션 없음(relation 필드는 in-memory 응답). 잔여(별도 PR): daily_haps 컬럼 영속화(relation_id/nickname/today_compat_score) · 인연 chip 인터랙티브 드롭다운 · lazy relation chart compute · C10 browser manual smoke 사용자 검증.
- **Phase 3 main 통합 + F1~F4 후속 완료 ✅ (2026-05-28)** — Phase A: main ff 머지 + push(`cf288f6..b278e7e`, 13 커밋, Vercel auto-deploy). Phase B: `feature/post-g2-followups` 11 커밋(`5b759bb`~`3b4809d`). 주요 산출물: F1(daily_haps 영속화) — `supabase/migrations/0029_daily_haps_g2_columns.sql`(relation_nickname/today_compat_score + llm_model default gpt-5) · route.ts saveCard 영속화 + fetchTodayCache primary_relation_id 필터(인연 교체 시 캐시 분리) · `rowToCard` export · `scripts/verify-0029.ts`. F2(인연 chip 인터랙티브) — `src/components/today/relation-chip.tsx` vaul Drawer + 5건 cap + ✓ aria-current + controlled open prop · today-hero outer Link 해제 + chipNode 슬롯 · today-page-client router.replace + invalidateQueries · ko.json `with_relation.menu_title/view_all/current_label/close_label`. F3(lazy KASI compute) — `src/lib/today/lazy-relation-chart.ts:ensureRelationChart` + error_events INSERT(KASI_COMPUTE_FAIL). F4(prompt_versions 시드) — `seed-prompts.ts` HAPCARD_MODE_NAMES + OTHER_VALID_NAMES 분리, today_with_relation v0.1 적재. §사용자 수동 절차: `pnpm db:push`(0029) → `pnpm tsx scripts/verify-0029.ts` PASS → `pnpm seed:prompts`(7 row, today_with_relation v0.1 active).
- **Phase 3 후속 4-task PR ✅ (2026-05-28)** — `feature/post-phase3-followups-batch` 5 커밋(`7ca2f6d`·`dd88c28`·`b2274b6`·`ed25700`·`30e4a30`). Task 4(DailyHapCard legacy 3필드 즉시 삭제: compat_score·headline_strength·delta_vs_yesterday) + Task 1(오늘카드 latency instrumentation — builder phase 측정 + 25s LLM timeout + error_events trace 적재 + LLM_PARSE_FAIL/TODAY_BUILD_FAIL 코드) + Task 2(ADR-008 canary 5% routing infra — loadPromptForUser SHA-256 sampling + seed-prompts canary frontmatter + today/openai.ts DB-backed 전환 + daily_hap 시드 추가 + 7건 v(bump) canary) + Task 3(393px RelationChip overflow guard — max-w + truncate + shrink-0). **1819/1819 PASS**(+35), 0 TS, 0 lint. §사용자 수동 절차: `pnpm seed:prompts` → 15 row (8 active + 7 canary) 시드 확인. §1.3 잔여: today cache_key 의 hardcoded prompt_version (canary 본문 변경 시 동기화 필요) · guest canary 노출 정책 (현재 `__guest__` fixed seed) — 별도 PR.
- **QA Phase 3 후속 검증 + 회귀 fix ✅ (2026-05-28)** — `/qa docs/qa/local_e2e_smoke.md` 9 flow 자동 검증. main 직접 4 커밋(`466935e`·`902af27`·`dfae986`·`c857b6d`). 2 회귀 발견 + fix: **ISSUE-001** (F6 오늘카드 LLM_PARSE_FAIL `Unexpected end of JSON input` → TEMPLATE fallback; root cause = `max_completion_tokens: 800` 한도, fix = 2000 상향) + **ISSUE-002** (error_events 테이블 0016 시점 RLS enabled + 0 policies → Task 1 instrumentation 적재 + lazy-relation-chart F3.3 KASI 로깅 모두 silently fail; fix = 0030 마이그레이션 authenticated INSERT). DB: 0029 + 0030 db push 완료, seed:prompts 15 row 적용. 9 flow sweep: F1/F4/F7 ✅, F2 redirect ✅, F0/F3 진입 ✅, F5/F8 manual smoke 위임, F6 ISSUE 발견+fix. **1820/1820 PASS** (+1 regression test). §사용자 수동: dev server 재시작 후 F6 manual smoke verify (LLM 응답 시간 회복 + daily_haps 오늘 row 시드 + error_events 신규 적재 없음). §1.3 잔여: error_events SELECT 정책 · F3.3 KASI 적재 e2e verify · today 14-26s latency 추가 최적화 · F5/F8 자동 QA — 별도 PR.
- **결제 전환(pay-per-use) Phase 1 완료 ✅ (2026-06-01)** — `/plan-eng-review` 설계·승인. 부적 충전(토큰 번들 구매) 제거 → 유료 기능(합카드·만약합·다시합) 사용 시 즉시결제(ADR-039 신규 예정). branch `feature/pay-per-use-billing`. §1.1 5결정: 하이브리드(부적 우선→부족 시 1회 현금)·1회성 Toss 결제창·800/500/400원·그린필드·**원자성 C(선생성+성공 시 결제)**. **git**: main에 이전 세션 미커밋 WIP 129파일 스냅샷 `b414e23`(토큰충전 흐름 본체+launch readiness+LLM resilience — Phase 6 삭제 복구용) + Phase 1 `4de3d76`(`feature-prices.ts` 800/500/400 단일출처 + migration `20260601000000` payments.charge_type/feature_id/feature_ref·`confirm_feature_payment` RPC(토큰적립 X)·drop `confirm_token_purchase` + error-codes `PAYMENT_REQUIRED`/`RATE_LIMITED` additive). tsc 0, 변경 테스트 GREEN. Phase 2~9(서버게이트 unlock/gate/cash-limit·결제라우트 init/confirm·3라우트 통합+replay 멱등fix·클라이언트 pay-sheet·제거·read-path 유출감사·docs/ADR·검증) 잔여 → task #11~#18. 핸드오프 메모: `session_pay_per_use_phase1_complete_2026_06_01.md`. 플랜: `~/.claude/plans/task-cached-goose.md`. ⚠️ `db:push`(0601 migration)는 Phase 6 제거 후에만(안 그러면 라이브 confirm 라우트 깨짐).
- **pay-per-use Phase 2+3 완료 ✅ (2026-06-02)** — TDD로 서버 코어 완성. **Phase 2 `f1aef9e`**(13 tests): `feature-unlock.ts`(`isFeatureUnlocked` 단일 잠금 게이트) + `feature-gate.ts`(`resolveFeatureCharge`→free|unlocked|pay_required, charged 플래그) + `cash-gen-limit.ts`(일일 미결제 선생성 한도 5, env `CASH_GEN_DAILY_LIMIT`). **Phase 3 `f31b457`**(20 tests): `feature-complete.ts`(`confirmFeaturePaymentForUser` 토큰적립X·멱등) + `POST /api/payments/feature/init`(confirmed 단락·pending 재사용·신규 insert) + `GET /api/payments/feature/confirm`(303 to allowlist `/hapcard`|`/whatif` + `&paid=ref`, open-redirect 방어) + `src/types/feature-payment.ts`. interim `database.types.ts` 패치(payments charge_type/feature_id/feature_ref + token_amount nullable + confirm_feature_payment RPC — Phase 6 regen이 정식화). 전체 1965/1965·payments 74/74 PASS, tsc 0, lint 0. 핸드오프: `session_pay_per_use_phase2_3_complete_2026_06_02.md`. **Phase 4(라우트 통합 + replay 멱등 fix)부터 재개**, 이후 Q2 = Phase 4 후 `/codex challenge` 중간 게이트. Q1 = me "충전" 버튼 제거(Phase 5).
- **pay-per-use Phase 4 완료 ✅ + Q2 /codex challenge 실행 (2026-06-02)** — 3 유료 라우트(합카드/만약합/다시합) pay-per-use 게이트 통합 + replay 멱등 fix. 커밋 `5d559c2`. **2003/2003 PASS, tsc 0, lint 0.** 산출물: `paymentRequiredResponse()` 402 헬퍼(`amount_krw`) · hapcard/whatif cache SELECT 제거→`resolveFeatureCharge`(free|unlocked|pay_required)+`checkCashGenLimit`(429)+선생성→402 · replay dated ref `replay:{id}:{jinjin_date}`+`deduct/refund_tokens_once`+3-way idempotency(row+unlocked→200 / row+미잠금→402 / no-row→gate) · `buildReplay` 23505 recovery(whatif 패턴 미러). 라우트 테스트=게이트 모듈 mock + real-gate 402 통합 1건/라우트. **Q2 /codex challenge**: codex CLI v0.118→**v0.136 업그레이드**(gpt-5.5 요구, MCP 격리 우회) 후 실행. **7 findings = §1.3 residuals**(cash gate 견고=결제 없이 유료 본문 잠금해제 불가, Phase 4 회귀 아님, 대부분 pre-existing): #1 환불 후 무료 재잠금해제(free-token) · #2 `deduct_tokens_once` 무락 동시 overspend(pre-existing RPC) · #3 KST 자정 cash 고아(모델C 엣지) · #4 init 임의 ref 결제(self-harm) · #5 cash-gen 한도 raceable · #6 RPC error→pay_required(잔액 있어도 현금 요구) · #7 init 더블탭 500. **disposition 확정(사용자 2026-06-02): Phase 5 진행 + 자연 단계 처리** — #4/#6/#7=Phase 5 서버 하드닝, #3=Phase 8 ADR-039 한계 문서화, #1/#2/#5=free-token backlog. 핸드오프: `session_pay_per_use_phase4_complete_codex_findings_2026_06_02.md`. **Phase 5(클라이언트 pay-sheet+402 처리+me 충전버튼 제거)부터 재개.** db:push는 Phase 6 후에만.
- **pay-per-use Phase 5 완료 ✅ + 서버 하드닝 #4/#6/#7 (2026-06-02)** — 4 원자 커밋 (§1.1 사용자 확정: charge-sheet 셸 재사용 + 4-commit). **A `0664598`**(#6): `feature-gate.ts` 2중 판별(errcode P0001 + message INSUFFICIENT_TOKENS만 pay_required, 그 외 RPC error 재throw→500, charged=false 유지로 환불 없음). **B `2d94491`**(#4+#7): `feature-ref-ownership.ts`(`verifyFeatureRefOwnership` per-feature SELECT — hapcard/whatif=cache_key, replay=parse `replay:{id}:{date}`→hapcard_replays) + init 소유검증→404 `PAYMENT_REF_NOT_FOUND`(에러코드는 INVALID_BODY/UNAUTHORIZED 형제처럼 bare string, 카탈로그 미추가) + 23505 catch→pending 재사용. **C `(committed)`**: `feature-pay-sheet.tsx`(vaul Drawer + charge-client Toss 마운트 회수, 전용 selector `#feature-payment-{methods,agreement}`, prop=`featureRef`(React `ref` 예약 회피)) + 3 뷰(HapcardView/WhatifView/replay-button) 402 payload 운반 + PAYMENT_REQUIRED 가로채기(whatif는 ErrorCard보다 먼저; replay `?replay=1` 복귀 시 다이얼로그 자동 재오픈+1회 재발화) + ko.json `replayButton.afterPayLoading`. **D `57a443d`**: `ERROR_CTA.INSUFFICIENT_TOKENS` 제거(→ `{}`) + TalismanCard 충전버튼·onCharge 제거. **2024/2024 PASS, tsc 0, lint 0.** 핵심 설계: confirm 303 full-document redirect→fresh QueryClient→remount 자동 refetch 이므로 `?paid` refetch 배선 불필요(Whatif useSearchParams 미추가로 Next 15 Suspense 빌드 회피). **§1.2 /review 완료**(Claude adversarial pass): 머니패스 정확성 clean(무결제유출·이중과금·결제후무콘텐츠 없음, #6·#4·PII 검증). 비차단 견고성 4건 → **사용자 결정: 전부 §1.3 후속 기록**(R-P1 unlock implicit remount 의존=CLARIFY-1 · R-P2 replay param 잔존(멱등) · R-P3a PAYMENT_REF_NOT_FOUND 카탈로그 미등록 · R-P3b confirm next strip). §1.3 잔여: `me.wallet.charge` dead i18n 키(무해) · 레거시 `/payments/charge` 페이지 4곳=Phase 6 삭제 대상 + 위 review 4건. **Phase 6(제거 + db:push)부터 재개.** db:push(20260601000000)는 Phase 6 후에만.
- **pay-per-use Phase 6 코드 제거 완료 ✅ (2026-06-03)** — 레거시 토큰충전 경로 1 원자 커밋 `98fcc61`. **1991/1991 PASS, tsc 0, lint 0, `pnpm verify:billing-policy-readiness` PASS(8/8).** 삭제: `products.ts`·`token-costs.ts`·`payments/charge/*`·`payments/success/page.tsx`·단수 `payment/*`(checkout/fail/success, §1.1 전부 삭제 확정)·`charge-sheet.tsx`·old `api/payments/{init,order,confirm}/route.ts` + 6 테스트. 슬림: `complete.ts`(`confirmPaymentForUser`+`getTossProduct` import 제거, `markPayment*`/`confirmOrQueryTossPayment`/`PaymentFlowError` 유지·export — feature-complete 의존) · `wallet.ts`(`WalletProduct`/`PaymentInit*`/`Order*` 제거, ledger 타입 유지). 편집: `payments/fail/page.tsx` CTA `/payments/charge`→`/feed`(유일 dangling) + 테스트 동기화 · `verify-billing-policy-readiness.ts` 재작성(feature-prices 800/500/400 + products/token-costs 부재 + feature routes + 20260601000000 + refund 안전망 코드 단언, PRD/doc 단언 제거=Phase 8). stale `.next/types` validator 가 삭제 라우트 참조 → `rm -rf .next` 후 tsc 0(소스 clean). **`migrations.manifest.ts` 미수정**(standalone `feature-pay-per-use.migration.test.ts` 가 커버). **db:push 적용 완료 ✅ (2026-06-03)**: `20260601000000` 라이브 반영(payments charge_type/feature_id/feature_ref + shape check + partial-unique index + `confirm_feature_payment` RPC + **drop `confirm_token_purchase`**). `db:push:dry`→"up to date", `tests/db/` 58/58 PASS. 링크 `jamhkucluhiibqpjsiov`. **재개**: Phase 7(read-path 감사 — GET hapcard/OG/feed/snapshots 가 `isFeatureUnlocked` 통과 검증). 핸드오프: `session_pay_per_use_phase6_complete_2026_06_03.md`.
- **pay-per-use Phase 7 완료 ✅ (2026-06-03)** — read-path 미결제 본문 유출 감사 + 차단. 커밋 `97a1b21`. 감사 결론: 과제 명시 4경로(hapcard page·OG·feed·replay)는 안전(page/replay 기게이트, feed/OG=메타데이터)이나 **미명시 2 GET 서브라우트**(`ohaeng-interpretation`·`role-analysis`)가 게이트 없이 hapcard 본문 섹션을 반환 = 유출. replay 게이트 패턴 이식: hapcard fetch 직후 `isFeatureUnlocked('hapcard', cache_key)` 미통과 시 402(stored/rules 분기 이전), `cache_key` SELECT 추가, DB 마이그레이션 없음. TDD 미결제→402+본문 미포함 단언 2건. **1994/1994 PASS, tsc 0, lint 0.** §1.1 범위 확정: 본문 2경로만 게이트, snapshots/OG/share(점수·메타)=Phase 8 ADR-039 수용 노출.
- **pay-per-use Phase 8 완료 ✅ (2026-06-03)** — ADR-039 신규(`docs/adr/ADR-039-pay-per-use-billing.md`, §1.1 **비협상**) + 문서 동기화. payments.md/db_schema.md 이전 WIP 마감(charge_type default `token_charge` 정정 · 인덱스명 `payments_feature_open_uidx` 정정 · `payments_feature_use_shape` check 추가 · 모델 C/read-path 게이트/수용 메타데이터 문단) + FGI §17 ADR-039 행 + CLAUDE.md §3 비협상 등록. `pnpm verify:billing-policy-readiness` PASS(8/8). **재개: Phase 9(/qa + /cso 통합 검증).**
- **앱인토스 검토 + P0 가격 개정 (2026-06-07, Cowork 세션)** — 앱인토스 연동 검토 보고서 `docs/research/apps_in_toss_integration_review_2026-06-07.md` 작성 + §1.1 D1~D6 확정(Vite SPA 신규·user_key만·IAP+웹 병행·8플로우·TDS 채널톡 후·**가격 1,000/800/600 웹·미니앱 통일 + 부적 10/8/6p**). 적용: `feature-prices.ts` + ADR-039 Amended + payments.md + FGI(§3.2/3.3/3.4/3.5/ADR표 등 16곳) + PRD(§13 stale 충전 모델 → pay-per-use 재작성 포함 12곳) + 본 파일 §3. ✅ **가격 테스트 동기화 완료** (2026-06-08 검증): `feature-prices.ts`·`feature-prices.test.ts`·`verify-billing-policy-readiness.ts` 모두 1,000/800/600 정합 — 이전 "RED 예상" 경고 해소.
- **컨텍스트 동기화 + main 정렬 완료 ✅ (2026-06-08, Claude Code)** — Claude Works(Cowork)·Claude Code 히스토리 desync 해소. ① 검증 베이스라인: **2153/2153 PASS**(264 files) · tsc 0 · lint 0 — 문서 stale "1997 PASS" 대체. ② `chore/twoday-rebrand-cycle` 26커밋(리브랜드 오늘사이→오늘케미 · D6 가격 1,000/800/600 · 앱인토스 리서치) `origin/main` ff-push 완료(`b414e23..4f2a0fc`, Vercel auto-deploy). ③ 본 §2 stale 경고 제거 + 브랜드 문서 통일. 플랜: `~/.claude/plans/claude-works-partitioned-turing.md`.
- **Phase 1A 커버리지 분석 + 용어 통일 완료 ✅ (2026-06-08, Claude Code)** — ① **Phase 1A**: 코드 테스트 양호(264 files/**2153 tests**). 라우트 ~완전 커버(legal/[slug]만 thin). 컴포넌트 갭 ~10건(ui/ 프리미티브·picker 제외): talisman-card·swipe-row·onboarding-access-guard·info-card·legal-document-dialog·legal-markdown·lang-sheet·theme-toggle·about-dialog. **최대 갭=E2E**(mocked smoke 3건뿐). ② **§1.1 용어 결정+통일**(`bc55dbc`, main 동기화): hapcard=**케미카드** · whatif=**또 다른 나**(자기진단 6모드, "만약에 우리"는 stale) · replay=**케미 다시 맞추기** · today=**오늘 케미**. talisman `hapcard_use='오늘 케미'` 버그 수정 + order_name/ko.json/§8 일괄 통일. 코드 식별자(hapcard/whatif/replay) 불변. **2153/2153 PASS**, tsc0, lint0. ③ §1.3 잔여: talisman REASON_LABEL `purchase`/`bonus` stale 키 + `initial_credit`/`daily_bonus` 미라벨(별도). **다음(Phase 1B~1J, tasks #6~#14)**: 1B 컴포넌트 테스트 ~10 · 1C(legal/[slug]) · 1D E2E 8플로우(최우선 갭) · 1E /qa · 1F cause_factors(ADR-038) · 1G AI 생성 고지 라벨(법적) · 1H design-review · 1I review+codex · 1J 외부 블로커. 핸드오프: `session_phase1_terminology_handoff_2026_06_08.md`.
- **Phase 1B/1C/1D/1F/1J 완료 ✅ (2026-06-08, Claude Code)** — branch `feature/phase1-coverage-qa` 4커밋(`8746309` 1B · `fdda8fa` 1C · `e34daa3` 1D · `66e1aea` 1F), **origin/main 미머지**. **1B**: 9개 미커버 컴포넌트 +33 tests(§1.3 talisman 미수정). **1C**: 라우트 감사 **28/28 직접 핸들러 테스트 보유**(replay 멱등·feed badge/sort 실검증), 유일 thin legal/documents/[slug]에 privacy 케이스 추가. **1D**: 기존 smoke 3종이 F0~F8 breadth+`verify:e2e-coverage-readiness` PASS 이미 충족 → 진짜 갭=인증 화면 browser-depth(F6 `/`·F7 `/me`) → `tests/e2e/today-me-depth.spec.ts`(@auth, mocked) 추가(playwright --list 4 tests+tsc+lint 검증, 실행은 `RUN_AUTH_E2E=1 pnpm e2e` 환경게이트). **1F**: cause_factors(명리 근거)가 생성·저장만 되고 렌더 없었음 → `HapcardCauseFactors`(번호 3-항목, screens-result.jsx 와이어) + HapcardView evidence 탭 배선 + ko.json `causeFactors`. **ADR-038 convertHanja 안전망 + ADR-015 충족**. 8 tests GREEN. **1J**: `docs/qa/external_settings_checklist.md` 이미 완성(전 외부게이트) — 신규 불필요. **§1.1 3결정**: 1F=와이어 새섹션 · 1G=결과 배지+온보딩1회 · 순서=1F1G 후 일괄검증. **다음: 1G(AI 생성 고지, greenfield) → 1E/1H/1I 브라우저 검증 → main 머지**. 핸드오프: `session_phase1_coverage_qa_handoff_2026_06_08.md`. **2190/2190 PASS**(274 files, baseline 2153 +37), tsc0, lint0.
- **Phase 1G(AI 생성 고지) + 1I(코드검증) 완료 ✅ (2026-06-08, Claude Code)** — `AiDisclosureBadge`(light/dark 톤) → 케미카드 hero(replay 동일뷰 포함)·오늘 케미 hero·또 다른 나 hero 3파일 배선(4결과면 커버) + `AiDisclosureNotice` 온보딩 review 단계 inline(온보딩=일회성→내재적 1회) + `aiDisclosure` i18n(배지 'AI 생성' + 표준톤 고지 "AI가 만든 해석이에요", **§1.1 확정**). 커밋 `b0e1131`. TDD +12 tests(badge4·notice4·onboarding1·wiring3). **1I 누적 코드검증**: `/review`(Claude critical pass + adversarial·frontend §1.6 서브에이전트 2개) **CLEAN** + `/codex` **NO ISSUES**(Windows 샌드박스 spawn hang → diff 임베드 우회) — 교차모델 ship 일치. §1.3 정보성 3건(rounded-2xl·bg-white/20·ui/Badge 재사용)=기존 컨벤션/백로그라 미수정. **2202/2202 PASS**(277 files, +12), tsc0, lint0. **Phase 1 전체 `feature/phase1-coverage-qa` → `origin/main` ff-push.** 잔여: 1E `/qa`·1H `/design-review` 브라우저 검증(dev서버+LLM비용, 별도).
- **인연 등록 슬롯 과금(relation_slot) 완료 ✅ (2026-06-10, Claude Code)** — ADR-039 **Amended §9**(모델 B): 인연 2명까지 무료, 3번째부터 `relation_slot` 1,000원/10부적(하이브리드). branch `feature/relation-slot-billing`, origin/main 대비 14커밋. **2264/2264 PASS**(280 files, baseline 2202 +62), tsc0, lint0, `verify:billing-policy-readiness` PASS. 산출물: `20260610000000_relation_slot_registration.sql`(pending_relation_registrations owner-SELECT-only RLS + FK 인덱스 + CHECK-drift 가드 + token_ledger 멱등 인덱스/RPC `relation_slot_use`·`relation_slot_refund` 확장) · `feature-prices.ts`(relation_slot 1000 + `FREE_RELATION_SLOTS`=2) · `feature-ref-ownership.ts`(relation_slot 분기) · `src/lib/relations/insert.ts`(`insertRelationAndComputeChart` + `RelationInsertError`) · `src/lib/relations/materialize.ts`(`materializeRelationSlot` claim-first 멱등 + un-claim 가드) · `route.ts`(count 게이트 + lazy recovery[paid ref 직접 추출] + 402/환불) · `confirm/route.ts`(relation_slot materialize 훅 + `/feed` allowlist) · mode page 402→FeaturePaySheet + 사전 고지 · `/feed` `?paid` draft reset. **§1.2 검증**: `/review`(Claude 5 서브에이전트: testing·security·data-migration·maintainability+api·adversarial) + `/codex challenge`(diff 임베드) — 12 finding 즉시 수정, 교차모델 일치. **§1.1 결정(2026-06-10)**: count 게이트 TOCTOU = **수용 리스크 유지**(자해성·매출 직접 손실 작음, launch 후 모니터링 트리거 재검토 — ADR-039 §9 수용 ①).
- **relation_slot DB 적용 + §1.3 잔여 전체 완료 ✅ (2026-06-10 2차, Claude Code)** — ① **DB 라이브 적용**(사용자 직접 SQL) 후 history desync 발견 → `supabase migration repair --status applied 20260610000000` + 라이브 검증(`pnpm test:integration` RLS 53/53 + 신규 `scripts/verify-relation-slot-migration.ts` 8/8: CHECK 왕복·RPC 멱등 2회차 inserted=false·anon 42501). ② **types regen**(`af3aec9`): `pnpm dlx supabase gen types --linked` → 슬롯 경로 stale 캐스트 전부 제거(materialize/route/confirm/ownership/cash-gen → `SupabaseClient<Database>`), default-null RPC 인자 `?? undefined` 정렬. ③ **§1.3 잔여 4건**(`1fb6eb7`·`90c6001`·`39ba890`·`14e4579`): origin `todaychemi` 전환+§15 · cash-gen 리스트를 `FEATURE_PRICES_KRW.llm_generated` 플래그에서 파생(3-list 드리프트 차단) · confirm ref shape 가드(위반 시 Sentry+스킵, fail redirect 금지 유지) · **draft PII purge cron**(`20260610120000`, §1.1 확정: 미결제·미머티리얼라이즈 30일 삭제[paid 고아 절대 보호] + 전달완료/소비 행 7일 draft `'{}'` 스크럽[크래시-복구 창 보존], 일 1회 pg_cron). **2273/2273 PASS**, tsc0, lint0, verify PASS. **purge cron 라이브 적용 ✅**(사용자 SQL 직접 실행, cron jobid 4 + `migration repair --status applied 20260610120000` + history 동기화 확인 — 절차 runbook: `docs/runbooks/manual_migration_apply.md`).
- **relation_slot /qa 라이브 smoke + P0 수정 완료 ✅ (2026-06-10, Claude Code)** — `/qa` 라이브 스모크에서 **P0(F-001) 발견·수정**: materialize claim-first 가 relations INSERT 전 `relation_id` 기록 → FK 위반(23503), 인연 등록 100% 실패(차감→환불, 등록 0). 단위테스트는 supabase mock 이 FK 미시뮬레이션이라 통과 — 라이브 smoke 에서만 노출(학습 기록). **수정**(`feb93af`): `delivered_at` 컬럼으로 상태 분리(`materialized_at`=클레임 / `delivered_at`=전달완료, `relation_id` 는 INSERT 후 기록·FK 충족, `relation_id=pending_id` deterministic → 23505 멱등). 마이그레이션 `20260610130000_pending_delivered_at.sql`(additive + purge 함수 delivered_at 재작성) 라이브 적용 + repair. **라이브 검증 PASS**: 등록 count 13→14(환불 없음·balance −10) · 삭제→슬롯 회복 · 삭제 소비 판별(lazy recovery 가 삭제 인연 부활 안 함). **F-002(INFO)**: 사전 가격 고지 direct-nav 미표시=의도된 동작(`['feed']` 캐시 의존), client-nav 정상 표시 확인. 402 결제 시트는 단위/통합 커버리지로 충분(§1.1, 토큰 조작 회피). **2275/2275 PASS**, tsc0, lint0. 리포트: `.gstack/qa-reports/qa-report-relation-slot-2026-06-10.md`(로컬). relation_slot 기능 코드·DB·라이브 전부 검증 완료.
- **백로그 3건(B1/B2/B3) 완료 ✅ (2026-06-10, Claude Code)** — **B1**(`d6492bd`): database.types regen 후 불필요해진 `as unknown as SupabaseClient` 캐스트 21개를 13개 라우트/lib 에서 제거(동작 0, tsc 안전망, chart_core ChartCore→Json·chart-row 이중캐스트만 유지). **B2**(`55bf955`): 유저당 미전달(delivered_at NULL) pending 10개 캡 — 402-이탈 abuse 시 row 누적 차단, 초과 시 429(정상 흐름 무영향). **B3**(`ceac290`, §1.1 TOCTOU 수용→차단): 무료 등록을 `insert_relation_if_under_free_cap` 원자 RPC(`INSERT...SELECT...WHERE count<FREE_RELATION_SLOTS` 단일문, `20260610140000`)로 전환 — 동시 요청 row-level 직렬화로 무료 슬롯 우회 완전 차단(null=초과→유료). chart compute는 RPC 밖 best-effort. route 무료 게이트가 count SELECT 2-step→원자 RPC. ADR-039 §9 수용①→차단 갱신. **2279/2279 PASS**, tsc0, lint0, verify PASS. §사용자 수동: `20260610140000` 라이브 적용(조건부 INSERT RPC) — db:push 또는 SQL 직접+repair(runbook). 라이브 적용 후 database.types regen 시 RPC 타입 자연 반영.
- **현황 점검 + `20260610140000` 라이브 적용 완료 ✅ (2026-06-11, Claude Code)** — 점검에서 **P0 발견**: B3 RPC 호출 코드는 main push(→Vercel 배포)됐는데 마이그레이션은 미적용 → RPC 부재 시 `POST /api/relations` 전면 500(인연 등록 차단) 창이 ~1일 열려 있었음. 사용자 SQL 직접 실행 + `migration repair --status applied 20260610140000` + 신규 `scripts/verify-free-cap-rpc.ts` 3/3 PASS(cap=0 단락 null·FK 23503 배선·anon 42501 차단 — 쓰기 흔적 0) + `db:push:dry` up-to-date + `test:integration` 53/53. **types regen**: `insert_relation_if_under_free_cap` + `purge_pending_relation_drafts`(0610120000 적용분) 타입 반영. **로컬 정리**(§1.1 확정): 머지 브랜치 14개 삭제 + agent worktree 제거 + stale 원격 ref prune(stash@{0} 보존). **2279/2279 PASS**, tsc0, lint0. **교훈 영구화**: 마이그레이션 의존 코드는 라이브 적용 확인(`db:push:dry` up-to-date) 전 main push 금지 — `feedback_migration_before_deploy.md`(§13).
- **/code-review main `b34a11b..HEAD` + 수정 완료 ✅ (2026-06-11, Claude Code)** — 미리뷰 17커밋/44파일 9-앵글 finder→적대 verifier→sweep. **CONFIRMED 3건 수정**(`528ec00`, TDD +4 tests): **F1[HIGH]** `recoverPaidPendings` 필터 `materialized_at`→`delivered_at` — 클레임 후 죽은 현금 paid 고아가 영구 미복구였음(confirm 라우트는 catch 후 성공 리다이렉트뿐이라 이 필터가 유일 안전망; purge 는 paid 보호라 row 영구 잔존 + 캡 잠식) · **F2[MED]** materialize 전달 마킹 UPDATE 에러 무시 → log+Sentry, **throw 금지**(인연 이미 INSERT 라 throw 시 "전달됐는데 환불" 이중보상) · **F3** open-pending 캡 count 에러 fail-open → 500 fail-closed(구 동작·삭제된 테스트 복원) · F4 `error.code ?? 'unknown'` 4곳. **2283/2283 PASS**, tsc0, lint0. **§1.3 정보성 5건(수정 안 함)**: I1 캡 count-then-insert 비원자(soft cap 수용) · I2 relations 컬럼 리스트 SQL RPC(`20260610140000`)↔`insert.ts` 이중화(스키마 드리프트 주의) · I3 `relation_slot:` ref 포맷 4곳 인라인(공용 build/parse 헬퍼 후보) · I4 route.test mock 잔재(`_countEq`/`_upsertCharts`) · I5 verify 스크립트 report() 중복(스탠드얼론 관례 보류).
- **사주 분석 엔진 고도화(파생층+교차분석, ADR-040 신규) 완료 ✅ (2026-06-11)** — branch `feature/saju-derivation-layer` (이후 후속 포함 **총 48커밋, main ff-push `5898dcf..1a4215b` 2026-06-12 배포 완료**). **2474/2474 PASS**, tsc0, lint0. ① **P0 C1 버그픽스**: `yunseAdjustment.ts` 한자 ganji 매칭(프로덕션 yunse_adjustment 항상 0이던 버그) + `SCORING_VERSION 1→2` + ADR-036 feed 배지 버전 가드 + timeline 경계 마커. ② **P1 파생층**: `src/lib/saju/{ganji,jijanggan,sipsin,unseong,sinkang,yongsin,derive}.ts` — 8글자 십신·지장간(정기10/중기5/여기3)·억부 신강약(70/30)·용신/희신·음양·띠, `chart_core.derived` 내장 + **theory v2→v3**(전 캐시 회전), ssaju 테이블 전수 대조 픽스처. ③ **P2 교차분석**: `cross.ts`(cross-v1) — 양방향 십신 교차(지지=정기)·궁위 귀속·운세 교차(합/충)·일간 배합·연령차 밴드(PII: band 문자열만). ④ **P3 전달층**: LlmDerived projection(Zod fail-open)+cross_analysis 페이로드(hapcard/replay/today/whatif) + 프롬프트 14파일(6모드 **v0.15**+v0.16 canary·today_with_relation v0.3/v0.4·daily_hap v0.4·whatif v0.2) + **환각 가드 "제공 필드 외 단정 금지"** + hanja-readings 파생 용어 + ADR-040/문서 6종 + RAG query-text 확장. ⑤ **§1.2 검증**: /review 5 스페셜리스트+Claude adversarial+codex(stdin 우회, 'Argument list too long' 시) — **8건 수정**(query-text Zod 우회·replay 시간층 stale facts→daeun만·cross fail-open(`computeCrossAnalysisSafe`)·birthYearOf 검증·daeun index 가드·derived_version 고정·혼합 인코딩 글자별 정규화·v2 백필 스크립트 삭제) + 테스트 보강(sinkang 경계·삼형·catch 분기). §사용자 수동(배포 시점): `pnpm seed:prompts`(15행: v0.15 active 6+v0.16 canary 6+today v0.3/v0.4+daily v0.4) → `pnpm tsx --env-file=.env.local scripts/recompute-charts-v3.ts`(선택, lazy 대체 가능) → /qa 라이브 스모크(케미카드 근거 밀도+환각 부재). **§1.1 해소(2026-06-12)**: `LlmYunse.daeun.current.year` 측채널 = **수용 확정**(사용자 결정, 법적 리스크 없음 — ADR-040 §4 Accepted 명문화, year 유지).
- **명리 RAG 검수 완료 ✅ (2026-06-12, §1.1 2결정: 즉시 시드·경미 즉시 수정)** — 고전 원문 14건 RAG 자산화(維基文庫·ctext public domain: 淵海子平 地支藏遁歌/繼善篇·三命通會 사령일수/궁위/12운성·滴天髓 衰旺/中和/體用·**자평진전 신규**(zpjj) 용신/음양생사) + curator 감사 14/14 PASS + `pnpm seed:classics` 라이브 적용(**20→34 rows**) + retrieval 스모크(신규 주제 최상위 hit). **검수 리포트** `docs/qa/myeongri_rag_review_2026_06_12.md`: 7항목 = PASS 6 · 검증 불가 명기 1(연령 밴드) — 지장간 테이블 12지 전수 일치, 궁위 4라벨 4/4 고전 직접 근거, 신강약·용신 구조 고전 근거(수치는 자체 단순화 명기). - **배포+시드 완료 ✅ (2026-06-12)** — main ff-push `5898dcf..1a4215b`(48커밋, Vercel auto-deploy) + `pnpm seed:prompts` 15행 라이브 검증(6모드 v0.17 active+v0.18 canary·today_with_relation v0.5/v0.6·daily_hap v0.5, 구버전 자동 rolled_back) + classics 34 rows 라이브. v3 백필 미실행=의도(전 경로 lazy + derived v1 self-heal). **/qa 라이브 스모크 완료 ✅ (2026-06-12)** — 케미카드 신규 2건(일합 v0.17·썸합) + 오늘 케미(3축, 합온도 78) + 또 다른 나(work) 검증. **파생층 PASS**: 십신 교차(방향)·운세 교차(대운 합충) 근거 탭 노출 ✓ · 환각 부재(신살·미제공 십신 0건) ✓ · 한자 미노출 ✓ · `[DERIVED_INVALID]`/`[CROSS_INVALID]` 0건 ✓. **ISSUE-001 [HIGH, pre-existing] 발견**: classic_citation 전면 0건 — 실 query-text 단문이라 유사도 max 0.349 < 임계 0.60, retrieval 항상 0 hit(v0.13부터 동일, 회귀 아님). ADR-015/018 핵심 가치 비작동 → **§1.1 결정 ③ lexical 하이브리드로 해소 ✅(2026-06-13)** — `src/lib/rag/query-tags.ts`(buildRagQueryTags: mode+신강약+dominant 십신+용신+궁위/합충 → 고전 태그) + `retrieveClassics` queryTags 옵션(겹침 ≥2, 겹침↓→유사도↓→asset_id↑, 임베딩 합집합) + hapcard builder 배선 + `llm_grounding.md` §6 동기. TDD 5커밋(`c97b082`~`f331c86`), 로컬 main ff(**origin 미푸시**). 라이브 검증: retrieval 0→5 hits + 실 생성 1건 citations 2건(연해자평·적천수) grounding 통과 + UI 렌더 ✓. **2514/2514 PASS**, tsc 0, lint 0. 잔여(§1.3): whatif 경로 임베딩 전용 유지(type→mode 매핑 §1.1 필요) · classics 신규 자산 추가 시 query-tags.ts 태그 매핑 동기 · glossary tooltip "충돌"内 '충' 오매칭(pre-existing). INFO: today 25s 생성 중 일시 에러 상태(기존 latency §1.3 계열) · whatif `[CLASSICAL_HANJA] 戊` warn 1건(안전망 정상) · Radix Dialog aria 경고. 리포트: `.gstack/qa-reports/qa-report-saju-derivation-smoke-2026-06-12.md`.
**산식 권고 처분 완료(2026-06-12 사용자 확정)**: **R1 적용** — `jijangganWeightsFor` 사계월(辰戌丑未) {정기10·여기5·중기3} 교환 + **derived_version 1→2**(저장 v1은 resolveDerivedForLlm self-heal 재계산, DB 재기록·백필 불필요) + 프롬프트 범프(6모드 **v0.17**/v0.18 canary · today_with_relation v0.5/v0.6 · daily_hap v0.5 · whatif v0.3 — seed 15행 버전 갱신됨). **R2 불채택**(사령일수↔장간가 구성 학파 차로 일수 비례 매핑 정의 불가 — 리포트 명기). §6.1~6.3/6.7 고전 cross-ref 동기. daeun.current.year 측채널 **수용 확정**(ADR-040 §4 Accepted). 모드 태그: 썸합 3→6·첫합 4→7 보강.
- **§1.3 잔여 6건 전체 해소 ✅ (2026-06-11~12, §1.1 3결정 확정: today 동적 버전·replay lazy 이식·gungwi 근본 해결)** — 6커밋(`a07d2f1`~`5cb7248`). W1 normalizeGanji trim · W2 십신 5그룹 단일 출처(saju/sipsin.ts) · W3 `HapChungEvent.participants` 메타데이터(점수 무영향, push-order 재구성 큐 삭제, **자형 궁위 귀속**, 프롬프트 예시·§6.6 동기) · W4 replay lazy 재계산(`BuildReplayDeps.kasiServiceKey`, 결제 후 500 창 제거) · W5 today prompt_version 동적 파생(요청당 메모 1쿼리, canary 라벨 ADR-008 동기, 상수 삭제) · W6 검수 의뢰서 `docs/qa/myeongri_specialist_review_request.md`(7항목 자기완결, 전문가 발주=사용자). **2481/2481 PASS**, tsc0, lint0. 잔여=명리 specialist 검수 발주(외부, 사용자) + §1.1 daeun.current.year 1건.
- **잔여 개발 계획 승인 + T7 문서 동기 완료 ✅ (2026-06-13)** — 기획서·PRD·UI/UX vs 코드 전수 대조 플랜 승인(`~/.claude/plans/ui-ux-tidy-crab.md`). **§1.1 6결정**: D1 우선순위 T1(런치 P0)+T2(Phase 1 마감)+T3(Phase 1.5) 병행·T5 앱인토스 후순위 · D3·D4 **Flow A 절충안**(온보딩 4-step 유지 + 오늘홈 인연 0건 hero 강화 + `/relations/new` "생일을 잘 몰라요"→whatif first Track B 분기, 전용 입구 화면 기각) · D5 **S-06 진단 폼 폐기**(question_slot 예약 유지, ADR-025 Amend) · D6 설계 승인. branch `feature/flow-a-compromise`(미푸시): `40f5051` 설계 문서 + `a66886f` 문서 동기(FGI ADR-025/029/032 Amend·§3.1 슬롯 정정·§3.2 문항 폐기·§4.2 Flow A Amend + PRD §4·§5 체크박스 전면 현행화 + today_hap/replay stale 마감). **런치 외부 게이트 6 FAIL 실측**(verify:launch-readiness — Vercel env·Auth prod·Toss live·ZDR·Sentry·실환경 E2E = 대표 콘솔 액션). §1.1 보류 큐: G-7 en·딥합 가격(잔여 2건). 핸드오프 `session_remaining_plan_flow_a_handoff_2026_06_13.md`.
- **§1.1 3결정 추가 + G-10 구현 완료 ✅ (2026-06-13 2차)** — 결정: ① **G-1 푸시 채널=앱인토스 확정**(푸시 전체 S-97·D+7/30·운기는 T5 미니앱 이후 순연, 웹 푸시 없음 수용, TWA 푸시 동기 소멸→우선순위 하락) ② **G-8=Google Analytics**(계측 코드는 T2 개발—env 부재 시 no-op, GA 속성 설정은 개발 완료 후 사용자) ③ **T1 외부 설정=개발 완료 후**(사용자 통지 대기). **G-10 구현**(`2a34d20`, TDD +7 tests): 오늘홈 인연 0건 hero 유도 블록(ADR-032 타이틀+목적 서브카피+CTA 주역, `today-hero.tsx`) + `/relations/new/dob-time` "생일을 잘 몰라요" 토글→"처음 보는 나"(`/whatif/first_meet`) Track B 분기 카드(등록 비차단) + ko.json 키 5개. 기존 page.test 느슨한 정규식 1건 정밀화. **2521/2521 PASS**, tsc 0, lint 0. **브라우저 검증 ✅**(신규 QA 계정 FlowA1@test.com 시드 후 예시 인연 6건 삭제→0건 상태): hero 블록 렌더·Track B 카드·링크 href·콘솔 에러 0, 스크린샷 2장(`.gstack/qa-reports/screenshots/g10-*`). 로컬 main ff 머지(`2a34d20`, **origin 미푸시**). INFO: 신규 계정 today 첫 생성 25s 초과 시 TEMPLATE fallback 관찰(기존 latency §1.3 계열, 폴백 동작은 정상).
- **T2 Phase 1 마감 배치 완료 ✅ (2026-06-13 3차)** — 3커밋 main ff(`b775b0c`·`9a3aeff`·`2664ee0`, **origin 미푸시**). **G-8 GA4 계측**: `src/lib/analytics/ga.ts`(타입드 GaEvent 화이트리스트, PII 파라미터 금지, env `NEXT_PUBLIC_GA_MEASUREMENT_ID` 부재 시 전부 no-op) + GaScript/GaPageView(SPA page_view)/GaPurchaseTracker(?paid→purchase, 경로 기반 feature 판별·ref 원문 미전송·sessionStorage 멱등) 루트 layout 배선(Suspense) + 퍼널 6지점(sign_up email·onboarding_complete·relation_create·hapcard_view·share 3종·begin_checkout) + `pnpm build` PASS. **G-2 오탐 정정**: 공유 표시 범위 선택은 기구현(ShareSheet RANGE_OPTIONS 3종·기본 별명만) — PRD만 정정. **G-4 시나리오 추정 표시**: `score_breakdown.scenario_estimate`(optional, 점수 본체 무접촉 ADR-035) builder 전달 + hero ± 인라인(`scoreDeltaToTemperatureDelta`)+「시간 미상 · 시나리오 추정」배지(needs_badge 강조) + changeScore NUMERIC_FACTOR_KEYS 한정. **G-5 쉽게 보기**(D7 평이어 사전 22종 사용자 확정): `easy-term-map.ts`(긴 용어 우선+조사 조화 결정형 치환, 단글자 4종 제외) + expand 헤더 토글(role=switch, localStorage 보존) + hero·요약·근거 적용. **2559/2559 PASS**, tsc 0, lint 0. **G-6 PWA 차단 2건**: ① 앱 아이콘 자산 부재(192/512 PNG — 사용자/디자인 제공 필요) ② FGI §8.3 해석 본문 Cache-first 30일 정책 보안 검토(§1.1). G-7 en = §1.1 보류 유지. **T2 가능 범위 종료 → 다음 T3**(H-1 타임라인 뷰·H-2 변화 폭·H-4 공유 5종).
- **시주 진태양시 보정 도입 ✅ (2026-06-11, ADR-021 Amended — §1.1 3결정 확정)** — 실사용 QA에서 시주 불일치(본인 17:0x 출생: 본 서비스 신유 vs 3eyes 경신) 발견 → 원인 = 경도 보정 미적용(ADR-021 MVP 보류). **§1.1 확정**: ① 시주만 보정(년/월/일주 KASI 앵커 유지 — G0 모트 보존) ② 균시차 포함(Spencer 1971, 3eyes 동일) ③ 서울 기본 126.978°E. 산출물: `src/lib/kasi/solar-time.ts`(EoT+경도항, 결정형) · `minutesToBranchIndex`(분 해상도 시지) · normalize 보정 통합(조자시 통합 유지: 보정이 자정 넘어도 당일 일간) · `ComputeInput.birth_longitude` 배선(relations 저장값 사용, chart_hash 입력 추가) · **`DEFAULT_THEORY_PROFILE_VERSION v1→v2`**(전 캐시 자연 분리) · `ensure-user-chart.ts` lazy 재계산(me/chart+today, 재온보딩 불필요) · `scripts/recompute-charts-v2.ts` 백필. **2311/2311 PASS**(+27), tsc0, lint0. 균시차 음력 함정(양력 날짜 필수) 메모리 영구화. §1.3 신규: **ADR-021 번호 충돌**(FGI=지역화 RAG 보류 ↔ manseryeok_theory=경도 보정) — 리넘버링 §1.1 대기. 온보딩 경도 입력 UI = Phase 2 유지.
- **T3 H-1 인연 디테일 타임라인 뷰(S-09) 완료 ✅ (2026-06-13 4차, Claude Code)** — feat `8932a13`. PRD §5.2 S-09 구현. **§1.1 3결정(2026-06-13)**: ① **메모 제외 타임라인**(메모는 기존 별도 섹션 유지) ② **최신순 정렬**(desc, 최근이 위) ③ **v1 표시 전용**(이벤트 탭 동작 없음 — 결제 게이트 복잡성 회피, 탭 이동은 별도 PR). 산출물: `GET /api/relations/[id]/timeline`(등록 `relations.created_at`+케미카드 `hapcards`+다시맞추기 `hapcard_replays` 3소스 병합, RLS 소유 게이트, **메타데이터만**=본문·점수·PII 미포함[ADR-039 read-path 비대상], 최신순+50건 상한[등록 이벤트는 상한 무관 항상 포함], 케미카드 0건 시 replay 쿼리 생략) · `RelationTimeline` 컴포넌트(self-contained useQuery=HapcardTimeline 패턴, 스켈레톤/에러/빈상태/데이터 4상태, KST 날짜 YYYY.MM.DD 표기, 점색상 타입별, 링크·버튼 없음) · `feed/[relationId]/page.tsx` 합흐름 그래프 아래 배선 · `RelationTimelineEvent/Response` 타입 · ko.json `relations.detail.timeline.*`. TDD +18 tests(route 10·component 6·page 2). **§1.2 검증**: ultracode 다차원 적대적 리뷰(correctness·security·convention 3차원 14 finding → 적대 verifier **0 confirmed**, 전부 RLS 방어심층/TDD 커버리지/비행동 스타일로 기각). **2577/2577 PASS**(300 files, baseline 2559 +18), tsc 0, lint 0. 로컬 main 커밋(**origin 미푸시**).
- **T3 H-4 공유케미카드 5종 레이아웃(S-96) 완료 ✅ (2026-06-13 6차, Claude Code)** — feat `803c203`(C1-C3 foundation) + `2d016a5`(C4-C5). PRD §5.2 S-96 구현 + **ADR-024 Amended(FGI §17, §1.1)**. **§1.1 3결정(2026-06-13)**: ① **5종=레이아웃 변형**(미니멀=온도만/오행 그래픽/영역 레이더/한 줄 코멘트/흐름 타임라인 — "5종" PRD §574에만 있고 열거 없어 §1.1 확정) ② **프리뷰=실제 OG 통일**(인앱 미리보기·저장 이미지=선택 레이아웃 인증 OG `/api/og/hapcard/[id]?layout=&gender=`) ③ **ADR-024 결합=표시범위 라디오(별명만/+오행/+성별)→'성별 표시' 단일 토글**(레이아웃이 노출정보 결정, 성별 옵트인 유지, **생일·시각·출생지 절대 비노출 핵심 불변**). 산출물: `render-payload`(ShareLayout 타입 + buildOgPayload(input,{layout,showGender}) + rangeToLayoutOptions/layoutToShareRange 하위호환) · `OgTemplate` 5 레이아웃(오각형 레이더 polygon·흐름 polyline) · authed OG 라우트 `?layout&gender`+레이아웃별 비-PII 페치(radar=content.area_scores·comment=main_text 첫 문장·flow=snapshots) · `extractShareHeadline`("결론 =" 접두 제거+첫 비어있지않은 줄+40자) · `ShareSheet` 레이아웃 탭+성별 토글+프리뷰 img · `share.tsx` handleShare(layout,showGender,action) · 죽은 share-preview-tile 삭제. **Satori de-risk**: SVG `<text>`/fill-opacity 미지원 가능(PR #98 inline svg 지원되나 text 별도처리) → 레이더 축 라벨 DIV 범례 분리 + opacity→솔리드 색. TDD +49 tests. **§1.2 검증**: ultracode 적대 리뷰(privacy·security·correctness·convention 17 finding → **1 confirmed MED 수정**: extractShareHeadline 선행 개행 시 빈 코멘트 → 첫 비어있지않은 줄 스캔). **2620/2620 PASS**(303 files, baseline 2595 +25), tsc 0, lint 0. 로컬 main 커밋(**origin 미푸시**). **잔여(Phase 2)**: 공개 토큰 OG(링크 수신자) 레이아웃 운반(URL 쿼리 `/h/[token]?layout=` → generateMetadata → 공개 OG, **마이그레이션 불요**) · S-18 상황별 변형. **T3(H-1·H-2·H-4) 완료 — H-3 운기 푸시는 G-1 앱인토스 채널 의존(T5 순연).**
- **T3 origin 푸시 + 브라우저 smoke 완료 ✅ (2026-06-13 7차, Claude Code)** — ① T2+H-1·H-2·H-4 누적 23커밋 `origin/main` push(`451396d..33f62d7`, Vercel 배포). ② `/qa-only` 브라우저 smoke(FlowA1 계정, dev): **H-1 타임라인 PASS**(지나온 사이/인연 등록 렌더) · **pay-per-use 게이트 PASS**(부적 소진→₩1,000 FeaturePaySheet+Toss V2) · **H-2 change API PASS**(status=first). ③ **ISSUE-001 [CRITICAL] 발견·수정·푸시(`e98c6c3`)**: OG 5 레이아웃 전부 500 — Satori "다중 자식 div는 display:flex 필수" 위반(단위테스트가 next/og mock해 미검출, H-4 프리뷰=실제OG 통일이 노출). 다중자식 텍스트→단일 문자열 + DOM 구조 회귀 가드 테스트. ④ **ISSUE-002/003 수정·푸시(`73a3bf6`, §1.1)**: radar `content.area_scores`=phantom 필드(빌더 미생산)→**나 vs 인연 오행 오버레이**(인앱 미니레이더 S-10-A 동일, user_charts+relation_charts fetch) · flow 1점→점 마커. **라이브 검증: 5 레이아웃(minimal/오행/radar 오버레이/코멘트/flow) 전부 200+콘텐츠 렌더 확인.** **2625/2625 PASS**, tsc0, lint0. 리포트 `.gstack/qa-reports/qa-report-t3-retention-smoke-2026-06-13.md`. **교훈**: next/og 라우트는 ImageResponse mock 단위테스트로 Satori 실렌더 못 잡음 — 라이브 렌더/DOM 가드 필수.
- **T3 H-2 케미카드 변화 폭 인디케이터(S-07-A) 완료 ✅ (2026-06-13 5차, Claude Code)** — feat `7067d74`. PRD §5.2 S-07-A 구현(기존 ADR-036 버전 경계 마커는 흐름 탭 타임라인 별도 유지). **§1.1 2결정(2026-06-13)**: ① **요인 라벨=쉬운말 세트**(둘 사이 작용/역할 관계/기운 균형/운의 흐름/관계 유형 — G-5 쉽게보기 철학 일관) ② **비교 불가=안내 문구**(직전 없음 "이번이 첫 해석이에요" / scoring·prompt 버전 다름 "점수 기준이 업데이트됐어요 — 이전 해석과 직접 비교는 어려워요", 변화 자리는 항상 유지). 산출물: `GET /api/hapcards/[id]/change`(케미카드 행=현재 authoritative, 같은 인연·모드 직전 스냅샷과 비교, **ADR-036 버전 게이팅**=scoring_version·prompt_version 둘 다 일치 시만 comparable, `computeChangeScore`/`topChangedFactors` 재사용, 단일 round-trip, **ADR-035 결정형 차분만**, jsonb→ScoreBreakdown `as unknown as` 이중캐스트=코드베이스 허용 패턴) · `HapcardChangeIndicator` 컴포넌트(self-contained useQuery, 5상태 loading/error/first/version_changed/comparable, 메인 delta=온도 `formatTemperatureDelta`[앱 일관성]·요인=원점수 ±N) · HapcardView evidence 탭 1단 배선 · `HapcardChangeStatus/Factor/Response` 타입 · ko.json `hapcard.changeIndicator.*`. TDD +18 tests(route 10·component 6·view 2). **§1.2 검증**: ultracode 다차원 적대적 리뷰(correctness·security·convention 16 finding → 적대 verifier **0 confirmed**). **2595/2595 PASS**(302 files, baseline 2577 +18), tsc 0, lint 0. 로컬 main 커밋(**origin 미푸시**). 잔여: 브라우저 smoke(별도) · **다음 H-4**(공유케미카드 5종 — "5종" 정의를 PRD §6/ADR-024 원문에서 먼저 확인, 모호 시 §1.1).
- **T6(c) 소형 기술부채 4건 완료 ✅ (2026-06-13, Claude Code)** — 로컬 main ff 병합(`5169793..1628ea8`, 4 원자 커밋, **origin 미푸시**). **2638/2638 PASS**(305 files, baseline 2625 +13), tsc0, lint0. **#4 `c44abe5`**: `wallet.ts` LedgerReason 에 relation_slot_use/refund 추가 + `0009` 주석 hapcard_refund 동기(REASON_LABEL 은 이미 11 reason 완비 — 핸드오프 stale, `initial_credit`/`daily_bonus` 는 repo 미존재). **#1 `6a4576a`**: glossary 단일글자 용어(합/형/충/해)만 뒤 비-한글 경계 요구(`evidence.tsx` _MULTI_ALT/_SINGLE 분기) — "충돌·해석·형성·합의" 오매칭 차단 + "부딪힘이"/"자오충" 보존(단일글자+조사는 수용 false-negative). **#2 `e07e7ee`**: `20260613000000_error_events_authenticated_select.sql` owner SELECT 정책(manifest 미수정=별도파일 정책, 0030 관례). **#5 `1628ea8`**: `20260613000100_fk_index_coverage.sql` FK 인덱스 4개(hapcard_shares.relation_id·hapcard_share_rewards.hapcard_id/ledger_id·daily_haps.primary_relation_id, idempotent). **§1.1 결정(2026-06-13)**: #2 SELECT 추가 승인 · #5 인덱스 추가 승인. #3 PAYMENT_REF_NOT_FOUND=무수정(의도된 bare-string) · #6 today latency=§1.1 보류(GPT-5 내재지연 — `model-router.ts` today=gpt-5, 25s timeout+TEMPLATE fallback+phase 계측 이미 적용; A 모델다운/B 스트리밍/C 캐싱 3안 별도 PR+§1.1). **배포 완료 ✅ (사용자 결정=마이그 적용 후 push)**: `pnpm db:push` 0613 2건 라이브 적용(db:push:dry up-to-date PASS) + `git push origin main`(`5169793..1353167`, Vercel 자동 배포).
- **미니앱 Bearer auth `main` 머지 + 프로덕션 LLM 복구 완료 ✅ (2026-06-17, Claude Code)** — ① **머지**: `feature/apps-in-toss-miniapp` → `main` clean fast-forward(`9e091f26..4ded0068`, 3커밋: `60069352` ait devtools+typecheck · `41e8a85` 미니앱 Bearer 토큰 API 인증(`src/lib/supabase/server.ts`) · `4ded0068` dev bearer 툴링) → `origin/main` push, Vercel 프로덕션 자동 배포. 머지 전 풀게이트: tsc 0 · lint 0 · **2831/2831 PASS**(314 files). 프로덕션 API가 미니앱 Bearer 토큰 인증 처리(웹 무영향=Authorization 헤더 없으면 쿠키 경로 유지). ② **프로덕션 LLM 장애 복구**: OpenAI 키 로테이션(새 키 + `proj_N32…` 짝) + Vercel Production env `OPENAI_PROJECT_ID` + `LLM_DAILY_BUDGET_USD=20` 추가 — 둘 다 `VERCEL_ENV=production|preview` 전용 가드(`src/lib/llm/clients.ts`/`budget.ts`)라 로컬 `verify:openai-readiness` PASS여도 prod만 실패·순차로 드러남(메모리 `feedback_vercel_prod_llm_env_gates.md`). 버짓 가드가 OpenAI 호출보다 먼저(`openai.ts`) 실행. 진단=error_events `TODAY_BUILD_FAIL/Missing env` + Vercel 런타임 로그(`[POST /api/hapcards]`). 합카드 라이브 생성 정상 확인. ③ **§사용자 잔여(외부 대시보드, 비차단)**: Vercel `TOSS_ALLOWED_ORIGINS` localhost 제거(prod=tossmini 2오리진만) · `NEXT_PUBLIC_APP_URL=https://todaychemi.vercel.app` 설정(client 번들엔 `VERCEL_PROJECT_PRODUCTION_URL` 미노출이라 명시 필요) · 구 OpenAI 키 `sk-proj-wnKN…` revoke.
- **기운 케어(energy_food/meeting_vibe) Phase 1-8 + ADR-040 Amend 완료 ✅ (2026-06-18, Claude Code)** — 케미카드 보조 콘텐츠: 두 사람 **공통 보완 원소 음식**(energy_food, 전 6모드) + **만남 분위기**(meeting_vibe, 첫합·썸합 추상). 브레인스토밍(레드팀: 실제 장소 §5 삭제·조심/실행 중복 기각·음식 격상)→설계 스펙(`docs/superpowers/specs/2026-06-18-energy-food-date-package-design.md`)→**9-phase TDD**. branch `feature/apps-in-toss-miniapp` 9커밋(`eebb242`~`97fe733`, **origin 미푸시**). 산출물: `src/lib/saju/pair-complement.ts`(합산 최소 원소, yongsin 중화 미러, 1000-run 결정성) · `src/lib/hapcard/element-food-map.ts`(오행→맛→음식 잠금자산) · `src/lib/hapcard/energy-food.ts`(buildEnergyFood 폴백 + validateEnergyFoodCopy 가드) · `output-schema.ts`(EnergyFoodLlmSchema `.optional().catch(undefined)` fail-soft) · `types/hapcard.ts`(EnergyFood/MeetingVibe) · `builder.ts`(systemPrompt 주입+가드+폴백, LlmPayload 미변경) · `energy-care.tsx`(ExpandPanel 탭, convertHanja) · `prompts/system/*.md`(v0.18 active/v0.19 canary). 설계 보정(적대 Plan): ① 음식 선택=서버 결정형, LLM은 copy만 ② **seed-prompts active=canary 본문 공유**→canary 5% 콘텐츠 게이팅 불가, LLM윤문 ~100% 노출(가드+폴백 보호, §1.1 확정) ③ derived fail-open(스케일 혼용 금지). **2871/2871 PASS**(318 files), tsc0, lint0, prompt-version-auditor PASS. **잔여(Phase 9 docs, push 前 §12)**: FGI §17 ADR표/§4.2 · PRD §6 · spec §3.1 정정 · ADR-016 note. **§사용자 수동(deploy)**: `pnpm seed:prompts`(v0.18/v0.19, 캐시 1회 재생성). **검수 대기**: pair-complement 규칙 + element-food-map 자산(명리 specialist, ADR-040 §6.7 잠정). 핸드오프: `session_energy_food_phases1_8_complete_2026_06_18.md`.
- **기운 케어 Phase 9 + 검증 + push 완료 ✅ (2026-06-18 2차, Claude Code)** — ① **Phase 9 §12 doc-sync**(`3c924ccf`): FGI §17 ADR-040 Amended 행 + §3.2 보조 콘텐츠 bullet + ADR-016 additive note + PRD §6.1 + spec §3.1 경로정정(element-food-map `saju/`→`hapcard/`)·§3.2 v0.18 active/v0.19 canary·§8 canary=routing-only + CLAUDE/AGENTS §3 ADR-040 행 동기. ② **§1.2 적대 리뷰**(Ultracode 6차원 workflow `wf_bc7b4c0c`: determinism/PII·ZDR/schema fail-soft/composition·hanja/prompts/doc-sync, 모든 MEDIUM+ refute-by-default) — **0 confirmed 버그**. 검증-검증: prompts FAIL 2건 = 오탐(meeting_vibe 결정형이라 prompt 부재 정상[`builder.ts:365`] · energy_food=classic_citation 컨벤션·fail-soft). 후속 cleanup `16f5095f`(spec §3.1 tie-break=고정 목화토금수만·yongsin 연기 + HapcardView 5탭→6탭 주석 3곳). ③ **§1.1 결정**: B(가드 banned-phrase 불일치)=**문서만 정정**(`bf50c6d4` ADR-040 §4 — 금칙어는 전역 validateLlmText 하드페일 차단, 로컬 가드는 지명·한자만 폴백, 동작 무변경). ④ **/qa+/design-review 브라우저 검증**(Test1@test.com, 393px): 6탭 NOT cramped(327px bar·52px 등폭·overflow 0) · 첫합 energy_food(금/매운맛)+meeting_vibe 렌더 · 돈합 energy_food(목/신맛)+**meeting_vibe 없음**(mode gating live) · classical hanja 0 · console error 0. 리포트 `.gstack/qa-reports/qa-report-energy-food-2026-06-18.md`. ⑤ **origin push 완료**(`0aaca27e..bf50c6d4`, 14커밋, DB 마이그레이션 0, Vercel 프리뷰). **잔여(§사용자 수동)**: `pnpm seed:prompts`(v0.18 active/v0.19 canary, 캐시 1회 재생성) — main 머지·배포 시점. **검수 대기**: pair-complement 규칙 + element-food-map 자산(명리 specialist, ADR-040 §6.7 잠정).
- **기운 케어 배포 + seed + 검수 완료 ✅ (2026-06-18 3차, Claude Code)** — 사용자 3-task 직접 실행. **순서 재정렬**(seed는 코드 배포 後에만 — v0.18 prompt 본문이 energy_food.copy 지시→구 strict 스키마 거부 위험). ① **Task 2 명리 검수**(`912503d7`): `docs/qa/myeongri_review_energy_food_2026_06_18.md` — 五行五味 매핑 黃帝內經 정합 PASS(목신·화쓴·토단·금매운·수짠), 식료·분위기 아키타입 PASS, pair-complement 방향(억부/중화) PASS·합산방식·신강약 잠정(specialist 사인오프 항목 명시), review_status `ai_pending_human` 유지. ② **Task 3 main ff 머지**(`4ded0068..912503d7`, 19커밋, clean ff): 풀게이트 GREEN(**2871/2871**·tsc0·lint0) 후 push → Vercel 프로덕션 배포. **배포 positive 확인**(`vercel inspect`: target=production·Ready·alias `todaychemi.vercel.app`+`git-main`, created=push시각). ③ **Task 1 seed:prompts**(배포 Ready 확인 後): 15행 시드, **MCP 쿼리 검증** — 6모드 v0.18 active/v0.19 canary + daily_hap v0.5 + today_with_relation v0.5/v0.6 확정. ④ **프로덕션 E2E 스모크**: 예시상대1 일합 카드 production 생성(761부적, ~36s, console 0) → energy_food(수/짠맛) 렌더·meeting_vibe 없음(mode gating)·hanja 0 · DB 저장 확인(LLM윤문 copy 영속·신규 optional 스키마 수용=가드한 실패모드 제거 입증). **3-task 전부 완료·프로덕션 라이브.** 잔여: 면허 명리 specialist 인간 사인오프(합산방식·신강약·식료, 외부).
- **앱인토스 미니앱 코드 잔여 작업 라운드 완료 ✅ (2026-06-18 4차, Claude Code)** — 콘솔 미완(인증서만 도착)이라 배포·라이브 검증 불가 상태에서, 배포 없이 안전한 코드 잔여 작업만 수행(§1.1 사용자 확정: 범위=코드 잔여 우선·외부링크=제거·추가항목 전부 포함). branch `feature/apps-in-toss-miniapp` 5 원자 커밋(`cc31df2`~`075a86c`, **origin 미푸시·main 미머지**). ① **케미카드 결제 버그 픽스(P0, `ec56c6d`)**: `miniapp/src/app/pages/HapcardPage.tsx` 자체 webFetch 래퍼가 402 응답에서 `error.payment` 미설정 → 결제 버튼 영구 비활성(미니앱에서 케미카드 결제 불가) → 공용 `apiFetch`(client.ts, .payment 채움)로 통일 + replay-button·ohaeng·timeline·role-analysis·change-indicator 중복 fetch 래퍼 통일. ② **공유 OG 이미지 연동(`2ac0e50`)**: `toss-share.ts` 가 기존 공개 토큰 라우트(`/api/og/share/[token]`)로 og_image_url 발급→`getTossShareLink` 전달, 실패 시 딥링크 폴백. ③ **회사소개 외부링크 제거(`4295f66`)**: deephalabs.com — 앱인토스 외부링크 정책(자사 웹/홍보 랜딩 제한). ④ **vitest 인프라 신규(`cc31df2`)**: miniapp 자체 테스트 0→9(`--ignore-workspace` 격리). ⑤ mTLS 인증서 zip `.gitignore`(`075a86c`). **검증**: miniapp tsc 0 · vitest 9/9 · `ait build`→todaychemi.ait. **§1.2 Ultracode 적대 리뷰(3차원 워크플로우 `wf_a28cf238`)**: 6 confirmed 전부 info/pre-existing, **이 diff 도입 must-fix 0건 = SHIP**. §1.3/§1.1 잔여(별도): MP-1 = HapcardPage 페이월 ₩1,000 하드코딩 vs 서버 오픈할인 ₩500 실청구 불일치(pre-existing·유저유리 방향·가격고지 §1.1 결정 대상) · 미니앱 라이브검증·main 머지·배포·콘솔 등록·IAP SKU·토스 scope·채널톡 = 외부 선행작업 대기. 교훈: [[miniapp-pnpm-ignore-workspace]].
- **앱인토스 미니앱 MP-1 픽스 + main 머지 + 배포 준비 ✅ (2026-06-18 5차, Claude Code)** — §1.1 사용자 확정 3건: 페이월=할인가 표시·feature→main 머지+푸시·정확한 외부 체크리스트. ① **MP-1(`a8b656b`)**: HapcardPage 페이월 ₩1,000 하드코딩 → 서버 제공 실청구액(`payInfo.amount_krw`, 오픈할인 ₩500, 폴백 `IAP_DISPLAY_PRICE_KRW.hapcard`) 표시(replay-button 패턴 일치), 테스트 ₩500 단언 추가. ② **머지+푸시**: `feature/apps-in-toss-miniapp` 8커밋 → `main` ff(`69386c1..fe8db0d`) + `origin/main` push(Vercel 재빌드=동일 백엔드 산출물, 변경 전부 miniapp/·.gitignore·docs). ③ **배포 준비 진단**: 토스 브릿지(`646f43b`)·`toss_connections` 마이그레이션 **이미 main+라이브 적용 완료**(`db:push:dry` up-to-date — 구 "미적용" 메모 stale) · 루트 tsc/lint PASS · launch-readiness 게이트 = **External settings FAIL**(유일 블로커=콘솔/env). ④ **배포 체크리스트** `docs/research/apps_in_toss_deploy_checklist_2026-06-18.md`(`fe8db0d`): Vercel env(mTLS PEM 추출·SKU map·시크릿·NEXT_PUBLIC_APP_URL)·토스 콘솔(앱등록·appName=`todaychemi`·IAP SKU 4종·로그인 scope=user_key·disconnect 콜백)·채널톡 4문항·ait build/deploy·샌드박스 8플로우+IAP 검증. **§사용자 대기**: 콘솔 등록·env 설정(실행=대시보드)·§1.1 IAP SKU 최종가(오픈할인 500/400/300/500 vs 정가) + **코드 대기 1건**: `miniapp/granite.config.ts` brand.icon 빈 값(아이콘 자산 확정 후 반영).
- **앱인토스 미니앱 아이콘 확정 + 등록용 `.ait` 빌드 ✅ (2026-06-18 6차, Claude Code)** — 사용자 확정: 채널톡(C)까지 완료·appName=`todaychemi` 확정·앱 아이콘=`assets/appintoss/twoday_app_logo_600.svg`. ① **brand.icon 설정**: web-framework `defineConfig` icon=string(URL); 공식문서 "콘솔 업로드 이미지 URL". 로컬 SVG를 Next `public/apps-in-toss/twoday_app_logo_600.svg` 로 복사(Vercel 호스팅) → `granite.config.ts brand.icon` = `https://todaychemi.vercel.app/apps-in-toss/twoday_app_logo_600.svg`. ② **등록용 빌드**: `pnpm build` → `miniapp/todaychemi.ait`(3.99MB, deploymentId `019eda12-f8f5-736f-8d7d-d2900c248c91`). **Phase 4.2 검증 PASS**(appName RFC-1123·displayName 한글·icon SET). ③ **GitHub/Vercel 동기화**: granite.config+public 아이콘+docs main push → Vercel 자동 재배포가 아이콘 URL 서빙. ⚠️ icon URL은 Vercel 배포 후 해석 + 미니앱 API/인증/IAP는 **Vercel Toss env(A절: mTLS PEM·TOSS_USER_PASSWORD_SECRET·SKU map·NEXT_PUBLIC_APP_URL)** 설정돼야 동작 — env 설정·`.ait` 콘솔 업로드 = 사용자. **`.ait`는 gitignore(빌드 산출물).** 코드 측 배포 대기 0건.
- **미니앱 온보딩 진입 플로우 수정 완료 ✅ (2026-06-18 8차, Claude Code)** — 새 `.ait` 실기기 테스트에서 "내 프로필 '시작하기' → 온보딩이 아니라 홈으로 튕김" + "로그인 후 프로필 없어도 온보딩 안 뜸" 보고. 근본원인 2건 해소(같은 `feature/miniapp-onboarding-consent` 브랜치 2 원자 커밋 `d68d567`·`e291ad0`, **origin 미푸시·main 미머지**). **Fix A(인증 미성립=튕김 근본원인, `d68d567`)**: `toss-session.ts` `isNativeTossEnv()`가 아무도 set 안 하는 가짜 전역 `__AIT_NATIVE__`를 검사 → 실기기 항상 false → AuthProvider 자동로그인(`appLogin`) 미발화 → token null → `isAuthed=false` → OnboardingPage 가드가 홈으로 튕김. SDK 실 API `getOperationalEnvironment()`(`'toss'|'sandbox'`)+try/catch(브라우저 dev=throw→false)로 교체. 교훈 [[feedback-ait-native-env-detection]]. **Fix B(프로필 강제 게이트, `e291ad0`, §1.1 강제 게이트 확정)**: 신규 공유 훅 `lib/me/use-me-chart.ts`(`['me-chart']` 단일출처, 에러 흡수 안 함=확정null vs 실패 구분, ChartCore 단일 shape) + 신규 `app/ProfileGate.tsx`(탭바 그룹 pathless layout route: loading→LoadingState / 확정 chart=null→`<Navigate to="/onboarding" replace>` / error→fail-open Outlet / chart→Outlet, 온보딩은 게이트 밖→루프 없음) + routes.tsx 배선 + HomePage·MePage가 `useMeChart` 소비 + Step4Review 성공 시 `['me-chart']`(+today/relations) **await invalidate**(게이트 재튕김 방지). **Fix C(부수)**: HomePage 죽은 today-401 리다이렉트(`error.message==='UNAUTHORIZED'` 비교라 미발화) 제거. **검증**: miniapp tsc 0 · vitest **38/38**(+12) · build OK · JWT grep 0건 / root tsc 0 · lint 0. DB·루트 변경 0. **§1.2 검증**: plan 단계 Explore 2 + Plan 1 적대 검토(이중 Outlet·캐시 공유·invalidate 필수·sandbox 분기 5개 정정 반영). **§사용자 수동**: Vercel Toss env 필요(미설정 시 appLogin→`/api/toss/login` 실패→AuthRetryGate 노출) · `.ait` 재빌드+콘솔 재업로드 · 실기기 4케이스 E2E(신규→온보딩 자동/완료→홈 재튕김없음/기존프로필→홈/me 버튼→온보딩). 그룹2(hapcard/whatif/relations) 게이트 미적용=의도(hapcard 자체 `userChartMissing`→/onboarding 방어 보유, whatif first_meet은 chart 불필요).
- **미니앱 온보딩 저장실패(3중 블로커) 수정 완료 ✅ (2026-06-18 7차, Claude Code)** — 실기기 온보딩 "시작하기" → "저장에 실패했어요" 근본원인 3중 블로커 전체 해소. branch `feature/miniapp-onboarding-consent` 4 원자 커밋(`c6f0990` chore vitest exclude · `696f4f0` auth 자동로그인+dev-bearer DEV게이트 · `21092e7` toss consent flow+endpoint · `6a17b9d` 온보딩 동의 UI), **origin 미푸시·main 미머지**(사용자 merge+push 선호). **B1 빌드/보안**: `toss-session.ts`+`AuthProvider.tsx` dev-bearer 를 `import.meta.env.DEV` 게이트 → 프로덕션 `.ait` 미인라인(빌드 후 `grep eyJ dist` 0건 확인). **B2 자동로그인**: `AuthProvider` 마운트 부트스트랩(저장토큰 복원→없으면 `isNativeTossEnv()` 시 `login()` 자동호출, StrictMode ref 가드, 실패 시 `AuthRetryGate`). **B3 법적동의**: `/api/toss/consent`(Bearer getUser→`createClaimedLegalConsentRecord(flow='toss')`) + 마이그레이션 `20260618000000`(legal_consents flow CHECK widening +toss, provider null) + `consent.ts` LEGAL_CONSENT_FLOWS +toss + Step4Review 동의 게이팅 UI(`LegalConsentBlock` 3필수 + consent→onboarding 순차 + 401/403/generic 분기) + `LegalDocSheet`(vaul 바텀시트, 진행상태 보존) + `LegalDocContent` DRY 추출(LegalPage 중복 제거). **DB**: `db:push` 라이브 적용 완료 + `db:push:dry` up-to-date(링크 `jamhkucluhiibqpjsiov`). **검증**: miniapp tsc 0 · vitest **26/26** · build OK · JWT grep 0건 / root tsc 0 · lint 0 · 타깃 테스트 11/11(consent route 6 + migration 5). **§1.2 Ultracode 4차원 적대 리뷰(`wf_5449b6ac`)**: 9 findings → 2 confirmed 즉시 수정(LOW=Step4Review 401 `void login()` 거부 삼킴→`.catch` 복구 폴백+테스트 · INFO=`legal-doc-sheet` `DrawerDescription` 누락→추가), 0 HIGH/MED. **§사용자 수동(외부, 비차단)**: Vercel Toss env(mTLS PEM·TOSS_USER_PASSWORD_SECRET·SKU map·NEXT_PUBLIC_APP_URL) · 수정 반영 위해 `.ait` 재빌드+콘솔 재업로드 필수(현 업로드본=구 토큰) · test1@test.com 비번 회전 권장 · 실기기 8플로우 E2E(자동로그인→동의→온보딩 저장→약관 시트).
- **미니앱 온보딩(consent 7차 + 진입플로우 8차) main 머지 + 프로덕션 배포 완료 ✅ (2026-06-18, Claude Code)** — `feature/miniapp-onboarding-consent` → `origin/main` clean fast-forward `f62f366..6490ff7`(8커밋, Vercel 프로덕션 자동 배포). 위 7차·8차 항목의 "origin 미푸시·main 미머지" 표기는 **본 머지로 해소(stale)**. 마이그레이션 `20260618000000`은 머지 전 이미 라이브 적용 완료라 프로덕션 DB 안전. 웹 백엔드(`/api/toss/consent` 신규 라우트=additive) 즉시 반영. **잔여(실기기 동작 전제, 코드 외)**: Vercel Toss env(mTLS PEM·`TOSS_USER_PASSWORD_SECRET`·SKU map·`NEXT_PUBLIC_APP_URL`) 설정 + `.ait` 재빌드·콘솔 재업로드 = 사용자 수동. 피처 브랜치는 origin 유지(삭제 안 함).
- **미니앱 실기기 로그인 실패 + 온보딩 409 수정 완료 + 배포 ✅ (2026-06-19, Claude Code)** — 9차. 실기기 순차 디버깅(자동로그인→로그인실패→온보딩409)으로 3건 해소. branch `fix/miniapp-login-base-url` 3커밋 → `origin/main` clean ff `b8ee504..0cc9dbd`(Vercel 프로덕션 배포). **① 로그인 실패 근본원인(`de3e8b1`)**: `miniapp/.env.local` `VITE_API_BASE_URL=`(빈값)+`.env.production` 부재 → 프로덕션 `.ait`가 빈 base URL 인라인, `??`가 빈문자 못 거름 → 로그인 POST가 미니앱 자기 오리진(`*.tossmini.com`)으로 가 404. (`VITE_API_BASE_URL`은 로컬 vite 빌드 인라인 — Vercel env 무관. Vercel CLI로 서버 env 전부 설정 확인.) `resolveApiBaseUrl()`(빈값/공백→PROD는 prod 호스트, dev는 상대=프록시)+AuthProvider 공유 → 빌드 산출물에 `todaychemi.vercel.app` 인라인 확인. 교훈 [[feedback-miniapp-vite-build-env]]. **② 로그인 진단 관측성(`de3e8b1`+`026b078`)**: AuthProvider 에러 비삼킴(console.error+게이트 `원인:` 한 줄) + `/api/toss/login` 단계별 `console.error(sanitizeErrorForLog)`(PII 금지) → Vercel 로그로 mTLS/토스API/Supabase 단계 식별. **③ 온보딩 409(`0cc9dbd`)**: `/api/onboarding`의 `users` plain INSERT(PK=user_id 충돌→23505→409 USER_ALREADY_ONBOARDED)를 **`upsert(onConflict:'user_id')`**(user_charts 패턴 미러)로 멱등화 → 재온보딩(로그아웃→/onboarding 재진입·재로그인)도 200. 죽은 23505 분기 제거. **검증**: miniapp tsc0/vitest 43/build OK/JWT0+prod호스트 인라인 / root tsc0/lint0/onboarding 16·login 16. **Supabase Toss SQL**: `db:push:dry` up-to-date — toss_connections·legal_consents toss-flow 등 **전부 라이브 적용, import 할 SQL 없음**(409는 코드 멱등성 문제). ②③은 서버측이라 main 배포로 반영, ①의 클라(base-url)는 사용자가 이미 빌드한 `.ait`에 포함(로그인 동작). **잔여(실기기)**: 동일 `.ait`로 재테스트 → 온보딩 200→홈. (TOSS_ALLOWED_ORIGINS 실오리진 일치는 CORS 이슈 시 점검.)
- **미니앱 광고 SDK 레드팀 리뷰 + 수정 + 가격 문서 정합 완료 ✅ (2026-06-19, Claude Code)** — `/code-review` 레드팀(오늘 배치 `4f481c5..HEAD`: 인앱 광고 배너·무료부적 +50/+5·IAP 가격 인상 1,100/880/880/1,100·`maxDuration=60`) + 앱인토스 MCP(BannerAd 공식 문서 `61f1416628b310bb`) 교차검증. **머니패스·마이그레이션 클린**(무료부적 RPC 멱등·`for update`·`/api/rewards/session` 전체 jsonb 포워딩·feature-prices·cadence `%4`/`%3`·RPC 시그니처·§12 동기화 모두 통과 — 이중과금·무결제유출·재지급 없음). **F1[HIGH] 광고 SDK 컴포넌트별 `TossAds.initialize`(공식 문서 ❌ 안티패턴)** — SDK가 중복 init의 onInitialized를 보장 발화하지 않으면 피드 2번째+ 배너 attach 누락(빈 96px·노출 손실). 수정(`61800b8`): 모듈 싱글톤 1회 init(`ensureTossAdsInitialized`, App 마운트 호출) + 공유 `useTossAdsReady` 구독 + 각 AdBanner는 ready 시 attach. **F2[MED]**: `isAdSlotAvailable()` 가드로 미지원/`VITE_TOSS_AD_GROUP_ID` 미설정 시 빈 `<li aria-label="광고">` 유령 행·이중 gap 제거(Feed/Home). **F3**: 회귀 테스트(첫 init만 onInitialized 발화 mock에서 배너 2개 → `initialize` 1회·`attachBanner` 2회 단언). **운영 광고 그룹 ID(`18053cb`)**: `miniapp/.env.production`(tracked·비시크릿) `VITE_TOSS_AD_GROUP_ID=ait.v2.live.a36156fd5d3c461d` — 빌드타임 인라인(.ait, **Vercel env 아님**), dev는 테스트 ID 폴백. `pnpm build` 후 web 번들 인라인 ×2 검증 + 사용자 `.ait` 콘솔 재업로드 완료. **F4 가격 문서 정합(`0b010ea`)**: replay 사용자 노출 CTA/flow(fluttering Flow B/D·PRD 카드 CTA) ₩880→**₩440**(오픈할인 실청구가) 통일, 정가 ₩880 descriptor는 유지(§1.1 결정 ₩440 통일). branch `fix/miniapp-ad-banner-init` → `origin/main` ff `22e279a..18053cb` + docs `0b010ea`·상태 로그 push(Vercel 자동 배포). miniapp tsc0/vitest **63/63**/`ait build` OK · root tsc0/lint0. **비결함(미수정)**: `maxDuration=60` Hobby 상한 도달(LLM>60s 타임아웃 잔존, 주석 명시) · token_cost 880→9부적 반올림(의도) · `ee72734` 탭바 z-index 100→30. **잔여**: 실 토스앱(5.241.0+) 광고 노출 육안 확인(광고 그룹 구글 등록 최대 2h).
- **미니앱 프로덕션 인증 장애(만료 토큰) 수정 ✅ (2026-06-19, Claude Code)** — 실 토스앱 QA에서 홈·케미카드·프로필 전 메뉴 로딩 실패(모든 인증 라우트 401). **근본 원인 확정**: 미니앱이 만료된 Supabase access token 을 재전송 → 서버 `getUser` → `/auth/v1/user` **403 `bad_jwt: token is expired`**. Supabase auth 로그 정밀분석: 403 전수가 "만료", 서명/프로젝트/anon key 오류 **0건** → 만료는 서명검증 성공 의미 → **설정 정상**(anon key/프로젝트 혼동 배제). 코드: `AuthProvider` 부트스트랩이 저장 토큰을 **만료검사 없이 복원·early-return** + 토큰 있으면 재로그인 안 함 + `refresh_token` 미보관·401 복구 없음 → 만료 토큰 영구 재전송, 자가복구 불가. **부차**: `use-me-chart` 의 `['me-chart']` 가 `enabled` 가드 없어 null-token cold-start가 공유 캐시 오염(`/api/me/chart` 토큰 없이 401 로그 설명). **수정(재로그인 방식, §1.1 확정)**: `miniapp/src/lib/auth/jwt.ts isJwtExpired`(exp 디코드, 네트워크 없음, malformed=만료) · `getToken()` 만료 폐기(dev bearer 면제) · `reauth.ts` single-flight 재인증 브리지 · `apiFetch` 401 시 1회 재로그인·재시도(루프 가드, 402·tokenless 제외) · `AuthProvider performLogin()` 추출+핸들러 등록(네이티브 한정) · `use-me-chart enabled:!!token`. **서버 진단 로그(§1.1 확정)**: `src/lib/supabase/server.ts` Bearer getUser 실패 시 status/code/sanitized 로그(토큰·PII 미로그, additive·웹 무영향). 커밋 `fc44832`(클라+테스트)·`f9e201e`(서버 로그) → `origin/main` ff-merge+push(Vercel: 서버 로그 즉시 반영). miniapp tsc0/vitest **87/87**(+24: jwt·reauth·client 401재시도·getToken 만료·AuthProvider 재로그인·me-chart 가드)/`ait build` OK·JWT 인라인 0 · root tsc0/lint0. (루트 풀스위트 6 실패 = `.env.local`의 `NEXT_PUBLIC_APP_URL=localhost` 로컬 host 아티팩트로 share/legal/og 라우트 테스트가 request-host 대신 localhost 방출 — 본 변경 무관, stash 후 main 코드에서도 동일 실패 확인. CI/clean env 통과.) **§사용자 수동**: `.ait` 재빌드+콘솔 재업로드해야 클라 수정이 실기기 반영(서버 로그는 배포 즉시). **전제**: `/api/toss/login` 프로덕션 동작 시 재로그인 성공(9차 검증); 실패 시 `AuthRetryGate`+신규 서버 로그로 식별 → Vercel Toss env 점검. **dev 위생(별도)**: `VITE_DEV_BEARER`(2026-06-17 만료) 재발급 — 로컬 dev 인증용(프로덕션 무관, .ait 미인라인).
- **웹 OAuth 로그인 장애 수정(카카오 제거 + Google 리다이렉트) ✅ (2026-06-20, Claude Code)** — 웹(배포) 2장애(미니앱 정상). **① 카카오 KOE205** "서비스 설정 오류": KOE205 = **설정 안 한 동의항목(scope) 포함 요청** — Supabase Kakao provider 기본 `account_email` scope가 **카카오 비즈니스(사업자) 인증 앱에만 허용**(개인 개발자 앱 실패). Kakao→Google 마이그레이션(S-00) 후 카카오 버튼이 로그인/회원가입에 **활성 잔존**. **§1.1 결정(2026-06-20): 카카오 로그인 제거**(카카오톡 **공유**는 유지). 코드: `src/lib/auth/kakao.ts`(+`tests/auth/kakao.test.ts`) 삭제 · `login-client`/`signup-client` 카카오 버튼·핸들러·import 제거 · `src/app/auth/oauth/route.ts parseProvider` google-only 하드닝(직접 `?provider=kakao`도 차단) · `messages/ko.json kakaoButton` 제거 · login/signup page test 동기. **② Google→localhost(다른 프로젝트) 리다이렉트**: 코드 `redirectTo`는 **요청 origin**(`auth/oauth/route.ts`)으로 정상 — 근본원인은 **Supabase가 redirectTo를 Redirect URLs 허용목록에서 못 찾으면 Site URL로 폴백**(현재 Site URL=localhost 또는 비정규 프로젝트). **코드 변경 불가 = 외부 대시보드**: Supabase `jamhkucluhiibqpjsiov` Auth → URL Configuration Site URL=`https://todaychemi.vercel.app` + Redirect URLs `…/auth/callback` · Vercel prod env(`NEXT_PUBLIC_SUPABASE_URL`/`ANON_KEY`)가 정규 ref 확인(`aseszttxkxpfzenmbylx` 등 아님 — "다른 프로젝트" 증상) · Google provider/Cloud OAuth 콜백=`…supabase.co/auth/v1/callback`. 문서 동기(§12): `auth.md`(Kakao Removed)·`kakao_oauth_share.md`·`external_settings_checklist.md`. **root tsc0/lint0 · 영향 auth 테스트 28/28.** branch `fix/web-oauth-kakao-removal` → `origin/main` ff+push(Vercel 자동배포=카카오 버튼 제거 즉시 반영). **§사용자 수동(외부)**: 위 Supabase Dashboard + Vercel env + Supabase Kakao provider Disabled — 적용 후 Google 리다이렉트 정상화. 런북 `docs/runbooks/google_oauth.md`.
- **미니앱 UI/UX 개선(테마 토글·글자 크기·CTA 효과·디자인 폴리시) 완료 ✅ (2026-06-21, Claude Code)** — 실 `.ait` 토스 콘솔 QA 피드백 4건. branch `feature/miniapp-ui-prefs-polish`(origin 미푸시·main 미머지). **§1.1 4결정**: 글자 크기=전체 UI 확대(zoom)·디자인=직접 개선+`/design-review`+`/ait:design` 진단·버튼 효과=주요 CTA만·스타일=흐르는 그라데이션 테두리+글로우. **① 홈 우상단 인연등록 링크 → 라이트/다크 테마 토글**(중복 제거; 홈 인연등록은 중앙 빠른카드·0건 hero 유지, 피드 인연등록 유지). 신규 `miniapp/src/lib/preferences/`(storage 플랫 localStorage·apply·use-preferences zustand 단일출처) + `index.html` pre-paint 인라인 스크립트(FOUC 방지, matchMedia 는 try/catch 밖이라 항상 data-theme 설정) + `tokens.css` `@media(prefers-color-scheme:dark)` → **`:root[data-theme="dark"]` 단일 블록**(시스템 해석은 JS 가 data-theme 으로 환원, 중복 0). 2단계 토글(라이트↔다크, 첫 로드 OS 추종 후 명시 저장). **② 프로필 글자 크기(보통/크게)** — InfoCard `글자 크기` 행 + `font-size-sheet` vaul 바텀시트(라이브 미리보기·활성 ✓) + `:root[data-font-scale="large"]{zoom:1.12}`(px 고정 인라인 구조라 토큰 스케일 불가 → 루트 zoom 으로 전체 UI 확대, fixed 탭바·body 포털 좌표 유지). **③ 주요 CTA 그라데이션 테두리+글로우** — `tokens.css` `@keyframes cta-border-flow`+`.btn-cta`(mask 2px 링·`--p-60/--p-30/--lg-3` 그라데이션·`box-shadow` 글로우·`prefers-reduced-motion` 정적 폴백) → 8개 핵심 CTA(홈 빠른카드·피드 인연추가·온보딩 CtaBar·인연등록 제출/결제·다시맞추기 트리거/결제/확인·또다른나·빈상태·보상확인) 적용, 보조/닫기/취소/필터/아웃라인 제외. **④ AIT 제약 점검**(`/ait:design`=Figma↔제약 진단·스토어 자산 생성 도구라 인앱 스타일 비대상 — §1.1 확인 후 직접 점검): 테마 토글 탭 타깃 40→44pt 상향. **검증**: miniapp `tsc --noEmit` 0 · vitest **106/106**(신규 preferences store·theme-toggle·today-app-bar·font-size-sheet·info-card 6행·CTA 클래스) · `vite build` OK(인라인 스크립트·`[data-theme=dark]`·`zoom:1.12`·`cta-border-flow`·mask 룰 번들 보존) · **playwright 실Chromium 시각검증**(다크 `#fbf8f3→#14151c`·zoom 1.12·그라데이션 링/글로우 렌더·보조버튼 무효과, 라이트/다크 × 보통/큰글씨 스크린샷). **DB·루트 `src/` 변경 0**(루트 eslint 가 `miniapp/**` ignore — 미니앱 별도 lint 없음, tsc 가 타입 게이트). `setup.ts` jsdom Pointer Capture 폴리필 추가(vaul 시트 인터랙션 테스트). **§1.3(함께 수정 ✅, §1.1 승인)**: `LoadingState`/`hapcard/loading-state` 가 정의 없는 `pulse`/`spin` keyframes 를 참조해 로더 애니메이션이 죽어 있던 기존 버그 → `tokens.css` 에 `@keyframes pulse/spin` 정의(본 PR 이 최초 keyframes 도입). **§사용자 수동**: `.ait` 재빌드+콘솔 재업로드(기기 반영) · 인증 화면(홈/나) 전체 `/design-review` 는 `VITE_DEV_BEARER`(만료) 재발급 후 또는 온디바이스.
- **미니앱 디자인 정렬 Phase 1(리퀴드글래스 히어로) 완료 + main 배포 ✅ (2026-06-21, Claude Code)** — UIDesign(토스×iOS26×M3) 톤앤매너 정렬 멀티세션 작업의 Phase 1(파운데이션 + 히어로 3종). branch `feature/miniapp-design-alignment` → `origin/main` clean ff `2edb472..72848e8`(Phase 0 `600ebad` 포함 6커밋, Vercel 재빌드, 웹 무영향). 5 원자 커밋(매 커밋 게이트): `227dc7f`(Pretendard Variable 동적서브셋 CDN `index.html`+`--font-display` 'Pretendard Variable' 바인딩 — 시그니처 거대숫자 폰트가 system-ui 폴백이던 실버그 해소) · `79ca852`(`tokens.css .liquid` UIDesign 패리티: `::before` transparent 50%·`::after` 하단 rim 추가·`.liquid > * { position:relative; z-index:1 }` — whatif 흰 텍스트가 `::after` 위로) · `7f854ac`(today-hero→`className="liquid"`, 인라인 4-stop·수동 글로스 span 제거, 거대 온도숫자·폴백 헤드라인 `--font-display`) · `dd19881`(HapcardPage 히어로→`.liquid`, borderRadius 20→`--r-xl`, 402 결제흐름 테스트 명시 통과) · `72848e8`(whatif-hero 회색카드→`.liquid cool`, 텍스트 흰색 반전·`AiDisclosureBadge tone="dark"`, 긴 본문 대비 위해 cool 톤). miniapp tsc0/vitest **106/106**/`vite build` OK. **라이브 검증(test1@test.com, `pnpm mint:dev-bearer --write`, 백엔드 3001 재사용+vite 5173)**: 자동로그인✓·**Pretendard Variable 실로드✓**(`document.fonts.check('900 72px "Pretendard Variable"')`=true)·**today 리퀴드 히어로 라이브 렌더✓**(4-stop 보라→마젠타 그라데이션+우상단 글로스+흰텍스트). 합카드·또다른나 본문 히어로는 유료 생성 필요라 라이브 미렌더(동일 `.liquid` 클래스·메커니즘 today로 입증, 합카드=기본 톤·또다른나=cool). **§1.1 확정(2026-06-21)**: me-hero=풀 Dawn 워터컬러 별도 세션·Pretendard 지금 도입·검증=코드 먼저. **다음 세션 = me-hero Dawn**(플랜 `~/.claude/plans/phase-1-fluttering-knuth.md` Deferred: 한↔영 오행 키 불일치·`.dark`→`:root[data-theme="dark"]` 재작성·다크 그라데이션 공백·WebView backdrop-blur·신규 `--el-*`/dawn keyframes·`DawnHeroBg` 순수 SVG). 잔여 Phase 2-7(CTA pill·elevation+rgba 버그·레이더·iOS 휠피커·컴포넌트 통합). §사용자 수동: `.ait` 재빌드·재업로드(기기 반영). dev: `VITE_DEV_BEARER` = `.env.local`에 fresh 주입(만료 ~1h, `.env.development.local` stale 라인 비움).
- **미니앱 디자인 정렬 Phase 1 Deferred — me-hero Dawn 워터컬러 완료 + main 배포 ✅ (2026-06-21, Claude Code)** — 내 사주(me) 히어로를 UIDesign Dawn 타깃(`UIDesign/SAJU-handoff/.../me-hero-dawn`)으로 풀 패리티 이식. **§1.1 확정**: Q1 상단=풀 패리티(아이브로우+일주 타이틀+닉네임·생일+인히어로 편집 버튼, 하단 `MeEditRow` 중복 제거) · Q2 하단 카드 설명줄=전체 성향 문장(아래 `DayMasterCard`와 중복 수용). branch `feature/me-hero-dawn` → `origin/main` clean ff `2366251..07712b0`(3 원자 커밋, Vercel 재빌드, 웹 무영향). 커밋: `876d629`(tokens.css: `--accent-*-soft`+`--text-tertiary`+`--dawn-*`[워시/하늘/카드 라이트+다크]·`.dawn-grain`[다크 screen 블렌드]·`dawn-breathe/drift-1/drift-2` keyframes+reduced-motion 가드) · `37e2b02`(`DawnHeroBg` 데코 컴포넌트 — 인라인 SVG·`--accent-*` 5잉크블롭·새벽하늘·grain·6 TDD) · `07712b0`(me-hero 풀 패리티 재작성+신규 `useMeProfile` 훅[`['me-profile']` 리프트, 닉네임/생일 표시]+MePage 배선+`MeEditRow` 제거+7 TDD). miniapp tsc0/vitest **106→119**/`vite build` OK, 루트 `src/` diff 0. **확정 설계**: `--accent-*` 재사용(`--el-*` 미도입, 값 동일=한개념한용어) · **한글 키 tone 맵**(레퍼런스 영문 키 wood-폴백 버그 수정) · 다크 dawn 토큰 신설(레퍼런스 없음) · convertHanja 안전망(ADR-038) · 닉네임/생일 UI 표시만(LLM 미포함 §5). **라이브 `/browse` 검증(Test1@test.com, 393px)**: 라이트/다크 양쪽 Dawn 충실 렌더·프로스티드 카드 가독(backdrop-blur+솔리드 폴백)·`data-theme` 전환·원소 tone 정확(토/戊→earth-soft)·MeEditRow 제거 확인. §사용자 수동: `.ait` 재빌드·재업로드(기기 반영). §1.3 잔여: `me.editRow.*` dead i18n 키(무해) · `/design-review` UIDesign 패리티+다크 dawn 토큰값 튜닝(별도). **다음**: 마스터 플랜(`~/.claude/plans/cheerful-juggling-scroll.md`) Phase 2-7(CTA pill·elevation+RelationDetail rgba 버그·오행 레이더·iOS 휠피커·컴포넌트 통합).
- **미니앱 디자인 정렬 Phase 2+3 완료 ✅ (2026-06-21, Claude Code)** — 마스터 플랜 Phase 2-3. branch `feature/miniapp-design-phase2-7`→`origin/main` clean ff **`02c1f85..6dc2ba3`**(8커밋, Vercel 자동배포, 웹 무영향). miniapp tsc0·vitest **119/119**·`vite build` OK·루트 `src/` 0. **Phase 2 CTA pill 통일**(`507a3ad`): 주요/결제 CTA→Button `size="cta"` 풀폭 pill, 3 페이월(Step3ModeConsent·replay-button·HapcardPage[raw `<button>`→Button]) 결제행 side-by-side→**스택(pill 결제 + ghost 닫기)**, `.btn-cta` 글로우 규율(풀사이즈 프라이머리만, 홈 빠른카드·Feed 헤더 제거). **Phase 3 rgba 버그**(`597dcdf`): `rgba(var(--token),α)`(hex라 무효)→`color-mix`(RelationDetail 앱바·Home intro)+`-webkit-backdrop-filter`. **Phase 3 me 카드 통일**(`7da863b`): 신규 `me/section-card.tsx`(bg-card+r-lg+hairline+e-1+eyebrow)→PillarGrid·DayMasterCard·OhaengBars wrap·YunseCard, talisman 그라데이션 유지. **Phase 3 today 톤**(`bcdc2eb`): avoid=warn/favorable=ok 시맨틱+e-1. **Phase 3 elevation**(`b870145`): `.card-elevated` 보더 outline-variant→**hairline**+Feed 행/whatif 적용. **fix disabled affordance**(`d313549`): Button 프리미티브 disabled 무표시 회귀(결제 CTA 글로우만 빛나고 탭 무시)→전역 `.btn:disabled` opacity/cursor+`.btn-cta` disabled 글로우 제거(리뷰 `wf_36258f7c-717` 확정 MEDIUM, 3 페이월 동시 수정). §1.2: 적대적 4차원 리뷰(regression·tokens·darkmode·a11y)→확정 1건 수정·나머지 clean. **라이브 before/after 미실행**(VITE_DEV_BEARER 부재 — tsc/test/build+리뷰로 대체, 사용자 §1.1 지금 push 확정). **다음=Phase 4(오행 레이더, "큰 빌드") 새 세션**. 핸드오프 `session_miniapp_design_phase2_3_complete_2026_06_21.md`. (rule §1.7 한국어 보고 등록 `7ad197c`.)
- **미니앱 디자인 정렬 Phase 4(데이터 비주얼) 완료 ✅ (2026-06-21, Claude Code)** — 마스터 플랜 Phase 4. branch `feature/miniapp-design-phase4-dataviz`→**`origin/main` clean ff `3902605..94e9c6a`(6 코드+docs, Vercel 자동배포, 웹 무영향, §1.1 push 확정 2026-06-21)**. miniapp tsc0·vitest **145/145**·`vite build` OK·루트 `src/` 0. **§1.1 3결정(2026-06-21)**: A1 레이더 칩=중립 라벨 2칩('가장 강한/약한 기운'=최다/최소 카운트, 희신 라벨 회피=ADR-018 모트, 레퍼런스 한자 칩 미포팅=ADR-038) · B1 합카드=오버레이 레이더 추가+비교 바 유지 · C1 공용 바=수평 fill만 통일. 5 원자 커밋: **C1 `ba67fbf`**(공용 `ui/bar.tsx` 수평 fill 프리미티브 — value/max 클램프·max≤0 가드·anchor end=marginLeft auto·ariaLabel opt-in role=progressbar; HapcardPage area_scores + ohaeng.tsx ComparisonBar 2호출부 리팩터, 세로/SVG/진행바는 범위 외) · **A1 `1b3cbd9`**(`me/ohaeng-radar.tsx` 5축 펜타곤 레이더+인사이트 2칩, `--el-*`→`--accent-*` 매핑, 한글 라벨, SectionCard 래핑; MePage OhaengBars 교체+고아 `ohaeng-bars.tsx` 삭제; i18n `me.section.ohaeng.strongest/weakest`) · **B1 `c828243`**(`hapcard/mini-radar.tsx`+`primitives/mini-radar.tsx` user-vs-relation 오버레이 레이더 2폴리곤, 고아 `hapcard.miniRadar` i18n 채택, `hsl(var())`→`var()`·grid `--hairline`·color-mix 범례; HapcardOhaeng 상단 추가=ADR-016 섹션 잠금 불변[섹션 내 additive]) · **`a66e301`**(리뷰 LOW fix: Bar 기본 트랙 `--surface-1`→`--hairline` — 다크에서 `--surface-1`≈`--bg-card` 1.02:1 비가시, 커밋 "dark-safe" 주장 미성립이라 정정) · **`eef17ae`**(리뷰 INFO: `vertex()` 삼각함수 중복 단일화 → `lib/hapcard/radar-geometry.ts:radarVertex`, 두 레이더 공유+기하 단위테스트). **§1.2 검증**: 적대적 5차원 Workflow 리뷰(`wf_66e410c2`, 10 agents: regression·correctness·tokens·adr·convention → refute-by-default verifier) → **5건 제기·2 confirmed(LOW track·INFO geometry, 둘 다 수정)·3 refuted**(라이트 트랙 대비=의도 트레이드오프·Math.round 제거=동등). **라이브 `/browse` 검증(Test1@test.com, 393px, HashRouter `#/me`)**: me 5축 레이더 라이브 렌더✓(4 폴리곤·칩 화·3/목·0·한자0·SectionCard 일관). **합카드 오버레이 레이더는 라이브 미검증**(로컬 dev 백엔드 `OPENAI_API_KEY` 부재로 신규 생성 500·캐시 카드 없음 — Phase 4 무관, 단위테스트 6건+me 레이더로 입증된 동일 SVG 메커니즘으로 대체, 온디바이스 QA 이연). §사용자 수동: `.ait` 재빌드·재업로드(기기 반영). **다음=Phase 5(iOS 휠 피커) 새 세션**. 핸드오프 `session_miniapp_design_phase4_complete_2026_06_21.md`.
- **미니앱 디자인 정렬 Phase 5(iOS 휠 피커) 완료 ✅ (2026-06-21, Claude Code)** — 마스터 플랜 Phase 5. branch `feature/miniapp-design-phase5-wheelpicker`→**`origin/main` clean ff `cae1ef1..716094b`(11 코드+docs, Vercel 자동배포, 웹 무영향, §1.1 push 확정 2026-06-21)**. miniapp tsc0·vitest **186/186**(145 +41 신규)·`vite build` OK·루트 `src/` 0. **§1.1 2결정(2026-06-21)**: 패턴=필드 탭→바텀시트 트레이(인라인 휠 아님) · 범위=4 surface 전부(me-edit 포함 — 코드에 `TODO(P5)` 마커 존재). 네이티브 `<input type=date/time>`을 iOS 휠/트레이로 교체. **불변 보존**: 출력 `YYYY-MM-DD`/`HH:MM`·draft-store/폼 계약·min/max·일자 검증 동일. 6 feat 커밋: **`25febb0`**(`lib/datetime/wheel-options.ts` 순수 로직 — 옵션·daysInMonth·parse/format·clampDay·clampDateToBounds·pad2) · **`4864b19`**(프리미티브 `ui/wheel-column.tsx` scroll-snap 40px 휠 + `ui/wheel-tray.tsx` createPortal 바텀시트[vaul 아님=me-edit 중첩 회피] + `ui/picker-field.tsx` 탭 필드; tokens.css `.wheel-col` 스크롤바 숨김) · **`62262e5`**(`ui/date-wheel-field.tsx` 연/월/일 + `ui/time-wheel-field.tsx` 시/분, cancel 폐기·done 커밋·중앙 밴드) · **`7f57162`**(온보딩 Step1 date+Step2 time + i18n placeholder) · **`c45e1ef`**(인연등록 Step2DobTime) · **`61a01dc`**(me-edit-drawer, TODO(P5) 해소, [1900,오늘] 경계). **§1.2 적대적 5차원 리뷰**(`wf_f2dc2e9c`, 15 agents: invariant·correctness·a11y·portal·convention → refute-by-default) → **10건 제기·8 confirmed·2 refuted**. 4 fix 커밋: **`8dde2f2`**([HIGH portal] WheelTray가 vaul modal 안에서 `pointer-events:none` 상속→트레이 죽음; backdrop·panel `pointerEvents:auto` — **라이브 JS-click smoke가 놓침, 합성 클릭은 pointer-events 우회**) · **`68a2f5a`**([a11y MED×2+LOW] 휠 키보드[Arrow/Home/End+activedescendant]·트레이 aria-modal+Escape+포커스관리/트랩·PickerField aria-haspopup/expanded) · **`9e9c57b`**([correctness LOW] initialDraft 경계 미보정→범위밖 저장값이 휠 선택과 불일치; clampDateToBounds 적용) · **`65175fd`**([LOW×2] 온보딩 time 라벨 `birth.time`→`timeOptional` 일치·stale 헤더 주석 3개 갱신). INFO(스크롤 jank)=수정 불요(온디바이스). refuted: 드로어 닫힘=React 트리 정상·me-edit 경계=의도. **라이브 `/browse` 재검증(실제 마우스 클릭)**: 온보딩 date 휠✓·**me-edit 중첩 트레이 F1 수정 실포인터 검증✓**(년 옵션 1900 실클릭→aria-selected 이동→완료→필드 "1900.05.10" 갱신+닫힘, 저장 미클릭=DB무변경)·F3 aria-modal·F4 expanded 라이브✓·time 휠 시/분✓. §사용자 수동: `.ait` 재빌드·재업로드(기기 반영). **다음=Phase 6(컴포넌트 통합: Seg·back·탭바 pill·Dialog 일원화) 새 세션**. 핸드오프 `session_miniapp_design_phase5_complete_2026_06_21.md`.
- **미니앱 디자인 정렬 Phase 6(컴포넌트 통합) 부분 완료 ✅ (2026-06-21, Claude Code)** — 마스터 플랜 Phase 6 드리프트 제거. **`main` 직접 6커밋(origin 미푸시)**, 게이트 매 커밋 GREEN: tsc0·vitest **216/216**(baseline 186 +30)·`vite build` OK·루트 `src/` 0. **§1.1 3결정(A)**: Seg=한 `Seg<T>`(variant segment/fill, UIDesign .seg/.itabs 위해 `accent` 모디파이어 추가) · 확인창=센터 `ConfirmDialog`(reward-popup 선례) · 페이월=표현용 `FeaturePayCard`만 추출(머니패스 무변경). 커밋: `0a8cc8f`(탭바 M3 active pill — AppNav `.ic` 캡슐 `--p-90`/`--p-10`/700) · `b4f764a`(`ui/back-button.tsx` BackButton lucide ChevronLeft·44pt·aria '뒤로', StepHeader×2+RelationDetail/Hapcard/Legal 마이그, LegalPage 비중앙 글리프 버그 해소) · `f63f5ce`(`ui/seg.tsx` 프리미티브 — role radiogroup|tablist·aria-checked|selected·로빙 tabindex·Arrow/Home/End) · `5ccae37`(Seg 마이그: Feed 필터·Hapcard 탭 segment, 온보딩/인연/me-edit 토글 fill, me-edit accuracy native-radio→pill 정규화) · `a4b7e1b`(`ui/confirm-dialog.tsx`) · `7e04f1b`(홈/피드/Hapcard 삭제확인→ConfirmDialog, destructive variant 통일). **잔여(다음 세션)**: Batch D 나머지(홈 인트로 팝업·Hapcard rename 폼→Dialog, Hapcard `⋯` 메뉴→Drawer 액션시트) · **Batch E FeaturePayCard**(hapcard/whatif/replay/relation_slot 4 페이월, ADR-039 머니패스 §1.2 /qa 필수, replay `?replay=1`·payDismissed 흐름 보존) · Phase 7(적대 리뷰+/browse before/after+baseline→final 델타) · §1.1 push 확정 후 origin ff-push. 핸드오프 `session_miniapp_design_phase6_partial_handoff_2026_06_21.md`.
- **미니앱 디자인 Phase 6 Batch D/E + 트랩 픽스 완료 ✅ (2026-06-22, Claude Code)** — Phase 6 컴포넌트 통합 마감. `main` 5커밋 → `origin/main` **clean ff-push `00d80c4..0ac1de5`**(Phase 6 전체 12커밋 첫 푸시, Vercel 미니앱 리빌드, 웹 무영향). **230/230 PASS**(baseline 216 +14), tsc0, build OK. **Batch D**: 홈 인트로 팝업→센터 Dialog(`1cb0b3d`) · 케미카드 이름변경 폼→Dialog + ⋯메뉴→바텀 Drawer 액션시트(`29bc664`, +`hapcard.menu.title/description/close` i18n). **Batch E(ADR-039 머니패스)**: 공용 `FeaturePayCard`(`8a1c048`, +12 TDD — "동의 전 결제버튼 비활성" 전자상거래법§17 게이트 4중복→단일출처, 표현용·controlled·머니패스 무변경) → 4 페이월 마이그(`2481872`, whatif `PayRequiredBlock` 삭제+`refundConsent` 페이지 리프트·raw버튼→`Button` 정규화·`payDismissed`/`?replay=1`/`handleRetry` 보존). **🔴 트랩 근본해결(`0ac1de5`)**: ⋯메뉴(vaul Drawer)가 닫히는 ~500ms `body{pointer-events:none}` 동안 base-ui Dialog(열린 Popup/Backdrop에 pointer-events 미명시, 상속 속성)가 죽는 Drawer→Dialog 전환 트랩(Phase 5 동일 계열) → `dialog.tsx` `pointerEvents:auto` + 스타일 단언 가드 2건([[feedback-synthetic-click-bypasses-pointer-events]]). **검증**: 적대 Workflow(money-path·convention 차원 **0 confirmed** 5제기·전부 refute; design/regression/a11y 차원 세션한도 미완→직접리뷰 클린, INFO 2건 whatif 중앙→좌측정렬·replay role=alert 소실=수용) + 라이브 `/browse`(홈 인트로 Dialog **실포인터 클릭 닫힘 입증** + Feed Seg radiogroup 렌더). 케미카드 본화면(⋯메뉴/페이월)은 로컬 LLM 게이트 500으로 미도달 → 온디바이스 QA 이연(트랩 자체는 가드 완료). dev-bearer는 `import.meta.env.DEV` 게이트(내 diff 무관, `ait build` 0건). **§사용자 수동**: `.ait` 재빌드·재업로드(기기 반영).

---

## 3. 단일 핵심 (ADR 잠금)

본 제품의 단일 핵심 피처는 다음 두 가지이며, 모든 화면·KPI가 이쪽으로 유입되도록 설계되어 있다. 이 위계를 흔드는 제안은 §1.1 사용자 승인 대상.

| 핵심 | 위치 | ADR |
|---|---|---|
| §4.2 관계 사주 해석 (케미카드 정가 ₩1,100/11부적, 현금 550) | `fluttering-gathering-island.md` §4.2 / `PRD.md` §6 | ADR-010, ADR-016, ADR-026 |
| §4.3 관계 진화 타임라인 재해석 (정가 ₩880/9부적, 현금 440) | 같은 문서 §4.3 (Phase 1.5) | ADR-033 |

### 비협상 ADR (변경 시 §1.1 승인 필수)

- **ADR-002** 자유채팅 미제공 — 모든 LLM 결과는 구조화 카드/리포트 안에서만
- **ADR-010** 단일 핵심 위계 — 보조 콘텐츠가 핵심을 가리지 않음
- **ADR-011** 별명만, 실명 수집 금지 — DB 컬럼·UI 라벨 모두 "별명"
- **ADR-015** 재해석 시 명리 근거 항상 표시
- **ADR-016** 결과 카드 6 컴포넌트 Phase 1 잠금
- **ADR-018** 모트 = 명리 정확성 자산 (KASI Agreement + 다중 검증 + 고전 RAG)
- **ADR-035** 점수 결정형 — LLM은 점수 산출에 개입 금지 (`compatibility_scoring_spec.md` 참조)
- **ADR-037** 기술 스택 잠금 (`tech_stack.md` 참조)
- **ADR-038** Hanja 노출 금지 — UI display layer에서 한자 제거. RAG/DB verbatim 유지. `convertHanja()` safety-net 의무.
- **ADR-039** Pay-per-use 결제 — 부적 충전 폐지, 유료 기능 사용 시 즉시 결제. 하이브리드(무료 부적 우선→부족 시 현금)·가격 정가 1,100/880/880(현금 550/440/440, 부적 11/9/9p, 2026-06-19 개정 — 웹·미니앱 통일) 단일출처(`feature-prices.ts`)·원자성 모델 C(선생성→성공 시 결제)·잠금 단일진실 `isFeatureUnlocked`(쓰기+read-path 본문 라우트 모두 게이트). **Amended 2026-06-10 §9**: 인연 등록 슬롯(모델 B) — 2명까지 무료, 3번째부터 `relation_slot` 정가 1,100원/11부적(현금 550), 현재 보유 수 게이트(삭제 시 회복), draft 스테이징→머티리얼라이즈(claim-first 멱등+lazy recovery), cash-gen 한도 미적용. (`docs/adr/ADR-039-pay-per-use-billing.md`)
- **ADR-040** 파생·교차분석층 = LLM 해석 근거 전용, 점수 무개입 — `chart_core.derived`(theory v3) + `cross_analysis`(cross-v1, 비영속). 순수 결정형(1000회 테스트 의무) + 프롬프트 환각 가드("제공 필드 외 단정 금지") + PII 연령차 밴드만. 신강약·용신 룰은 specialist 검수 전 잠정(`manseryeok_theory.md` §6.7). **Amended 2026-06-18 (기운 케어)**: 케미카드 보조 추천층 `energy_food`(전 6모드 공통 보완 원소 음식) + `meeting_vibe`(첫/썸 만남 분위기) — 결정형 파생(`saju/pair-complement.ts` + `hapcard/element-food-map.ts`, 점수 무개입), 음식 선택은 서버·LLM은 `energy_food.copy` 한 문장만 윤문(이름 제약 가드→결정형 폴백), UI = ADR-016 잠금 1~6 위 additive "기운 케어" 탭, seed-prompts active=canary 동본문(콘텐츠 게이팅 불가). (`docs/adr/ADR-040-derivation-cross-analysis-layer.md`)

---

## 4. 기술 스택 (ADR-037 잠금)

상세는 `tech_stack.md` 참조. 핵심 잠금만 요약:

- **Frontend**: Next.js 15 App Router + TypeScript strict + Tailwind + shadcn/ui + Radix
- **State**: TanStack Query v5 (서버) + Zustand (UI)
- **Backend**: Next.js Route Handlers (별도 서버 X) + Supabase Free (Postgres + Auth + RLS + Storage)
  - **Canonical Supabase project_ref**: `jamhkucluhiibqpjsiov` (`goonghap`, Northeast Asia / Seoul). 다른 ref(예: `aseszttxkxpfzenmbylx`, `muuudarddkvevwdpefvy`)는 작업용 아님. 링크 확인: `pnpm db:status`. 재링크: `pnpm db:link`. push: `pnpm db:push:dry` → `pnpm db:push`.
- **i18n**: next-intl (KO 1차, EN/VI/TH/MS/ID Phase별)
- **만세력**: ssaju (年/月/時柱 절기·입춘 기준 source + day_pillar cross-validator) + KASI (day_pillar 진본) + manseryeok-js (보조 cross-validator) — 2026-05-03 §1.1: ssaju 역할 年/月/時柱 프로덕션 source로 확대. 야자시 = 조자시 통합 학파 (ADR-037)
- **사주 엔진**: 자체 TypeScript `fortune-core` (monorepo 패키지) — 결정형
- **LLM**: OpenAI 4단 (GPT-5 핵심 / GPT-5 딥합 / GPT-5 mini 오늘합 / Anthropic Claude fallback)
- **결제**: 토스페이먼츠 (KR Phase 1) / Stripe (Phase 3 SEA)
- **Hosting**: Vercel Hobby (Phase 3 진입 전 Cloudflare 전환 재검토)
- **Tests**: Vitest + Playwright + Zod

스택 변경은 §1.1 사용자 승인 대상이며, 승인 시 `tech_stack.md` + `fluttering-gathering-island.md` + `PRD.md` 동시 갱신 의무 (§12).

---

## 5. PII / ZDR 절대 규칙 (협상 불가)

OpenAI / Claude 등 외부 LLM에 **절대 보내지 않는** 필드:
- `birth_date` (원본)
- `name`, `nickname`
- `email`
- `birth_place`
- `gender` (원본)

LLM 페이로드에 허용되는 것은 **`chart_core` + `question_slot` + `theory_profile.profile_version`** 뿐이다. (출처: `tech_stack.md` §3.5, `FRONTEND-PREP.md` §11.2)

OpenAI는 **ZDR (Zero Data Retention)** 계약 적용 필수. 이 규칙을 우회하는 코드 작성 시 즉시 중단하고 사용자에게 보고.

---

## 6. 명령어

### 6.1 현재 (스캐폴드 미생성)
- 빌드/테스트 명령어 없음. `package.json` 미존재.
- 스펙 변경만 가능. 변경 시 §12 변경 매트릭스 적용.

### 6.2 스캐폴드 생성 후 (Phase 0 G0 이후, 사용자 승인 필요)
다음은 `FRONTEND-PREP.md` §16 작업 순서 기준 *예정* 명령어. 실제 실행 전 §1.1 적용.

```bash
# 초기 스캐폴드 (1회)
pnpm create next-app --typescript

# shadcn 컴포넌트 (필요할 때마다 add)
pnpm dlx shadcn@latest add button badge dialog drawer popover accordion toggle-group progress

# 개발
pnpm dev

# 타입 체크 (Contracts-first 검증, 매 PR 전)
pnpm tsc --noEmit

# 단위 테스트
pnpm vitest run
pnpm vitest run path/to/file.test.ts   # 단일 파일

# E2E
pnpm playwright test
pnpm playwright test --ui              # 디버그 UI
pnpm playwright test tests/flow-a.spec.ts   # 단일 플로우

# 린트 / 포맷
pnpm lint
pnpm format
```

이 표는 스캐폴드 생성 시 실측 명령어로 갱신한다 (§12 의무에 포함).

---

## 7. 디렉토리 가이드

### 7.1 현재 구조

```
C:\DEV\SAJU\
├─ fluttering-gathering-island.md   # 서비스 기획서 (v1.1 잠금)
├─ PRD.md                           # UI 디자이너 PRD (v1.1 잠금)
├─ compatibility_scoring_spec.md    # ADR-035 점수 결정형 명세
├─ tech_stack.md                    # ADR-037 스택 잠금
├─ FRONTEND-PREP.md                 # 프로토타입 → 프로덕션 마이그레이션 핸드북
├─ docs/                            # 보조 spec/runbook/legal/pattern (인덱스: docs/README.md)
├─ src/types/                       # Contracts-first stubs (index, mode, relation, chart, hapcard, scoring, prompt)
├─ scripts/                         # 운영 스크립트 (verify-ssaju-accuracy.ts)
├─ tests/fixtures/                  # G0 게이트 KASI reference (placeholder)
├─ prompts/                         # banned_phrases catalog + 6모드 system prompt scaffolds
├─ .env.example                     # 환경변수 카탈로그
├─ .github/workflows/               # CI (typecheck, lint, vitest)
├─ .gitignore
├─ UIDesign/                        # Babel CDN 기반 React 프로토타입 (수정 금지)
└─ CLAUDE.md                        # 본 파일
```

> fortune_architecture.md(v3.3)는 2026-05-03 폐기. 유효 내용은 `docs/specs/`, `docs/runbooks/`, `docs/legal/`, `docs/patterns/`로 분산 추출.

### 7.2 미래 구조

스캐폴드 후 디렉토리는 `FRONTEND-PREP.md` §2 참조. `src/types/` 이미 생성됨 — Contracts-first 시작점 (`C:\DEV\CLAUDE.md`).

---

## 8. 도메인 용어 사전 (프로젝트 전용)

`C:\DEV\CLAUDE.md` "Terminology Consistency" 표를 확장한다. 한 개념 = 한 용어 원칙은 동일.

| 개념 | 사용 | 사용 금지 |
|---|---|---|
| 사용자 | `user` | `member`, `account` |
| 인연 (CRM 대상) | `relation` | `contact`, `friend`, `partner`, `target` |
| 인연 별명 | `nickname` | `name`, `displayName` (UI 라벨도 "별명") |
| 케미카드 (결과 카드) | `hapcard` | `result-card`, `compat-card`, `합카드` |
| 합점수 | `compatScore` (코드) / "합게이지" (UI) | `score`, `rating` |
| 6모드 | `mode` — `'일합' \| '친구합' \| '돈합' \| '첫합' \| '썸합' \| '오래합'` | `category`, `type` |
| 본명식 | `chart` — `chart_core`, `userChart` | `birthChart`, `natal` |
| 일주 | `ilju` | `dayPillar` |
| 오행 | `ohaeng` | `fiveElements`, `wuxing` |
| 십신 | `sipsin` | `tenGods` |
| 만세력 | `manseryeok` | `lunarCalendar` |
| 합·형·충·해 | `hapChungHyungHae` (코드 키) | 영문 분리 식별자 |
| UI 소프트 alias | `끌림/긴장/부딪힘/소모` (display_label) — 합→끌림, 형→긴장, 충→부딪힘, 해→소모. GlossaryKey는 classical(`합\|형\|충\|해`) 유지, UI 표면만 소프트 용어 | — |
| 오늘 케미 | `todayHap` | `dailyFortune`, `오늘합` |
| 딥합 (깊이 리포트) | `deepHap` | `report`, `deepReport` |
| 케미피드 (인연 그리드) | `feed` | `list`, `grid` (라우트 키), `합피드` |
| 케미 다시 맞추기 (재해석) | `replay` | `reInterpret`, `다시합`, `그럴리 없어! 다시` |
| 또 다른 나 (자기진단 6모드) | `whatif` (DiagnosticType) | `만약에 우리`, `마이플레이`, `만약합` |
| 인연 슬롯 / 등록비 (3번째+ 정가 1,100원/현금 550원) | `relation_slot` (feature_id) | `사람칸`, `person-slot`, `seat`, `quota` |
| 지장간 | `jijanggan` | `hiddenStems`, `장간` (코드 식별자) |
| 신강약 | `sinkang` — `'신강' \| '중화' \| '신약'` | `dayStrength`, `strength` |
| 용신 | `yongsin` (희신 `huisin`) | `usefulGod`, `favorableElement` |
| 궁위 | `gungwi` — 년주/월주/일주/시주 | `palace`... 단 LLM payload 키 `palace`/`palace_meaning`은 전송용 영문 키로 예외 (`palace_name` 금지 — PII 키 스캔) |

새로운 도메인 용어 추가 시 본 표를 갱신하며 §1.1 승인 절차 적용.

---

## 9. 미결정 항목 (R1-R7)

`MEMORY.md` 의 `project_open_questions.md` 가 권위 있는 출처. 미결정 항목과 관련된 코드는 **결정 전까지 구현 보류**한다 (스텁·플레이스홀더 금지). 해당 영역 작업 요청 시 §1.1 적용하여 결정 시점부터 확인.

---

## 10. 검증 스킬 매핑 (필수 규칙 §1.2 운용)

| 작업 종류 | 우선 호출 스킬 | 보조 |
|---|---|---|
| 신규 컴포넌트·페이지 구현 | `/qa-only` → 결과 보고 → 수정 → `/qa` | `/design-review` (시각 QA) |
| API 라우트 / Server Action | `/qa` (단위 테스트 포함) | `/codex` (보안 2차 검토) |
| 시각 변경 (Tailwind, 토큰) | `/design-review` | `/browse` (실 페이지 캡처) |
| 결제 / 토스페이먼츠 흐름 | `/cso` (보안 감사) → `/qa` | `/codex` (도전) |
| 만세력·점수 결정형 로직 | Vitest deterministic 테스트 + `/codex consult` | — |
| LLM 프롬프트 변경 | banned_phrases 회귀 코퍼스 + `/codex challenge` | — |
| PR 머지 직전 | `/review` | `/autoplan` |
| 배포 후 모니터링 | `/canary` | `/benchmark` |

스킬 호출 결과를 사용자에게 요약 보고. PASS/FAIL 명시.

---

## 11. 컴팩션 규칙 (필수 규칙 §1.4 운용)

압축 시 반드시 보존:
- 현 세션에서 수정한 파일 목록
- 작업 중인 `types/*.ts` 인터페이스 (Contracts-first)
- 활성 ADR 번호와 결정 상태
- `C:\DEV\CLAUDE.md` 및 본 파일 내용
- `fluttering-gathering-island.md` v1.1 갱신 진행 상황
- `MEMORY.md` 의 미결정 항목 (R1-R7) 변동
- 미해결 사용자 질문

압축 산출물은 메모리 시스템(`C:\Users\batis\.claude\projects\C--DEV-SAJU\memory\`)에 작성하며, `MEMORY.md` 인덱스에 한 줄 추가.

---

## 12. 변경 매트릭스 의무 (ADR-037)

스택·핵심 정책 변경 시 다음 파일을 **동시에** 갱신해야 한다:

| 변경 | 갱신 대상 |
|---|---|
| 기술 스택 (라이브러리·모델·호스팅) | `tech_stack.md` + `fluttering-gathering-island.md` + `PRD.md` |
| ADR 신규/수정 | `fluttering-gathering-island.md` §17 ADR 표 + 본 파일 §3 |
| 도메인 용어 추가 | 본 파일 §8 + `C:\DEV\CLAUDE.md` 해당 표 |
| LLM 모델 매핑 변경 | `tech_stack.md` §3 + `prompt_versions` 테이블 + 카나리 절차 |
| 6모드 taxonomy | `fluttering-gathering-island.md` §4.2 + `PRD.md` §6 + `types/relation.ts` (`mode` enum) |
| `CLAUDE.md` §2/§3/§8 등 변경 | `AGENTS.md` 대응 섹션 동시 갱신 (Claude Code ↔ Codex 공유 진실 유지) |

부분 갱신 후 PR 제출 금지. 동시 갱신 누락 발견 시 §1.1 적용.

---

## 13. 메모 위치 (필수 규칙 §1.5 운용)

| 종류 | 위치 |
|---|---|
| 반복 실수 / 도메인 함정 | 메모리 시스템에 `feedback_*.md` 신규 작성 + `MEMORY.md` 인덱싱 |
| 미결정 사항 신규 발생 | `project_open_questions.md` 항목 추가 (R 번호 부여) + `MEMORY.md` 갱신 |
| 외부 API 차이 / 비결정형 발견 | 메모리 시스템에 `project_*.md` 작성, ADR 위반 우려 시 즉시 사용자 보고 |
| 단기 작업 노트 | `TaskCreate` (현 세션 한정) |

기록 → 다음 작업 진입 전 검색·확인이 의무. 같은 실수 두 번 발생 시 본 파일 §13에 한 줄 추가하여 영구화.

---

## 14. 참조

- `C:\DEV\CLAUDE.md` — 보편 규칙 (언어·터미놀로지·Contracts-first·Agent routing)
- `tech_stack.md` — ADR-037 잠금 스택 (단일 진실 출처)
- `FRONTEND-PREP.md` — 프로토타입 → 프로덕션 마이그레이션 (스캐폴드 시 1차 참조)
- `docs/README.md` — fortune_architecture.md(v3.3) 폐기 후 추출된 spec/runbook/legal/pattern 인덱스
- `docs/legal/pii_minimization.md` — PII 5필드 + gender 단일 truth source (ADR-004/ADR-011)
- `MEMORY.md` — 결정·미결정 인덱스 (`C:\Users\batis\.claude\projects\C--DEV-SAJU\memory\`)

## 15. Git 저장소

- **Remote**: `origin` = `git@github.com:DeepHighAI/todaychemi.git` (canonical, 유일 remote — 2026-06-10 `twoday`에서 리네임, 구 주소는 GitHub redirect)
- **기본 브랜치**: `main` (배포 Production 브랜치 — `docs/specs/secrets.md` §3)
- **커밋 규칙**: `C:\DEV\CLAUDE.md` "Git Conventions" 준수 — English, imperative mood, `type: description` (feat|fix|refactor|test|docs|chore), 72자 이내
- **시크릿 금지**: `.env.local` 등 런타임 시크릿은 `.gitignore` 등록 완료. 추적 파일에 실제 키 값 commit 절대 금지 (`docs/specs/secrets.md` 참조)
- **force-push / `main` 브랜치 삭제**: destructive 작업은 §1.1 사용자 승인 대상

## Skill routing

When the user's request matches an available skill, invoke it via the Skill tool. When in doubt, invoke the skill.

Key routing rules:
- Product ideas/brainstorming → invoke /office-hours
- Strategy/scope → invoke /plan-ceo-review
- Architecture → invoke /plan-eng-review
- Design system/plan review → invoke /design-consultation or /plan-design-review
- Full review pipeline → invoke /autoplan
- Bugs/errors → invoke /investigate
- QA/testing site behavior → invoke /qa or /qa-only
- Code review/diff check → invoke /review
- Visual polish → invoke /design-review
- Ship/deploy/PR → invoke /ship or /land-and-deploy
- Save progress → invoke /context-save
- Resume context → invoke /context-restore
