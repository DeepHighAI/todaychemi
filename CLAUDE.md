...
컨텍스트 사용량이 60%를 넘으면:
1. 진행 중 결정·미결 질문·다음 단계를 `MEMORY.md` 인덱스에 추가하거나 갱신
2. 작업 중인 파일 경로·타입 인터페이스·활성 ADR을 별도 메모리 파일로 저장
3. 사용자에게 압축 사실을 알리고 새 세션 시작을 권유
4. 새 세션에서 메모리 파일을 먼저 읽어 컨텍스트 복원 후 작업 재개
...
- **Phase 0 G0 진입 전** — 만세력 엔진(KASI + ssaju + manseryeok-js) 다중 검증 단계
- **Production 스캐폴드 미생성** — `package.json` 없음, `pnpm create next-app` 미실행
- 현재 디렉토리에는 **스펙 문서 + UI 프로토타입만** 존재
- `UIDesign/` 는 Babel CDN 기반 참조 프로토타입 — **수정 금지**, 프로덕션은 새 Next.js 프로젝트로 별도 생성

스캐폴드 생성 시점은 사용자가 결정한다 (§1.1 적용).
