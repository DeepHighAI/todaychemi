...
export function buildRagQueryText(input: BuildHapcardInput): string {
  ...
  // resolveDerivedForLlm 경유 의무 — jsonb 변형 derived 도 Zod 검증 후 fail-open(생략)되어
  // 유료 합카드 빌드가 RAG 토큰 때문에 죽지 않는다. derived 미확보 시 기존 텍스트 그대로 (회귀 0).
  if (self.derived) {
    const llmDerived = resolveDerivedForLlm(self);
    if (llmDerived) {
