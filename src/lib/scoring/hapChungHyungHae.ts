...
export interface HapChungEvent {
  type:
    ...
    | 'hae';
  score: number;
  pillarIndex?: number;   // 0=연주 1=월주 2=일주 3=시주
...
export function computeHapChungHyungHaeRaw(self: ChartCore, rel: ChartCore): HapChungEvent[] {
  ...
  for (const triple of HYUNG_TRIPLES) {
    if (triple.every((b) => combined.includes(b))) {
      events.push({ type: 'hyung', score: HYUNG_SCORE });
    }
  }

  // 자형: 같은 기둥 위치에서 동일 자형 지지 쌍
  for (const { sp, rp } of pairs) {
    const sb = sp[1];
    const rb = rp[1];
  ...
  for (const group of SAMHAP) {
    const matchCount = group.full.filter((b) => combined.includes(b)).length;
    if (matchCount === 3) {
      // 전체 삼합 — 중복 반합 억제
      events.push({ type: 'samhap_full', score: group.fullScore });
      ...
      if (inSelf >= 1 && inRel >= 1) {
        events.push({ type: 'samhap_half', score: group.halfScore });
      }
    }
  }
