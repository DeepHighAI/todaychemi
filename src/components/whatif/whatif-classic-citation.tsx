      ...
      <ul className="space-y-4">
        {citations.map((c, i) => (
          <li key={i} className="border border-border rounded-xl p-3 space-y-1">
            <p className="text-xs font-medium text-primary">{convertHanja(c.source_title)}</p>
            <p className="text-xs text-muted-foreground">{convertHanja(c.source_chapter)}</p>
            <p className="text-sm text-foreground italic">{c.original_text}</p>
            <p className="text-xs text-muted-foreground">{convertHanja(c.modern_translation)}</p>
          </li>
        ))}
      </ul>
    </div>
